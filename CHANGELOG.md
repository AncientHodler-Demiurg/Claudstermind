...
### Added
- **Tray icon + live status dot, minimize-to-tray, and a real desktop shortcut for the server app.** The
  Electron control panel now lives in the system tray with a coloured health dot (green/amber/red) that a
  single main-process poller keeps current — so status is visible **even when the window is closed**, and the
  window itself just listens for pushes instead of polling on its own. Closing the window **hides it to the
  tray** (right-click → *Open control panel* / *Quit app*); the two systemd services keep running regardless,
  and only *Quit* exits the app. Ships an app icon (`control/electron/icon.png`, generated by the same pure-
  Node PNG encoder as the tray dot) wired into the window and the `.desktop` launcher, and the launcher is now
  installed to both the app menu and the Desktop as a double-clickable shortcut. Purely additive — nothing in
  the running dashboard or engine is touched; `npm install` once (adds only Electron; existing deps stay
  lockfile-pinned) then `npm run app` or double-click the shortcut.
- Launches with `--no-sandbox` (baked into `main.cjs` and both launchers): Ubuntu 24.04+/26.04 restrict
  unprivileged user namespaces via AppArmor, which otherwise aborts a run-from-source Electron at startup.
  Safe here — the renderer only loads our own local HTML with `contextIsolation` on, no remote content.
