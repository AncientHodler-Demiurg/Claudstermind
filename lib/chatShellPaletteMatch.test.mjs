...
/**
 * The declarations of the rule whose selector list CONTAINS this needle. The needle is a selector,
 * not the whole prelude: it used to have to match up to and including the `{`, so adding a selector
 * to a rule — `.pactm-cockpit` joining Pact's palette, which is a widening, not a change — failed
 * here as "selector not found" and said nothing about any colour. Anchor on the selector we care
 * about and let the rule grow around it.
 */
function block(src, selectorNeedle) {
  const needle = selectorNeedle.replace(/\s*\{\s*$/, "");
  const i = src.indexOf(needle);
  assert.ok(i >= 0, `selector not found: ${needle}`);
