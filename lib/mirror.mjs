...
//
// Navigations are excluded from both: a mistyped dashboard URL must 404 on the dashboard
// rather than turn into the mirrored site.

const isNavigation = (get) => String(get("sec-fetch-mode") || "").toLowerCase() === "navigate";
const allow = (port, allowedPorts) =>
  !port ? null : (allowedPorts && !allowedPorts.includes(port) ? null : port);

/** The mirror this request demonstrably came from, per its Referer. */
export function mirrorFromReferer(headers = {}, { allowedPorts = null } = {}) {
  const get = (n) => headers[n] ?? headers[n.toLowerCase()];
  if (isNavigation(get)) return null;
 ...
 * sub-resource rather than the page — a stylesheet's `@import`, a module's static import.
 * Only safe once nothing else has claimed the path.
 */
