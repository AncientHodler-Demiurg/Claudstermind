...
const isJs = (contentType) => /(java|ecma)script/i.test(contentType || "");

/**
 * Prefix ROOT-ABSOLUTE ES-module import specifiers inside a served JS body with `/mirror/<port>/`.
 *
 * The HTML fix (rewriteRootAbsoluteAssets) gets the ENTRY module (`/src/main.tsx`) to load, but that
 * module's own body then does `import "/node_modules/.vite/deps/react.js"`, `import { App } from
 * "/src/App.tsx"`, `import "/@fs/…"` — all ROOT-ABSOLUTE, all issued by the browser's native module
 * loader (which the runtime fetch/XHR patch cannot intercept). Those resolve against the ORIGIN, so
 * through the mirror they land on the relay root, not under `/mirror/<port>/`, and reach the dev
 * server ONLY via Referer/cookie provenance. Proven locally: the app mounts fine when provenance
 * survives, but a nested module fetched with NO Referer and NO cookie → 404; the same module fetched
 * at the explicit `/mirror/<port>/…` path → 200 unconditionally. Over the HTTPS relay to a phone the
 * provenance does not reliably survive (an intervening proxy or the browser's framed-context rules
 * drop it), so every nested module 404s and the app never mounts: the white page.
 *
 * Rewriting the specifiers to the explicit prefix removes the dependency on provenance entirely — each
 * nested module is requested at a `/mirror/<port>/…` URL that routes by PATH, which needs no Referer
 * and no cookie. Covers the three specifier positions Vite/ESM emit: `… from "X"`, dynamic
 * `import("X")`, and bare `import "X"`. A specifier can't contain whitespace, so restricting the path
 * to non-space, non-quote chars keeps the rewrite off prose that merely contains the word "from".
 * Only a single leading `/` is bent; `//host`, `http(s)://…`, already-`/mirror/…`, and relative
 * specifiers are left byte-identical. Query strings (Vite's `?v=…`, `?import`) ride along untouched.
 */
export function rewriteJsImports(js, port) {
  const prefix = `/mirror/${port}`;
  const bend = (m, pre, q, spec) => {
    if (spec.startsWith("//")) return m;                                // protocol-relative → leave
    if (spec === prefix || spec.startsWith(prefix + "/")) return m;     // already mirror-prefixed
    return `${pre}${q}${prefix}${spec}${q}`;
  };
  return String(js)
    .replace(/(\bfrom\s*)(["'])(\/[^"'\s]*)\2/g, bend)                  // import/export … from "/…"
    .replace(/(\bimport\s*\(\s*)(["'])(\/[^"'\s]*)\2/g, bend)           // dynamic import("/…")
    .replace(/(\bimport\s+)(["'])(\/[^"'\s]*)\2/g, bend);               // bare import "/…"
}
...
/**
 * Prefix ROOT-ABSOLUTE `src`/`href` on `<script>` and `<link>` tags with `/mirror/<port>/`.
 *
 * `<base href>` only bends *relative* URLs; a root-absolute one like `<script src="/src/main.tsx">`
 * (exactly what a Vite dev server emits for its entry, alongside `/@vite/client`) ignores `<base>`
 * entirely and resolves against the ORIGIN — so through the mirror it becomes `https://host/src/main.tsx`,
 * NOT `.../mirror/<port>/src/main.tsx`. And the runtime fetch/XHR patch can't save it either: the HTML
 * parser fetches a literal `<script src>` directly, before any of our JS runs.
 *
 * That left the entry module reachable only via server-side provenance routing (Referer/cookie). On a
 * plain local http load that holds, but through the HTTPS relay in a phone browser it does not reliably
 * hold — the entry request's Referer/cookie can be reduced or dropped by the intervening proxy or the
 * browser's framed-context rules — so the entry module 404s and the app never mounts: a white page.
 * Reproduced directly: `GET /src/main.tsx` → 404 with no Referer, 200 only with a mirror Referer; the
 * explicit `GET /mirror/<port>/src/main.tsx` → 200 unconditionally.
 *
 * Rewriting the entry tags to that explicit prefix makes them load deterministically, with no
 * provenance guessing — AND it bootstraps the whole downstream module graph: each module is now loaded
 * from a `/mirror/<port>/…` URL, so the Referer on its own nested `import`s carries the prefix too.
 * Only a single leading `/` is touched; `//host`, `http(s)://…`, already-`/mirror/…`, and relative
 * URLs are all left exactly as they were.
 */
export function rewriteRootAbsoluteAssets(html, port) {
  const prefix = `/mirror/${port}`;
  return html.replace(/<(?:script|link)\b[^>]*>/gi, (tag) =>
    tag.replace(/\b(src|href)\s*=\s*(["'])([^"']*)\2/gi, (whole, attr, q, val) => {
      if (!val.startsWith("/") || val.startsWith("//")) return whole;   // relative or protocol-relative/absolute → leave
      if (val === prefix || val.startsWith(prefix + "/")) return whole;  // already mirror-prefixed → don't double it
      return `${attr}=${q}${prefix}${val}${q}`;
    }));
}
 ...
 * has client-side-navigated away from looking mirrored), AND prefix root-absolute `<script>`/`<link>`
 * asset URLs (see rewriteRootAbsoluteAssets — the entry-module white-page fix). Together they cover
 * every URL shape a mirrored page produces; a full top-level navigation after the address bar has
...
export function injectBase(html, port) {
  html = rewriteRootAbsoluteAssets(html, port);        // entry <script src>/<link href> → explicit prefix
...
export function buildMirrorResponse(upstream, port) {
  ...
  else if (isJs(ct)) body = Buffer.from(rewriteJsImports(body.toString("utf8"), port), "utf8");
