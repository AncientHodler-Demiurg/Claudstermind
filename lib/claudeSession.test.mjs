...
test("status re-arms to 'thinking' when the SDK keeps producing turn content after an early result — a backgrounded tool/task settling with no new prompt sent", async () => {
  ...
  // button looks ready while the agent keeps delivering answers.
  ...
  const rearmed = events.slice(0, secondAssistantIdx).some((e) => e.kind === "status" && e.status === "thinking"
    && events.slice(0, secondAssistantIdx).indexOf(e) > events.findIndex((x) => x.kind === "result"));
  assert.ok(rearmed, `a "status":"thinking" event must re-arm the busy indicator before the continuation's content — got: ${events.map((e) => e.kind + (e.kind === "status" ? ":" + e.status : "")).join(" -> ")}`);
});
