...
import { ClaudeSession, cleanClaudeEnv, toEvent, addUsage, emptyUsage, toolResultText, contentBlocks } from "./claudeSession.mjs";
...
test("a Bash call shaped like the leaked-poll-loop bug is denied by the wired PreToolUse hook", async () => {
  // Real regression: 11 background bash jobs in one Pact session ended up polling files that had
  // stopped changing hours earlier — each an `until ...; do sleep N; done` loop with no ceiling of
  // its own. This proves the guard (lib/bashPollGuard.mjs) is actually WIRED into what a live
  // session sends the SDK, not just correct in isolation.
  let seenOptions = null;
  const capQuery = function ({ prompt, options }) {
    seenOptions = options;
    return (async function* () { const it = prompt[Symbol.asyncIterator](); await it.next(); yield { type: "result", subtype: "success", usage: {}, total_cost_usd: 0 }; })();
  };
  const s = new ClaudeSession({ key: "kh", cwd: "/repo", sdkQuery: capQuery, onEvent: () => {} });
  s.prompt("hi");
  await s.start();
  const matcher = seenOptions?.hooks?.PreToolUse?.find((m) => m.matcher === "Bash");
  assert.ok(matcher, "a Bash-matched PreToolUse hook must be registered");
  const denied = await matcher.hooks[0]({ tool_name: "Bash", tool_input: { command: "until grep -q done out; do sleep 5; done" } });
  assert.equal(denied?.hookSpecificOutput?.permissionDecision, "deny");
  const allowed = await matcher.hooks[0]({ tool_name: "Bash", tool_input: { command: "npm test" } });
...
test("autoWrap defaults ON, setAutoWrap(false) flips it and emits, and a no-op call emits nothing", () => {
  const events = [];
  const s = new ClaudeSession({ key: "kaw", cwd: "/repo", onEvent: (k, ev) => events.push(ev) });
  assert.equal(s.autoWrap, true, "a session must default to auto-wrap ON — this is not a new opt-in, only a new opt-OUT");
  assert.equal(s.setAutoWrap(false), false);
  assert.equal(s.autoWrap, false);
  assert.deepEqual(events.map((e) => e.kind), ["autoWrap"]);
  assert.equal(events[0].autoWrap, false);
  // Setting the SAME value again must be a no-op — no duplicate event, matching setFastMode's own
  // "next === this.fastMode" short-circuit one function up.
  s.setAutoWrap(false);
  assert.equal(events.length, 1, "re-setting the same value must not emit a second event");
  assert.equal(s.setAutoWrap(true), true);
  assert.equal(events.length, 2);
});

test("a session constructed with autoWrap:false starts opted out, without needing setAutoWrap called", () => {
  const s = new ClaudeSession({ key: "kaw2", cwd: "/repo", autoWrap: false });
  assert.equal(s.autoWrap, false);
});
...
test("a respawn whose interrupt()/return() REJECTS does not leak an unhandledRejection", async () => {
  // REGRESSION (seen live in the sessiond journal, 4x in one afternoon): _requestRespawn wrapped
  //   try { this._q?.interrupt?.(); } catch {}
  // but interrupt() is ASYNC — a bare try/catch only catches a synchronous throw. During a roll the
  // query stream has just been ended, so the subprocess transport is already down and interrupt()
  // REJECTS with "ProcessTransport is not ready for writing". That rejection escaped as an
  // unhandledRejection on every roll and every lane-switch respawn. sessiond has a keep-alive net so
  // nothing crashed, but any host without one would have died. A dead transport is the state we are
  // trying to REACH, so the failure is expected and must be swallowed, not propagated.
  const spawns = [];
  const sdk = function ({ prompt, options }) {
    const idx = spawns.length;
    const gen = (async function* () {
      const it = prompt[Symbol.asyncIterator]();
      await it.next();
      yield { type: "system", subtype: "init", session_id: "s-" + idx, model: "m", cwd: "/repo" };
      yield { type: "result", subtype: "success", is_error: false, usage: {}, total_cost_usd: 0, duration_ms: 1, result: "ok" };
      await it.next();   // park
    })();
    // Both control methods reject, exactly like a live SDK query over a closed transport.
    gen.interrupt = () => Promise.reject(new Error("ProcessTransport is not ready for writing"));
    gen.return = () => Promise.reject(new Error("ProcessTransport is not ready for writing"));
    spawns.push(gen);
    return gen;
  };
  ...
  try {
    const s = new ClaudeSession({ key: "kreject", cwd: "/repo", sdkQuery: sdk, onEvent: () => {} });
    s.prompt("hi");
    const started = s.start();
    await new Promise((r) => setTimeout(r, 40));
    assert.equal(spawns.length, 1, "first spawn happened");

    assert.equal(s.roll("SEED"), true, "roll accepted even though the transport is dead");
    await new Promise((r) => setTimeout(r, 80));   // let any unhandled rejection surface

    assert.deepEqual(leaked.map(String), [], "no rejection escaped _requestRespawn");
    assert.equal(spawns.length, 2, "the respawn still happened — a failed interrupt must not block it");
    await s.stop().catch(() => {});
    await started.catch(() => {});
    await new Promise((r) => setTimeout(r, 30));
    assert.deepEqual(leaked.map(String), [], "still clean after stop()");
  } finally {
...
// ---------------------------------------------------------------------------------------------
// 1.5.97 — "parts.filter is not a function". A message's `content` may be a plain STRING, not an
// array of blocks; the Messages API allows both and which you get varies by model/turn. The old
// code did `(msg.message?.content) || []`, and a non-empty string is TRUTHY, so it sailed straight
// through to `.filter` and threw. Because toEvent() runs INSIDE the `for await` stream loop, that
// throw escaped to the catch, set status "error" and KILLED THE WHOLE TURN.
// ---------------------------------------------------------------------------------------------

test("1.5.97: contentBlocks normalises every content shape the API can send", () => {
  assert.deepEqual(contentBlocks([{ type: "text", text: "a" }]), [{ type: "text", text: "a" }]);
  // the regression: a bare string becomes ONE synthetic text block, which is what the API means by it
  assert.deepEqual(contentBlocks("hello"), [{ type: "text", text: "hello" }]);
  assert.deepEqual(contentBlocks(""), []);          // empty string is not a block
  assert.deepEqual(contentBlocks(null), []);
  assert.deepEqual(contentBlocks(undefined), []);
  assert.deepEqual(contentBlocks(42), []);          // never throws, degrades to nothing
  assert.deepEqual(contentBlocks({ type: "text", text: "solo" }), [{ type: "text", text: "solo" }]);
});

test("1.5.97: toEvent handles STRING content without throwing (the actual reported bug)", () => {
  // Before the fix this threw: TypeError: parts.filter is not a function
  const ev = toEvent({ type: "assistant", message: { content: "plain string reply" } });
  assert.equal(ev.kind, "assistant");
  assert.equal(ev.text, "plain string reply");
  // the user-message path had the identical bug
  assert.equal(toEvent({ type: "user", message: { content: "just text, no tool results" } }), null);
  // and the array path must be untouched
  assert.equal(toEvent({ type: "assistant", message: { content: [{ type: "text", text: "hi" }] } }).text, "hi");
});

test("1.5.97: a malformed message SKIPS and warns — it must never kill a live turn", async () => {
  // A stream where one message blows up the translator. The turn must still deliver the assistant
  // text that comes AFTER it, and must not end in status "error".
  const boom = { type: "assistant", get message() { throw new Error("synthetic translate failure"); } };
  const sdkQuery = function ({ prompt }) {
    return (async function* () {
      const it = prompt[Symbol.asyncIterator]();
      await it.next();
      yield { type: "system", subtype: "init", session_id: "s1", model: "m", cwd: "/tmp" };
      yield boom;                                                            // <- would have been fatal
      yield { type: "assistant", message: { content: "survived" } };         // <- and a string, for good measure
      yield { type: "result", subtype: "success", usage: {}, total_cost_usd: 0 };
    })();
  };
  const events = [];
  const s = new ClaudeSession({ cwd: "/tmp", sdkQuery, onEvent: (_key, e) => events.push(e) });
  s.start();
  s.prompt("go");
  await new Promise((r) => setTimeout(r, 60));

  const warn = events.find((e) => e.kind === "warning");
  assert.ok(warn, "the skipped message must be surfaced as a warning, not silently dropped");
  assert.match(warn.message, /Skipped one malformed engine message/);
  assert.equal(warn.scope, "translate");

  // the turn kept going past the bad message
  assert.ok(events.some((e) => e.kind === "assistant" && e.text === "survived"),
    "content after the malformed message must still arrive");
  assert.ok(events.some((e) => e.kind === "result"), "the turn must still reach its result");
  assert.ok(!events.some((e) => e.kind === "error"),
    "a translator hiccup must NOT be reported as a turn failure");
  await s.stop();
});

// ---------------------------------------------------------------------------------------------
// DOCUMENT ATTACHMENTS. The SDK types a user message as a Messages API MessageParam whose content may
// contain "text, image, document, tool_result, …" — so the platform always accepted documents and we
// simply never emitted one. Claude Desktop and DMP both use that path.
// ---------------------------------------------------------------------------------------------

async function captureFirstUserMessage(attachments) {
  let sent = null;
  const sdkQuery = function ({ prompt }) {
    return (async function* () {
      const it = prompt[Symbol.asyncIterator]();
  ...
  const s = new ClaudeSession({ cwd: "/tmp", sdkQuery, onEvent: () => {} });
  s.start();
  s.prompt("look at this", attachments);
  await new Promise((r) => setTimeout(r, 40));
  await s.stop();
  return sent;
}

test("a PDF is sent as a document block with a base64 source", async () => {
  const sent = await captureFirstUserMessage([
    { kind: "document", mediaType: "application/pdf", sourceKind: "base64", base64Data: "JVBERi0=", title: "spec.pdf" },
  ]);
  const blocks = sent.message.content;
  assert.ok(Array.isArray(blocks), "attachments force the array form");
  assert.equal(blocks[0].type, "text", "the prompt text still leads");
  assert.equal(blocks[1].type, "document", "a PDF must be a document block, not an image");
  assert.equal(blocks[1].source.type, "base64");
  assert.equal(blocks[1].source.media_type, "application/pdf");
  assert.equal(blocks[1].title, "spec.pdf", "a title rides along so the model can refer to it by name");
});

test("a text document is sent as a TEXT source, not base64", async () => {
  // The API expects `type: "text"` for plain-text documents; base64-ing them would be wrong.
  const sent = await captureFirstUserMessage([
    { kind: "document", mediaType: "text/markdown", sourceKind: "text", textData: "# notes" },
  ]);
  const doc = sent.message.content[1];
  assert.equal(doc.type, "document");
  assert.equal(doc.source.type, "text");
  assert.equal(doc.source.media_type, "text/markdown");
  assert.equal(doc.source.data, "# notes");
});

test("images still travel exactly as before — documents are additive", async () => {
  const sent = await captureFirstUserMessage([
    { mediaType: "image/png", base64Data: "iVBOR" },
  ]);
  const img = sent.message.content[1];
  assert.equal(img.type, "image");
  assert.equal(img.source.type, "base64");
  assert.equal(img.source.media_type, "image/png");
});

test("mixed attachments keep their order and their kinds", async () => {
  const sent = await captureFirstUserMessage([
    { mediaType: "image/png", base64Data: "a" },
    { kind: "document", mediaType: "application/pdf", sourceKind: "base64", base64Data: "b" },
    { mediaType: "image/webp", base64Data: "c" },
  ]);
  assert.deepEqual(sent.message.content.map((b) => b.type), ["text", "image", "document", "image"]);
});

test("no attachments still sends a plain string — the common case is unchanged", async () => {
  const sent = await captureFirstUserMessage(undefined);
  assert.equal(typeof sent.message.content, "string", "a text-only prompt must not become an array");
});
