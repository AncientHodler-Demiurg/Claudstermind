...
class FakeNode {
  ...
  // append MOVES a node, as the real DOM does — it detaches from its previous parent first. Without
  // that, a portaled node appears to be in two places at once and "did it come back?" is unanswerable.
  append(...kids) {
    for (const k of kids) {
      if (k === "" || k == null) continue;
      if (k && typeof k === "object" && k.parentNode && k.parentNode !== this) {
        k.parentNode.childNodes = k.parentNode.childNodes.filter((c) => c !== k);
      }
  ...
  scrollIntoView() {}
  contains(n) { return n === this || this.childNodes.some((c) => c && typeof c === "object" && c.contains && c.contains(n)); }
  // The disclosure panel is positioned as a POPOVER against the real rect of the control that opened
  // it, so the shim has to be able to answer "where are you" — zeros are fine (the maths clamps), but
  // the method has to exist or the render path throws instead of rendering.
  getBoundingClientRect() { return { left: 0, top: 0, right: 0, bottom: 0, width: 0, height: 0 }; }
  get offsetHeight() { return 0; }
}
const documentShim = {
  createElement: (tag) => new FakeNode(tag),
  // Both hosts put the chat inside an element carrying `contain: layout paint`, which makes it a
  // containing block for `position: fixed` — so an anchored popover left inside the pane is positioned
  // against the PANE, not the viewport, and lands off-screen. The panel is portaled to <body> while
  // open; the shim needs a body for that path to be exercised at all.
...
const windowShim = { innerWidth: 1400, innerHeight: 900, addEventListener() {} };
...
// --- T3.6 the disclosure panel is a POPOVER anchored to the control you clicked -------------------
//
// Reported from the live app: "clicking the context medallion should bring it in a popup and that is
// not the case." It wasn't: the panel rendered in flow inside `root`, and since the chat box became a
// package `root` sits in the HEADER while the control you click (the context readout) sits in the
// FOOTER's model row — so the breakdown opened at the opposite end of the pane from the cursor.
// These pin the popover behaviour, not the CSS.

const chipOf = (bar, cls) => {
  const walk = (n) => {
    if (!n || typeof n !== "object") return null;
    if (n.className && n.className.split(/\s+/).includes(cls)) return n;
    for (const k of n.childNodes || []) { const hit = walk(k); if (hit) return hit; }
    return null;
  };
  return walk(bar.root);
};

test("T3.6 opening the context panel makes it a popover, anchored to the VISIBLE proxy control", () => {
  const p = newConv();
  const bar = H.exoMountBar(p, ctxFor(p));
  const ctxChip = chipOf(bar, "exo-chip-ctx");
  const panel = chipOf(bar, "exo-panel");
  assert.ok(ctxChip && panel, "the bar must build a context chip and a panel");
  assert.ok(!panel.classList.contains("exo-panel--pop"), "closed: no popover positioning at all");

  // A workspace that hides this chip and drives it from its own visible readout declares that node
  // as `_visProxy` — the popup must anchor THERE, not to the hidden chip in the other region.
  const visible = el("button", { class: "ws-usage" }, ["93,016 / 1,000,000"]);
  ctxChip.hidden = true;
  ctxChip._visProxy = visible;

  ctxChip.fire("click");
  assert.equal(panel.hidden, false, "clicking must open the panel");
  assert.ok(panel.classList.contains("exo-panel--pop"),
    "…as a popover positioned against the control, not as an inline row in the header");
  assert.ok(panel.style.top !== "" && panel.style.left !== "", "…with real anchored coordinates");
  // …and OUT of the pane while open: the pane is a containing block for fixed positioning
  // (`contain: layout paint`), so a popover left inside it is positioned against the pane and lands
  // off-screen — measured at x=1583 in a 1600px window before this.
  assert.ok(documentShim.body.childNodes.includes(panel),
    "an open popover must be portaled to <body>, or `position: fixed` is not against the viewport");

  ctxChip.fire("click");   // close again
  assert.equal(panel.hidden, true);
  assert.ok(!documentShim.body.childNodes.includes(panel), "…and returned to the bar when closed");
  assert.ok(!panel.classList.contains("exo-panel--pop"), "…with its popover positioning removed");
});

test("T3.6 a click outside closes the popover; a click inside it does not", () => {
  const p = newConv();
  const bar = H.exoMountBar(p, ctxFor(p));
  const ctxChip = chipOf(bar, "exo-chip-ctx");
  const panel = chipOf(bar, "exo-panel");
  ...
  documentShim.fire("mousedown", { target: panel });
  assert.equal(panel.hidden, false, "clicking INSIDE the breakdown must not dismiss it");

  documentShim.fire("mousedown", { target: el("div", {}, []) });
  assert.equal(panel.hidden, true, "clicking anywhere else must dismiss it");
});

test("T3.6 Escape closes the popover", () => {
  const p = newConv();
  const bar = H.exoMountBar(p, ctxFor(p));
  const ctxChip = chipOf(bar, "exo-chip-ctx");
  const panel = chipOf(bar, "exo-panel");
  ctxChip.fire("click");
  assert.equal(panel.hidden, false);
  documentShim.fire("keydown", { key: "Escape" });
  assert.equal(panel.hidden, true);
});
