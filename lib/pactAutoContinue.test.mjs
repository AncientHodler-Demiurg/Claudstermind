...
/* ---- the loop has to survive leaving the VIEW ------------------------------------------------- */
// "The auto trigger on pact workspace doesn't trigger correctly, it triggers when I visited."
// It was not the decider: it was the engine being switched off. Navigating away from Pact called
// pactChatStop(), which closes the SSE stream and clears PACT_HEAL_TIMER — the stream is what
// delivers the result that ends a turn, and that timer is the watchdog that re-arms the loop. So with
// auto-continue on, going anywhere else in the dashboard froze the sweep: the tab stayed busy with
// nothing to say otherwise, nothing re-evaluated, the countdown never reached zero. Returning
// reopened both, the deadline was long past, and it fired at once. Measured with auto on one tab:
// stream readyState 1 on the view, `es: null` the moment you leave, still null six seconds later.
const stripComments = (s) => s.replace(/\/\*[\s\S]*?\*\//g, "").split("\n").filter((l) => !/^\s*\/\//.test(l)).join("\n");
const fnBody = (name) => {
  const at = src.indexOf("function " + name);
  assert.ok(at > 0, name + " was renamed — the assertions below are aimed at nothing");
  return stripComments(src.slice(at, src.indexOf("\n}", at)));
};

test("leaving the view keeps the engine running while a sweep is live", () => {
  const teardown = stripComments(src.slice(src.indexOf('if (VIEW !== "pact"'), src.indexOf('if (VIEW !== "usage")')));
  assert.match(teardown, /_autoContinue/,
    "the teardown must ask whether a sweep is running before switching the engine off");
  assert.match(teardown, /pactChatDetachUI\(\)/,
    "a live sweep must take the detach path, which keeps the stream and the watchdog");
  assert.match(teardown, /else\s+pactChatStop\(\)/,
    "…and everything else must still be torn down completely, as it always was");
});

test("the detach path drops what paints and keeps what drives", () => {
  const body = fnBody("pactChatDetachUI");
  assert.ok(!/PACT_HEAL_TIMER/.test(body),
    "clearing the watchdog is precisely the bug: it is what re-arms auto-continue after a turn ends");
  assert.ok(!/\.es\b|close\(\)/.test(body),
    "closing the stream leaves the tab busy forever — nothing else delivers the end of a turn");
  assert.match(body, /clearInterval\(PACT_TICK_TIMER\)/,
    "the elapsed readout paints a screen nobody is looking at and should stop");
});

test("a detached engine is closed before the view rebuilds one, or two streams deliver everything twice", () => {
  const body = fnBody("pactChatInit");
  const stopAt = body.indexOf("pactChatStop()"), assignAt = body.indexOf("PACT_CHAT = {");
  assert.ok(stopAt > 0 && assignAt > 0, "both the guard and the assignment must be present");
  assert.ok(stopAt < assignAt,
    "the survivor must be closed BEFORE the global is replaced — afterwards the only reference that " +
    "could close that EventSource is gone, and it is orphaned for the life of the page");
});

test("a cleared interval is nulled, so a stopped watchdog cannot read as a running one", () => {
  const body = fnBody("pactChatStop");
  for (const t of ["PACT_HEAL_TIMER", "PACT_TICK_TIMER", "PACT_STREAM_STALE_TIMER"])
    assert.match(body, new RegExp("clearInterval\\(" + t + "\\);\\s*" + t + " = null"),
      t + " keeps a stale, truthy id after clearInterval, so nothing can tell a running watchdog " +
      "from a stopped one — a probe written to check exactly that was misled by it");
});
