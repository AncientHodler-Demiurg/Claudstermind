...
test("the type field is one step above its surface, by the same amount in both workspaces", () => {
  // "it's its own rectangle, whereas on pact workspace it's incorporated in the background."
  // `--field` resolves to `--panel-2`, and that is a much bigger jump in Core's palette than in
  // Pact's: measured on a phone, Core's box sat +11/+17/+28 off its background where Pact's sat
  // +9/+11/+14. Same component, same rule, two very different readings — one a panel dropped onto the
  // footer, the other part of it. Deriving the step from `--panel` makes it identical in both
  // (measured after: +13/+13/+14 and +13/+13/+13), the way `--core-bg` is derived in theme.css.
  const box = declOf("mc-box");
  assert.ok(!/background:\s*var\(--field\)/.test(box),
    "--field is --panel-2, whose distance from --panel differs per workspace — that difference IS the bug");
  assert.match(box, /background:\s*color-mix\(in srgb,\s*var\(--panel\)/,
    "the field must be derived from the surface it sits on, so the step cannot differ between palettes");
});
