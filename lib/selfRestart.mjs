 ...
 *
 *  Restarts BOTH units: `claudstermind-sessiond` (the session engine) AND `claudstermind` (the web).
 *  This is the fix for "Reload didn't pick up engine changes": the engine lives in the sessiond
 *  daemon, which deliberately SURVIVES a web restart (so routine deploys don't interrupt agents) —
 *  but that meant a Reload, whose whole purpose is "run the current on-disk code," silently left the
 *  engine (lib/workspace.mjs, lib/claudeSession.mjs, …) on stale code. A Reload is an explicit,
 *  deliberate act — its own banner already warns running agents are interrupted — so it now restarts
 *  the engine too, and genuinely picks up ALL on-disk code. The daemon is listed FIRST so it's on its
 *  way back up by the time the web returns and its SessiondClient reconnects. (Routine agent-
 *  preserving updates remain the DEPLOY path's job, which stays web-only unless a daemon file changed.)
 *
...
export function restartCommand() {
  return { cmd: "sudo", args: ["-n", "systemctl", "restart", "claudstermind-sessiond", "claudstermind"] };
}
