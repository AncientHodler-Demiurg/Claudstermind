 ...
 *
 *  Restarts the web (`claudstermind`) and — only when `daemon` is true — the session engine
 *  (`claudstermind-sessiond`) too. The engine lives in the sessiond daemon, which deliberately
 *  SURVIVES a web restart so routine reloads/deploys don't interrupt running agents. A Reload that
 *  needs to pick up ENGINE code (lib/workspace.mjs, lib/claudeSession.mjs, …) MUST restart the daemon
 *  — which interrupts in-flight turns — so the caller passes `daemon: true` only when engine code
 *  actually changed since the running process started. A web/client-only reload passes `daemon: false`
 *  and the engine keeps running, so a pending prompt mid-turn is NOT lost (the deploy-survivable-agents
 *  property). When both restart, the daemon is listed FIRST so it's on its way back up by the time the
 *  web returns and its SessiondClient reconnects.
 *
 ...
 *  `<user> ALL=(root) NOPASSWD: /usr/bin/systemctl restart claudstermind`, extended to also cover
 *  `claudstermind-sessiond`, or a broader `/usr/bin/systemctl` grant). */
export function restartCommand({ daemon = true } = {}) {
  const units = daemon ? ["claudstermind-sessiond", "claudstermind"] : ["claudstermind"];
  return { cmd: "sudo", args: ["-n", "systemctl", "restart", ...units] };
}
