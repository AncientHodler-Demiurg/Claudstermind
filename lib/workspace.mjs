...
export class WorkspaceManager {
  ...
  _prompt(sessionKey, { repo, worktree, text, trusted, mode, model, effort, fastMode, resume, fresh, by: from, image, images }) {
    ...
    if (!s) {
      ...
      // latest one automatically — a fresh "new chat" pane (the Core cockpit model, where a pane
      // IS a repo/worktree and there's one ongoing conversation per workspace) must continue
      // Claude's REAL context, not just show a transcript that looks continuous while the model
      // starts blank. But that assumption breaks for a surface that runs MANY independent
      // conversations under ONE workspace id (the Pact chat: Master + per-audit tabs all share
      // `Ouronet@main`) — there, auto-resuming "the workspace's latest" always means "resume
      // Master", so a genuinely-new chat silently inherits Master's context. Such a caller sends
      // `fresh: true` to opt OUT of the auto-seed and start truly blank; it still resumes its OWN
      // saved session when it later sends an explicit `resume`.
      const rawResumeId = resume || (fresh ? null : this._latestWorkspaceRow(workspaceId)?.sessionId) || null;
      // Last-line defense, right at the point of consumption: NEVER hand the SDK a resume value
      // equal to this workspace's own id — structurally impossible as a real Claude session id
      // (always a UUID), and exactly the shape of a real corruption that broke every single
      // prompt to a workspace with "--resume ... is not a UUID", regardless of source (an
      // explicit client-sent `resume`, or the auto-resume lookup above — both are covered here,
      // not just one of them).
      const resumeId = rawResumeId && rawResumeId !== workspaceId ? rawResumeId : null;
      s = new ClaudeSession({
      ...
      // Seed the pane's DISPLAYED transcript. Default (Core cockpit, one conversation per repo): the
      // full merged workspace history (every past session file for this repo+worktree, chronological)
      // so what's shown agrees with the context the SDK just resumed; fall back to the sessionKey-
      // scoped read (a same-key restart, before this key ever had a workspaceId) when empty. But a
      // `fresh` chat MUST start blank: the Pact chat runs MANY conversations under ONE workspace id,
      // so this merged seed would dump EVERY other conversation (Master + every audit tab) into a
      // brand-new tab — exactly the "new chat picked up Master" bug. For fresh, show only its own new
      // turn (its own key has no saved file yet anyway); resuming a SPECIFIC past chat still reads
      // just that one session via _openSession, never this merge.
      if (fresh) {
        s.transcript = [];
      } else {
  ...
  _control({ action, args = {} }) {
    ...
    switch (action) {
      ...
      case "resync": return this._resync(args.sessionKey);
  ...
  _contextUsage(sessionKey) {
    ...
    if (!s) return this.send("event", sessionKey, { kind: "contextUsage", usage: null });
    s.getContextUsage().then((usage) => this.send("event", sessionKey, { kind: "contextUsage", usage }));
  }
  ...
  _usageLimits(sessionKey) {
    ...
    s.getUsageLimits().then((limits) => this.send("event", sessionKey, { kind: "usageLimits", limits }));
  }
  ...
  _openSession({ repo, worktree, sessionId }) {
    if (!sessionId) return;
    ...
    this.send("transcript", sessionId, { sessionKey: sessionId, sessionId: found.sessionId || sessionId,
      workspaceId: wid, repo: found.repo, worktree: found.worktree, transcript, usage: found.usage || null });
  }
  ...
  _liveOrSavedState(sessionKey) {
    ...
    if (live) {
      return { sessionId: live.sessionId, repo: live.repoLabel || null, worktree: live.worktree || "main",
        workspaceId: live.workspaceId || sessionKey, usage: live.usage, status: live.status, transcript: live.transcript };
    }
    ...
    return { sessionId: row?.sessionId ?? bySession?.sessionId ?? null, repo: row?.repo ?? repo,
      worktree: row?.worktree ?? worktree, workspaceId, usage: bySession?.usage ?? null, status: "idle", transcript };
  }
  ...
  _openTranscript(sessionKey) {
    const s = this._liveOrSavedState(sessionKey);
