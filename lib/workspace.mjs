...
// (so the client knows there's more to fetch) and the true total.
function capTranscript(transcript, full) {
  const arr = transcript || [];
  if (full || arr.length <= WS_RESYNC_MSG_CAP) return { transcript: arr, transcriptTruncated: false, transcriptTotal: arr.length };
  return { transcript: arr.slice(-WS_RESYNC_MSG_CAP), transcriptTruncated: true, transcriptTotal: arr.length };
}
