...
export class WorkspaceManager {
  ...
  _onEvent(sessionKey, ev) {
    ...
    if (s) {
      ...
      if (ev.kind === "result") {
        ...
        if (s.keyName) s.getUsageLimits().then((limits) => { this._recordKeyUsage(s.keyName, limits); this._sendKeysUsage(); }).catch(() => {});
      }
    }
  ...
  _recordKeyUsage(keyName, limits) {
    ...
    this._keyUsage[keyName] = { limits: limits || null, available, exhaustedUntil: ex.exhausted ? ex.until : null, at: Date.now() };
  }
  ...
  _keysUsage() {
    ...
    return keys.map((k, i) => {
      ...
      return { name: k.name, fingerprint: keyFingerprint(k.token), active: i === activeIdx, exhausted,
        checked: !!(u && u.at), available: !!(u && u.available),
