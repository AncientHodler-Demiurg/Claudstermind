...
import { omniModelChoices } from "./omniRoute.mjs";
...
function capTranscript(transcript, full) {
  const arr = transcript || [];
  if (full || arr.length <= WS_RESYNC_MSG_CAP) return { transcript: arr, transcriptTruncated: false, transcriptTotal: arr.length, promptOffset: 0, responseOffset: 0 };
  const start = arr.length - WS_RESYNC_MSG_CAP;
  let promptOffset = 0, responseOffset = 0;
  for (let i = 0; i < start; i++) { const r = arr[i]; if (r && r.role === "user") promptOffset++; else if (r && r.role === "assistant") responseOffset++; }
  return { transcript: arr.slice(start), transcriptTruncated: true, transcriptTotal: arr.length, promptOffset, responseOffset };
}
...
export class WorkspaceManager {
  ...
  _control({ action, args = {} }) {
    ...
    switch (action) {
      ...
      case "open": return this._openTranscript(args.sessionKey, { scoped: !!args.scoped, full: !!args.full });
      case "resync": return this._resync(args.sessionKey, { scoped: !!args.scoped, full: !!args.full });
  ...
  _models(sessionKey) {
    // Append the OmniRoute picks (free/other models via the local gateway) to the live Claude catalog when a
    // gateway key is configured — empty otherwise, so this is a pure add-on that never disturbs Claude-only setups.
    const omni = omniModelChoices(process.env);
    const s = (sessionKey && this.sessions.get(sessionKey)) || [...this.sessions.values()][0] || null;
    if (!s) return this.send("state", sessionKey, { models: [...(this._modelsCache || []), ...omni] });
  ...
  _liveOrSavedState(sessionKey, { scoped = false, full = false } = {}) {
    ...
    if (live) {
      return { sessionId: live.sessionId, repo: live.repoLabel || null, worktree: live.worktree || "main",
        workspaceId: live.workspaceId || sessionKey, usage: live.usage, status: live.status, ...capTranscript(live.transcript, full) };
    }
    ...
    return { sessionId: row?.sessionId ?? bySession?.sessionId ?? null, repo: row?.repo ?? repo,
      worktree: row?.worktree ?? worktree, workspaceId, usage: bySession?.usage ?? null, status: "idle", ...capTranscript(transcript, full) };
  }
  ...
  _openTranscript(sessionKey, { scoped = false, full = false } = {}) {
    const s = this._liveOrSavedState(sessionKey, { scoped, full });
  ...
  _resync(sessionKey, { scoped = false, full = false } = {}) {
