...
export class WorkspaceManager {
  ...
  _autoReschedule(sessionKey) {
    const s = this.sessions.get(sessionKey);
    if (!s) return;
    s.auto = s.auto || this._autoFor(sessionKey);
    this._autoCancel(s);
  ...
  _control({ action, args = {} }) {
    ...
    switch (action) {
      case "autoContinue": {
        ...
        const s = this.sessions.get(args.sessionKey);
        if (!s) return;
        s.auto = this._autoFor(args.sessionKey);
        if ("on" in args) {
          const on = !!args.on;
          // Switching ON is what grants a fresh batch — the ceiling is per batch, and a human
          // flipping the switch is the human decision the ceiling waits for.
          if (on && !s.auto.on) s.auto.count = 0;
          s.auto.on = on;
        }
        if ("hold" in args) s.auto.hold = !!args.hold;
        ...
        if (args.grant) { s.auto.count = 0; s.auto.on = true; }
        return this._autoReschedule(args.sessionKey);
      }
