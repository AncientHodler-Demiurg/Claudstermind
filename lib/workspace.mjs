...
import { archiveSegment } from "./conversationArchive.mjs";
...
export class WorkspaceManager {
  ...
  _contextUsage(sessionKey) {
    ...
    s.getContextUsage().then((usage) => this.send("event", sessionKey, { kind: "contextUsage", usage, contextBreakdown: shapeContextUsage(usage) }));   // `usage` kept for the compact badge; `contextBreakdown` drives the Claude-GUI popover
  }
  ...
  _maybeRoll(s) {
    try {
      ...
      try { if (s.workspaceId) archiveSegment(join(this.transcriptDir, store.slugFor(s.workspaceId)), { conversationId: s.key, n, rows: head, summary, at: new Date().toISOString() }); } catch { /* archive is best-effort; the roll still proceeds */ }
