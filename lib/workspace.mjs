...
export class WorkspaceManager {
  ...
  _prompt(sessionKey, { repo, worktree, text, trusted, mode, model, effort, fastMode, resume, by: from, image, images }) {
    ...
    if (!s) {
      ...
      const rawResumeId = resume || this._latestWorkspaceRow(workspaceId)?.sessionId || null;
  ...
  _onEvent(sessionKey, ev) {
    ...
    if (s) {
      ...
      if (ev.kind === "result") { this._persist(s); ev = { ...ev, persisted: true }; }
    }
    ...
    // exact session open in a pane, so cross-client visibility needs this explicit push.
    if (ev.kind === "status" || ev.kind === "result") this._sendSessions();
  }
