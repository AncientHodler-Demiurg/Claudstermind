...
/* THE VERSION THIS ENGINE ACTUALLY LOADED. The web is served by the relay and the engine runs on the
 * work machine; they are deployed and restarted SEPARATELY, so the two can differ at any moment — and
 * when they do, the failure is silent and confusing rather than loud. It happened tonight: a browser
 * shipped with the auto-continue loop REMOVED (the server owns it now) met an engine that did not yet
 * have that loop, so the feature belonged to nobody and simply stopped, with nothing on screen saying
 * why. Stamping this on the state frame lets the client refuse to send and SAY SO instead of guessing.
 */
...
import { autoDecide, autoNextText, AUTO_CAP, AUTO_DELAY_MS } from "./autoContinue.mjs";
...
import { windowTail, windowAround, windowAroundTurn, countTurns } from "./conversationWindow.mjs";
...
// A well-formed `aroundTurn` arg: `{ kind: "prompt"|"response"|"user"|"assistant", number: N }`.
// Anything else is IGNORED (falls through to `around`/`limit`) rather than erroring — a windowing
// option is not worth failing an open over.
function validAroundTurn(t) {
  return t !== null && typeof t === "object" && Number.isFinite(Number(t.number));
}
function capTranscript(transcript, { full = false, limit, around, aroundTurn } = {}) {
  const arr = transcript || [];
  // `promptTotal`/`responseTotal` are how many P#/R# turns the WHOLE conversation holds. `transcriptTotal`
  // is a ROW count and rows include tool output, so it can never answer "turn 137 of 600" or validate a
  // typed turn number before firing a request — these can. Always present, on every window mode.
  const turns = countTurns(arr);
  const totals = { transcriptTotal: arr.length, promptTotal: turns.promptOffset, responseTotal: turns.responseOffset };
  // PRECEDENCE, fixed and echoed back as `windowMode` so the client never has to guess which of its
  // options was honoured: full > aroundTurn > around > limit (tail).
  if (full) return { transcript: arr, transcriptTruncated: false, ...totals, promptOffset: 0, responseOffset: 0, windowMode: "full" };
  // `aroundTurn` (a P#/R# TURN NUMBER) and `around` (a ROW INDEX) → a BAND centred there (jump-to-#N
  // without loading the whole history). Otherwise `limit` → the last N rows (the "Show earlier" growing
  // tail). All delegate to the shared, unit-tested conversationWindow module so Core, Pact (and later
  // DMP) window identically.
  //
  // `aroundTurn` exists because the turn-number → row-index mapping is only computable where the FULL
  // transcript is — here. A client can only map turns it has already loaded, and the whole point of
  // jump-to-#N is reaching one it hasn't, so leaving it client-side forced an interpolate-then-bisect
  // SEARCH (multiple round-trips). This collapses it to one exact hop.
  if (validAroundTurn(aroundTurn)) {
    const w = windowAroundTurn(arr, { kind: aroundTurn.kind, number: aroundTurn.number },
      { before: WS_RESYNC_MSG_CAP, after: WS_RESYNC_MSG_CAP });
    return { transcript: w.transcript, transcriptTruncated: w.truncatedBefore || w.truncatedAfter, ...totals,
      promptOffset: w.promptOffset, responseOffset: w.responseOffset, windowStart: w.start, windowEnd: w.end,
      windowMode: "aroundTurn", clamped: w.clamped,
      // Everything the client needs to say "showing #5, the last prompt — #500 doesn't exist (yet)"
      // instead of silently rendering the wrong band.
      turn: { kind: w.turn.kind, number: w.turn.number, resolved: w.turn.resolved, index: w.turn.index,
        found: w.turn.reason === "exact", reason: w.turn.reason, count: w.turn.count } };
  }
  if (typeof around === "number" && around >= 0) {
    const w = windowAround(arr, around, { before: WS_RESYNC_MSG_CAP, after: WS_RESYNC_MSG_CAP });
    return { transcript: w.transcript, transcriptTruncated: w.truncatedBefore || w.truncatedAfter, ...totals,
      promptOffset: w.promptOffset, responseOffset: w.responseOffset, windowStart: w.start, windowEnd: w.end,
      windowMode: "around", clamped: w.clamped };
  }
  const n = (typeof limit === "number" && limit > 0) ? Math.floor(limit) : WS_RESYNC_MSG_CAP;
  const w = windowTail(arr, n);
  // NOTE: `windowStart`/`windowEnd` are deliberately NOT emitted here. CONTRACT §4 documents their
  // PRESENCE as the "this is a band, not the tail" discriminator and the client helper keys off exactly
  // that, so adding them to a tail response would be a breaking change. `windowMode` is the new,
  // unambiguous discriminator; for the tail, `windowStart` is exactly `transcriptTotal - transcript.length`
  // (`scoped` picks the SOURCE array, it never filters rows out of a window — see CONTRACT §4).
  return { transcript: w.transcript, transcriptTruncated: w.truncatedBefore, ...totals,
    promptOffset: w.promptOffset, responseOffset: w.responseOffset, windowMode: "tail" };
}
...
export class WorkspaceManager {
  ...
  constructor(o = {}) {
    ...
    /* The auto-continue pause, injectable so the server-loop tests do not have to wait eight real
       seconds per round. Production passes nothing and gets AUTO_DELAY_MS. */
    this.autoDelayMs = Number.isFinite(o.autoDelayMs) ? o.autoDelayMs : undefined;
    /* AUTO-CONTINUE STATE, KEYED BY SESSION — not stored on the session object. A session is
       ephemeral: it ends, it is cleaned up, a new one is built for the next prompt. The round count
       is not ephemeral — it is the ceiling's memory, and the whole point of a ceiling is that it
       cannot be forgotten by walking away. Hanging it on `s` meant every teardown silently granted a
       fresh batch, and reported as "the number of automatic rounds is still not kept". */
    this.autoState = new Map();
    ...
    // Where Claude Code keeps its OWN session logs — the files `--resume` reads. Injectable so a test
    // can point it at a store it controls (and so a machine that keeps them elsewhere can be told,
    // rather than guessed at). See _sdkStoreVisible for why "not found here" is never assumed to
    // mean "does not exist".
    this.sdkProjectsDir = o.sdkProjectsDir || join(process.env.HOME || "", ".claude", "projects");
  ...
  sessionSummary(s) {
    const rawBg = s.backgroundTasks ? [...s.backgroundTasks.values()] : [];
    const panel = typeof s.backgroundPanel === "function" ? s.backgroundPanel() : null;
    // `background` is the RAW SDK live set and carries no lifecycle status of its own, so a client
    // rendering that path was showing an ASSUMPTION ("running") rather than data. Stamp each row with
    // the status the reducer actually tracks (it folds taskStarted/taskDone too), matched by id.
    // Purely additive — the array, its order and every existing field are untouched.
    const statusById = new Map((panel?.agents || []).map((a) => [a.id, a.status]));
    const bg = rawBg.map((t) => (t && t.id != null && statusById.has(t.id)
      ? { ...t, status: t.status ?? statusById.get(t.id) }
      // In the authoritative live set but not in the shaped panel (a stub engine with no
      // backgroundPanel(), or an id the reducer never saw): being in the live set IS the evidence
      // that it is running, so that is the honest default rather than a guess.
      : (t && typeof t === "object" ? { ...t, status: t.status ?? "running" } : t)));
    /* THE LOOP'S OWN STATE, so every device shows the same truth about a sweep that is running
       without any of them. `at` is when the next prompt is due — the browser renders its countdown
       from this rather than owning a clock of its own. */
    const auto = s.auto || this.autoState.get(s.key) || null;
    return { sessionKey: s.key, cwd: s.cwd, repo: s.repoLabel || null, status: s.status,
      auto: auto ? { on: !!auto.on, count: auto.count | 0, cap: auto.cap | 0,
                     at: auto.at || 0, text: auto.text || "", reason: auto.reason || "" } : null,
  ...
  _prompt(sessionKey, { repo, worktree, text, trusted, mode, model, effort, fastMode, resume, fresh, scoped, by: from, image, images }) {
    ...
    /* A HUMAN SEND RESETS THE CEILING — it exists to stop a runaway ROBOT, not to ration you — and
       it cancels any pending round, because whatever the loop was about to say is superseded by what
       you actually said. `by: "auto"` is the loop's own send and does neither.
       THIS MUST SIT ABOVE THE SESSION BRANCHES. Placed inside the new-session branch it ran only for
       a conversation that did not exist yet, which is the one case where there is no ceiling to
       reset — so it silently never fired for any real send. */
    if (from !== "auto") {
      const sNow = this.sessions.get(sessionKey);
      if (sNow && sNow.auto) { sNow.auto.count = 0; this._autoCancel(sNow); }
    }
    ...
    if (!s) {
      ...
      let resumeId = rawResumeId && rawResumeId !== workspaceId && rawResumeId !== sessionKey ? rawResumeId : null;
      // …and one more, which the checks above cannot catch: an id that is perfectly well-formed and
      // simply is not there any more. The SDK's own session logs live outside this app and can be
      // pruned, moved, or lost with a project dir; `--resume` on a missing one exits immediately with
      // "No conversation found with session ID: …" and produces no turn at all. That is a DEAD
      // conversation — every prompt fails, forever, and (before the `turnError` row above) silently.
      //
      // The durable history is OURS, in the transcript; the SDK file is only the model's warm window.
      // So a missing one is not fatal: drop it, start a fresh window, and SAY so, rather than
      // resuming into nothing. `_sdkSessionFile` is the same lookup the cold-load cue below already
      // does — it was being asked "how big is it?" a few lines later without ever asking "is it
      // there?".
      let _resumeLost = null;
      if (resumeId && this._sdkStoreVisible() && !this._sdkSessionFile(resumeId)) { _resumeLost = resumeId; resumeId = null; }
      s = new ClaudeSession({
      ...
      if (_resumeLost) {
        const at = Date.now();
        const msg = "The previous window for this conversation is no longer on disk (session "
          + String(_resumeLost).slice(0, 8) + "…), so this turn starts a fresh one. Your transcript above is intact; "
          + "the agent just does not carry the earlier window's own context into this turn.";
        s.transcript?.push({ kind: "turnError", message: msg, subtype: "resume-missing", at });
        this.send("event", sessionKey, { kind: "turnError", message: msg, subtype: "resume-missing", at });
      }
  ...
  /** Did the CURRENT turn ANSWER — produce text the reader actually got? Cleared when the turn ends,
   *  so the verdict is always about this turn and never inherited from the last one.
   *
   *  This is what separates "the turn failed" from "the SDK had a remark about a turn that worked",
   *  and it used to count `tool_use` and `tool_result` as well — despite asking, in this very
   *  comment, whether the turn produced anything THE USER CAN SEE. Tool calls are the engine talking
   *  to itself. So a turn that ran four tools, never answered, and then ended with `is_error`
   *  counted as a working turn with a remark: a quiet beige ENGINE NOTE under a prompt that never
   *  got a reply. That is exactly the "the prompts are stuck" report this classification exists for,
   *  wearing the disguise meant for successful turns.
   *
   *  Whitespace is not an answer either — an `assistant` frame carrying only tool-use blocks arrives
   *  with empty text, which is the same claim by another route. */
  _noteTurnOutput(s, ev) {
    if (!s || !ev) return;
    if ((ev.kind === "assistant" || ev.kind === "assistant_delta") && String(ev.text || "").trim()) s._turnReply = true;
  }

  _onEvent(sessionKey, ev) {
    const s = this.sessions.get(sessionKey);
    this._noteTurnOutput(s, ev);
    ...
    //
    // The `status` filter exists because our own synthetic "thinking" status is emitted BEFORE any SDK output,
    // so it must not be mistaken for the load finishing. A TERMINAL outcome is different: if the cold load
    // FAILS, the session may end without ever producing output, and `{ kind: "status", status: "ended" }` is
    // then the only event that will ever arrive — skipping it left the "Loading history…" cue stuck ON forever,
    // the exact stuck-cue class the lookingUp/recall pair was careful about. Flush on a terminal event too,
    // with `ok: false` so the client renders "load failed", not "✓ loaded".
    if (s && s._coldLoadAt && ev) {
      const terminal = ev.kind === "error" || (ev.kind === "status" && (ev.status === "ended" || ev.status === "error"));
      if (terminal || ev.kind !== "status") {
        this.send("event", sessionKey, { kind: "loadingHistoryDone", ms: Date.now() - s._coldLoadAt, bytes: s._coldLoadBytes || 0, ok: !terminal });
        s._coldLoadAt = null;
      }
    }
    ...
    if (s) {
      ...
      // COMPACTION IS A TRANSCRIPT EVENT, not just a notification. Two real bugs came from it being
      // only the latter:
      //  1. `/compact` produces no assistant reply, so the prompt that ran it stayed the trailing
      //     UNANSWERED user turn — which is exactly the shape both clients use to recognise an
      //     INTERRUPTED prompt. A successful compaction therefore painted itself dark blue with
      //     ▶ resume / ✕ discard buttons ("the compacting prompt looked like it didn't process").
      //  2. There was no mark in the conversation saying where the window was summarised — only a
      //     line in the activity rail, which is transient, per-client, and gone on reload ("the
      //     compaction bar didn't come, like it was supposed to").
      // Recording it as a row fixes both at once, durably: it persists with the turn, so it survives
      // a reload and shows on every device, and it terminates the trailing-unanswered scan.
      // Stamped with the same `at` on the row AND the forwarded event (see the assistant case above)
      // so a client that renders it live and a client that renders it after a resync agree.
      if (ev.kind === "compacted") {
        const at = Date.now();
        s.transcript?.push({ kind: "compacted", trigger: ev.trigger || "manual",
                             preTokens: ev.preTokens ?? null, postTokens: ev.postTokens ?? null, at });
        ev = { ...ev, at };
      }
      ...
      if (ev.kind === "result") {
        // A FAILED TURN MUST SAY SO. The SDK ends a broken turn with an ordinary `result` frame
        // carrying `is_error` — no assistant text, no exception, nothing on the stream that looks
        // wrong. Everything downstream treated it as a completed turn, so the pane went quiet and
        // idle and the prompt simply appeared "stuck", with no reply and no explanation, forever.
        // Observed on AncientPantheon/constructors/Khronoton: two prompts in a row died this way
        // ("No conversation found with session ID: …") and the transcript recorded zero errors.
        //
        // Recorded as a durable transcript ROW for the same reason compaction and wrapping are: it
        // is something the reader has to know happened, and a cue that fades is no use to someone
        // coming back to a pane that looks merely empty.
        //
        // A FAILED TURN IS ONE THAT PRODUCED NOTHING — not merely one whose result carries
        // `is_error`. The SDK also flags results that are DIAGNOSTICS about a turn that worked fine:
        // observed live, `[ede_diagnostic] result_type=user last_content_type=n/a
        // stop_reason=tool_use`, on a turn that had just streamed 21,039 output tokens and finished.
        // It arrives with the SAME `subtype: "error_during_execution"` as a genuinely dead turn, so
        // the subtype cannot tell them apart — but what the turn DID can, and that is also the only
        // definition that matches the report this exists for ("the prompts are stuck": no reply at
        // all, ever). Shouting TURN FAILED over a turn that answered is the same class of lie as
        // staying silent over one that did not.
        /* Output TOKENS were once accepted as proof too, and they are the same loophole by another
           name: tool calls burn them. A count says work happened, not that the reader got anything. */
        const gaveAnswer = !!s._turnReply;
        if (ev.isError) {
          const why = (Array.isArray(ev.errors) && ev.errors.join("; ")) || ev.subtype || "the turn ended with an error";
          const at = Date.now();
          if (gaveAnswer) {
            // Still worth surfacing — it just belongs in the activity log, not as a permanent red
            // bar across a conversation that carried on perfectly well.
            this.send("event", sessionKey, { kind: "warning", message: "Engine diagnostic: " + why });
          } else {
            s.transcript?.push({ kind: "turnError", message: why, subtype: ev.subtype || null, at });
            this.send("event", sessionKey, { kind: "turnError", message: why, subtype: ev.subtype || null, at });
          }
        }
        s._turnReply = false;   // this turn is over either way — the next one is judged on its own
    ...
    /* A TURN ENDING IS THE LOOP'S HEARTBEAT. Re-evaluated on `status` too, not only on `result`:
       a turn can end by going idle without a terminal result (a stop, a failed turn), and a loop that
       only listened for `result` would sit disarmed for good after one of those. */
    if (ev.kind === "status" || ev.kind === "result") this._autoReschedule(sessionKey);
  ...
  _sendSessions() {
    // `engineVersion` rides every session snapshot: it is the one frame every client is guaranteed to
    // receive, so no extra round-trip is needed to learn what the engine is running.
    this.send("state", null, { engineVersion: ENGINE_VERSION,
                               sessions: [...this.sessions.values()].map((x) => this.sessionSummary(x)) });
  }
  ...
  /* ===== AUTO-CONTINUE — THE SERVER-SIDE LOOP =========================================== *
   * It used to run in the browser, which meant it ran only while a browser was awake and looking at
   * it: a locked phone froze the timers and the sweep stopped until you came back, when it fired at
   * once. Here it runs where the session lives, so it keeps going with every client closed.
   *
   * That is also what makes it dangerous, and the shape below is the answer to that:
   *   - ONE timer per session, always cleared before another is set. Two timers is two prompts, and
   *     this codebase has already shipped that bug twice from the client side.
   *   - The ceiling is enforced HERE, on the count this object owns. A client cannot raise it by
   *     sending a number; it can only ask for the loop to be on.
   *   - Every path that means "a human took over" — a typed prompt, Stop, closing the session —
   *     cancels the schedule, and Stop switches the loop off outright rather than merely pausing it.
   *   - Every decision comes from lib/autoContinue.mjs, which is pure and tested; nothing here
   *     decides anything, it only schedules what that function already decided.
   */
  /** The durable auto-continue record for a conversation, created on first use. Sessions come and go
   *  around it; this is what makes the count survive a teardown. */
  _autoFor(sessionKey) {
    let a = this.autoState.get(sessionKey);
    if (!a) { a = { on: false, hold: false, count: 0, cap: AUTO_CAP, at: 0, text: "", reason: "" };
              this.autoState.set(sessionKey, a); }
    return a;
  }
  _autoCancel(s) {
    if (s && s._autoTimer) { clearTimeout(s._autoTimer); s._autoTimer = null; }
    if (s && s.auto) s.auto.at = 0;
  }
  /** Re-evaluate a session's loop. Idempotent: safe to call on every event, which is exactly how a
   *  missed re-arm stops being possible. */
  _autoReschedule(sessionKey) {
    const s = this.sessions.get(sessionKey);
    if (!s) return;
    /* USE the record; never CREATE one here. `_autoFor` materialises `{ on: false, … }`, and this runs
       on every status and result — so creating one would publish "auto-continue: off" for every
       session that has never used it. A client adopts what the engine reports, so that would reach in
       and switch the toggle OFF, and (since the browser now writes down what it adopts) make it
       stick. Absence must stay absence: it is how the engine says "I have no opinion". */
    const rec = this.autoState.get(sessionKey);
    if (!rec) return;
    s.auto = rec;
    this._autoCancel(s);
    const busy = s.status === "thinking" || s.status === "deepwork" || s.status === "awaiting-permission";
    const d = autoDecide({ on: s.auto.on, hold: s.auto.hold, busy, transcript: s.transcript || [],
                           count: s.auto.count, cap: s.auto.cap, at: 0, now: Date.now(),
                           delayMs: this.autoDelayMs ?? AUTO_DELAY_MS });
    s.auto.reason = d.reason;
    if (!d.arm) { s.auto.at = 0; this._sendSessions(); return; }
    s.auto.at = d.at;
    // The text is resolved AT SEND time, not now: a turn may still be flushing rows, and continuing
    // from a suggestion that has since been superseded repeats work.
    s._autoTimer = setTimeout(() => this._autoFire(sessionKey), Math.max(0, d.at - Date.now()));
    if (s._autoTimer.unref) s._autoTimer.unref();   // never hold the process open for a countdown
    this._sendSessions();
  }
  /** The moment of truth. Re-checks everything — the world may have moved during the wait. */
  _autoFire(sessionKey) {
    const s = this.sessions.get(sessionKey);
    if (!s || !s.auto) return;
    s._autoTimer = null;
    const busy = s.status === "thinking" || s.status === "deepwork" || s.status === "awaiting-permission";
    const d = autoDecide({ on: s.auto.on, hold: s.auto.hold, busy, transcript: s.transcript || [],
                           count: s.auto.count, cap: s.auto.cap, at: s.auto.at, now: Date.now(),
                           delayMs: this.autoDelayMs ?? AUTO_DELAY_MS });
    if (!d.fire) { s.auto.reason = d.reason; return this._autoReschedule(sessionKey); }
    // COUNT BEFORE SENDING. If the send throws, the round is still spent — an error that left the
    // count untouched would retry forever, which is the one failure mode a ceiling exists to prevent.
    s.auto.count += 1;
    s.auto.at = 0;
    const text = autoNextText(s.transcript || []);
    s.auto.text = text;
    this._prompt(sessionKey, { text, by: "auto" });
    this._sendSessions();
  }
  ...
  async _stop(sessionKey) {
    ...
    /* STOP SWITCHES THE LOOP OFF, it does not pause it. Someone reached for the brake on a sweep
       running by itself; re-arming eight seconds later and carrying on would be the single most
       alarming thing this feature could do. Turning it back on is one tap and grants a fresh batch. */
    {
      const s0 = this.sessions.get(sessionKey);
      if (s0 && s0.auto) { s0.auto.on = false; s0.auto.reason = "stopped"; this._autoCancel(s0); }
    }
    this._resolvePendingFor(sessionKey, "deny");
  ...
  _control({ action, args = {} }) {
    ...
    switch (action) {
      case "autoContinue": {
        /* The client may turn the loop ON or OFF and may say "someone is typing here" (`hold`). It
           may NOT set the count or the ceiling: those are the server's, and a loop whose limit is
           whatever the last client claimed has no limit at all. */
        const key = args.sessionKey;
        if (!key) return;
        /* THE RECORD IS PER CONVERSATION, not per session — so this works before the first prompt and
           after a session has been torn down. Without that, a client re-asserting its restored state
           into a freshly-restarted engine was silently dropped ("if (!s) return"), and the toggle sat
           there reading ON with no loop behind it. */
        const a = this._autoFor(key);
        if ("on" in args) {
          const on = !!args.on;
          /* Switching ON grants a fresh batch — the ceiling is per batch and a human flipping the
             switch IS the decision it waits for. `assert` is a client saying "this is what I already
             had", which must NOT be read as that decision, or every reconnect would silently hand out
             another ten rounds. */
          if (on && !a.on && !args.assert) a.count = 0;
          a.on = on;
        }
        if ("hold" in args) a.hold = !!args.hold;
        /* ▷ CONTINUE — the ceiling waits for a human, and this is that human saying so. It grants the
           NEXT batch by clearing the count; it cannot raise the cap, so every grant is the same size
           and an unattended loop can still never run further than one batch per deliberate act. */
        if (args.grant) { a.count = 0; a.on = true; }
        const s = this.sessions.get(key);
        if (s) s.auto = a;
        return this._autoReschedule(key);
      }
      ...
      case "open": return this._openTranscript(args.sessionKey, { scoped: !!args.scoped, full: !!args.full, limit: args.limit, around: args.around, aroundTurn: args.aroundTurn, have: args.have });
      // `have` rides along: the client's "this is what I already hold", which lets _resync answer a
      // watchdog poll without re-sending the transcript. This dispatcher hand-picks fields rather
      // than spreading `args`, so a new one is invisible until it is named here — which is exactly
      // how the first cut of this shipped a server that accepted `have` and never received one.
      case "resync": return this._resync(args.sessionKey, { scoped: !!args.scoped, full: !!args.full, limit: args.limit, around: args.around, aroundTurn: args.aroundTurn, have: args.have });
      ...
      case "setAutoWrap": return this.sessions.get(args.sessionKey)?.setAutoWrap(args.enabled !== false);
      ...
      case "wrapPreview": return this._sendWrapPreview(args.sessionKey);
      case "wrapNow": return this._sendWrapNow(args.sessionKey);
      default: return;
    }
  }
  ...
  _contextUsage(sessionKey) {
    ...
    // `live` is the fact the client could not otherwise know, and NOT knowing it is why three separate
    // readouts ("model not reported yet", "context: no reading yet", a model picker naming only
    // aliases) all read as malfunctions when they were describing one ordinary state. A conversation
    // that is OPEN but has never been prompted has no `claude` process at all — `s.start()` is only
    // ever called from _prompt — so there is genuinely no model, no context reading and no model
    // catalogue to report. That is very different from "a process is running and has not answered
    // yet", and the UI has to be able to tell them apart to say either one honestly.
    if (!s) return this.send("event", sessionKey, { kind: "contextUsage", live: false, usage: null, contextBreakdown: shapeContextUsage(null) });
    s.getContextUsage().then((usage) => this.send("event", sessionKey, { kind: "contextUsage", live: true, usage, contextBreakdown: shapeContextUsage(usage) }));
  }

  /** Manual wrap, part 1: send back what committing would archive, without touching anything. */
  _sendWrapPreview(sessionKey) {
    this.send("event", sessionKey, { kind: "wrapPreview", preview: this.wrapPreview(sessionKey) });
  }

  /** Manual wrap, part 2: actually do it. `ok:false` (e.g. "too-light") means the client offered
   *  Wrap on a conversation with nothing to roll yet — reported back rather than silently ignored,
   *  so the confirm dialog can say why instead of just closing as if it worked. */
  _sendWrapNow(sessionKey) {
    const result = this.manualRoll(sessionKey);
    this.send("event", sessionKey, { kind: "wrapResult", ok: result.ok, reason: result.reason || null });
  }
  ...
  _worktreeAdd(repo, name) {
    ...
    if (!r.ok) {
      /* SAY WHY, THEN PUBLISH THE TRUTH. The commonest refusal is "a worktree named X already
         exists — pick it from the list instead of creating it", and this used to `return` before
         the list was sent. So the message pointed at a dropdown that did not contain X, which is
         precisely the dead end that was reported: the worktree was on disk, absent from the list,
         and the only route to it refused with an instruction that could not be followed. */
      this.send("event", null, { kind: "error", message: r.error || "Could not create the worktree." });
  ...
  _liveOrSavedState(sessionKey, { scoped = false, full = false, limit, around, aroundTurn } = {}) {
    ...
    if (live) {
      return { sessionId: live.sessionId, repo: live.repoLabel || null, worktree: live.worktree || "main",
        ...
        ...capTranscript(live.transcript, { full, limit, around, aroundTurn }) };
    }
    ...
    if (!workspaceId) {
      /* A CONVERSATION THAT DOES NOT EXIST YET IS NOT A CONVERSATION THAT FAILED. A fresh multi-chat
         slot — `repo@main#2`, created by ticking multi-chat and adding a chat — has no live session,
         no saved workspace row and no saved session, so this returned null and the caller answered
         **"That conversation could not be opened."** in red, over an empty transcript, for every new
         chat before its first prompt. Opening a new conversation is the most ordinary thing there is.
         An id that PARSES as a workspace id (`repo@worktree`, optionally `#slot`) is a legitimate
         empty conversation; anything else — a stale uuid from a dead link, say — really is unknown,
         and still says so. */
      if (!String(sessionKey || "").includes("@")) return null;
      const fresh = store.parseWorkspaceId(sessionKey);
      return { sessionId: null, repo: fresh.repo, worktree: fresh.worktree, workspaceId: sessionKey,
               usage: null, status: "idle", ...capTranscript([], { full, limit, around, aroundTurn }) };
    }
    ...
    return { sessionId: row?.sessionId ?? bySession?.sessionId ?? null, repo: row?.repo ?? repo,
      worktree: row?.worktree ?? worktree, workspaceId, usage: bySession?.usage ?? null, status: "idle", ...capTranscript(transcript, { full, limit, around, aroundTurn }) };
  }
  ...
  _openTranscript(sessionKey, { scoped = false, full = false, limit, around, aroundTurn, have } = {}) {
    const s = this._liveOrSavedState(sessionKey, { scoped, full, limit, around, aroundTurn });
    ...
    /* NOTHING TO SEND IS WORTH SAYING CHEAPLY. Reattaching shipped the whole capped window every time —
       measured at 158 KB per 250 rows — even when the client already held exactly those rows. `_resync`
       has had this short-circuit for a while; `open`, which is the path a RETURNING client takes, did
       not, so coming back to a workspace re-downloaded the conversation over the tunnel on every
       visit. A transcript is append-only, so the row count plus the last row's `at` proves it.
       Answered rather than dropped: the client correlates opens by key, and silence would strand the
       pending entry (and its loader) forever. */
    const tx = s.transcript;
    if (Array.isArray(tx) && have && typeof have === "object" && Number(have.n) === tx.length
        && around == null && aroundTurn == null) {
      const last = tx[tx.length - 1];
      const lastAt = last && typeof last === "object" ? last.at : undefined;
      if (tx.length === 0 || (lastAt != null && Number(have.at) === Number(lastAt))) {
        const live0 = this.sessions.get(sessionKey);
        this.send("transcript", sessionKey, { sessionKey, unchanged: true });
        if (live0) this.send("state", sessionKey, { session: this.sessionSummary(live0) });
        this._resendPendingPerms(sessionKey);
        return;
      }
    }
    this.send("transcript", sessionKey, { sessionKey, ...s });
    /* AND THE SESSION'S OWN STATE, when it is live. Reattaching used to return the transcript and
       nothing else, so a client that had just rebuilt its view fell back to whatever it had persisted
       locally — including a stale auto-continue round count. That is the "the auto count resets if I
       go away from pact space and then come back" report: the engine's count was right the whole
       time, and nobody sent it. `auto` rides sessionSummary, so this is the one frame that corrects
       it, at exactly the moment a client reattaches. */
    const live = this.sessions.get(sessionKey);
    if (live) this.send("state", sessionKey, { session: this.sessionSummary(live) });
  ...
  /** `have` is the client saying what it already holds: { n, at } — the number of transcript rows and
   *  the `at` of its last one. A transcript is append-only, so a matching tail PROVES the client is
   *  up to date, and shipping it its own rows back is pure waste.
   *
   *  MEASURED, and this is why it matters: one resync of a 250-row window is **158 KB**. The client's
   *  self-heal watchdog fires one per pane every 8 seconds, so an eight-pane cockpit moved ~1.26 MB
   *  of JSON every 8 seconds — serialized here (once per connected terminal) and parsed there, on one
   *  thread each. That is enough to delay the echo of a freshly-typed prompt by seconds and to make
   *  typing stutter, for a watchdog whose entire job is to answer "did I miss anything?".
   *
   *  It still answers that question, and still carries status/usage/mode — everything a dropped event
   *  could have desynced — just without re-sending rows the client demonstrably already has. */
  _resync(sessionKey, { scoped = false, full = false, limit, around, aroundTurn, have } = {}) {
    if (!sessionKey) return;
    const s = this._liveOrSavedState(sessionKey, { scoped, full, limit, around, aroundTurn });
    if (!s) return;   // nothing known about this key on this process — leave the pane as-is
    const tx = Array.isArray(s.transcript) ? s.transcript : null;
    if (tx && have && typeof have === "object" && Number(have.n) === tx.length) {
      const last = tx[tx.length - 1];
      const lastAt = last && typeof last === "object" ? last.at : undefined;
      // Same count AND same final row ⇒ nothing was missed. Send the state, drop the rows: the
      // client's handler only touches its transcript when the field is present.
      if (tx.length === 0 || (lastAt != null && Number(have.at) === Number(lastAt))) delete s.transcript;
    }
  ...
  /** Where the SDK's OWN session log for a resume id actually is — "" when there is none anywhere.
   *  Globbed by filename across the project dirs so we never reverse-engineer Claude Code's
   *  cwd→dir encoding. This is the question `--resume` silently answers with an immediate exit. */
  _sdkSessionFile(resumeId) {
    if (!resumeId) return "";
    try {
      for (const d of readdirSync(this.sdkProjectsDir)) {
        const f = join(this.sdkProjectsDir, d, resumeId + ".jsonl");
        if (existsSync(f)) return f;
      }
    } catch { /* no projects dir — see _sdkStoreVisible */ }
    return "";
  }

  /** Can we actually SEE the SDK's session store? Everything that reasons about a resume id being
   *  "missing" has to ask this first, because the alternative reading of an empty answer is "we are
   *  looking in the wrong place" — and acting on that would drop every VALID resume on a machine
   *  whose sessions live somewhere else. A directory that exists and holds at least one project dir
   *  is the evidence; anything less and we say nothing and pass the id through unchanged. */
  _sdkStoreVisible() {
    try { return readdirSync(this.sdkProjectsDir).length > 0; } catch { return false; }
  }
  _sdkSessionBytes(resumeId) {
    const f = this._sdkSessionFile(resumeId);
    if (!f) return 0;
    try { return statSync(f).size; } catch { return 0; }
  }

  /** Where the CURRENT (un-archived) window starts in `s.transcript` — DERIVED, never remembered.
   *
   *  This used to be an in-memory counter (`s._rolledThrough`) that existed only inside the running
   *  process. A sessiond restart rebuilt the session with the full transcript reloaded from disk and
   *  the counter back at 0, so the "turns since the last wrap" slice was the ENTIRE conversation —
   *  which for any long-lived workspace is already far past `maxTurns`. The very next completed turn
   *  therefore auto-wrapped, every single restart, no matter how recently the last wrap had happened.
   *
   *  Measured on OuroborosNetwork/daimons/OuronetUI@main: three segments whose archives ALL begin at
   *  the conversation's first prompt (each one re-archiving everything its predecessor had), the last
   *  two only 57 turns apart against a 1000-turn threshold. It also inflates the absolute P#/R# range
   *  index those archives carry, which is what `recallByNumber` resolves against.
   *
   *  The transcript already carries a durable mark for every wrap — `{kind:"wrapped"}`, pushed by
   *  _performRoll and persisted with every other row — so the boundary is READ BACK from the
   *  transcript instead of remembered. A restart recovers the true window, and "where does this
   *  window start" has exactly one source of truth: the same marks the reader can see in the
   *  transcript. Pure and side-effect free, so wrapPreview can call it while a dialog sits open.
   *
   *  A conversation wrapped before wrap marks existed has no marker to find and will wrap once more
   *  than it strictly needs to; it is self-correcting, since that wrap leaves a mark behind. */
  _rollFrom(s) {
    const t = s && Array.isArray(s.transcript) ? s.transcript : [];
    for (let i = t.length - 1; i >= 0; i--) if (t[i] && t[i].kind === "wrapped") return i + 1;
    return 0;
  }
  ...
  _maybeRoll(s) {
    try {
      ...
      if (s.autoWrap === false) return;   // web-side auto-wrap checkbox opted this session out
      const from = this._rollFrom(s);
      const slice = s.transcript.slice(from);
      if (!shouldRoll(conversationStats(slice))) return;
      this._performRoll(s, slice);
    } catch { /* never let a roll failure sink the turn */ }
  }

  /** Does the actual roll: archive the head, seed the fresh session, respawn. Called by _maybeRoll
   *  once the automatic threshold fires, and by manualRoll() for a user-triggered wrap that bypasses
   *  that threshold entirely — ONE implementation either way, so a manual wrap can never drift from
   *  what the automatic one does. Returns true if a roll actually happened, false if there was
   *  nothing to roll (the conversation is still shorter than the tail budget). */
  _performRoll(s, slice) {
    const { head, tail } = splitForRoll(slice, ROLL_DEFAULTS.tailTurns);
    if (!head.length) return false;                                // nothing to summarize yet — wait
    // NOTE: the "don't re-roll until another window accrues" boundary is the `wrapped` row pushed
    // below, read back by _rollFrom — NOT a counter held here. See _rollFrom for why.
    ...
    // Record the WRAP in the conversation itself, for the same reason compaction is recorded (see
    // _onEvent): a boundary the reader has to know about — everything above it left the live window —
    // must be a durable mark in the transcript, not a cue that fades. The transcript is never spliced
    // by a roll (the wrap MARK below is the boundary - see _rollFrom), so this row keeps its place.
    const rollAt = Date.now();
    s.transcript?.push({ kind: "wrapped", segment: n, sourceRef, at: rollAt });
    this.send("event", s.key, { kind: "rolling", segment: n, sourceRef, at: rollAt });   // the "⟳ Rolling to a fresh window…" cue
    s.roll(seed);                                                  // fresh session, seeded — see ClaudeSession.roll
    return true;
  }

  /** MANUAL WRAP — a user-triggered roll that bypasses the automatic turns/bytes/context threshold
   *  entirely. Eligibility ("is this conversation heavy enough to be worth it") is a UI-level gate —
   *  ChatShell.wrapReadiness's manualAt (60% of context) — because that needs live context-token data
   *  only the client already has; mirrors how Compact has no server-side gate either, only the
   *  client's paneBusy/readonly checks. Server-side, the only real guard is _performRoll's own
   *  "nothing to roll yet" check, which fires if the client offers Wrap on a too-light conversation
   *  anyway — defense in depth, not the primary gate. */
  manualRoll(sessionKey) {
    const s = this.sessions.get(sessionKey);
    if (!s || !Array.isArray(s.transcript) || typeof s.roll !== "function") return { ok: false, reason: "no-session" };
    const from = this._rollFrom(s);
    const slice = s.transcript.slice(from);
    try {
      const rolled = this._performRoll(s, slice);
      return { ok: rolled, reason: rolled ? null : "too-light" };
    } catch { return { ok: false, reason: "error" }; }
  }

  /** Read-only preview of what a wrap would archive RIGHT NOW — no mutation at all, safe to call
   *  repeatedly while a confirmation dialog sits open. Uses the SAME head/tail split _performRoll
   *  uses, so the numbers shown are exactly what committing would actually do — never a separate,
   *  driftable estimate. */
  wrapPreview(sessionKey) {
    const s = this.sessions.get(sessionKey);
    if (!s || !Array.isArray(s.transcript)) return null;
    const from = this._rollFrom(s);
    const slice = s.transcript.slice(from);
    const { head, tail } = splitForRoll(slice, ROLL_DEFAULTS.tailTurns);
    if (!head.length) return { empty: true, keptTurns: tail.length };
    let pChars = 0, rChars = 0, pCount = 0, rCount = 0;
    for (const row of head) {
      if (!row) continue;
      const text = typeof row.text === "string" ? row.text : "";
      if (row.role === "user") { pChars += text.length; pCount++; }
      else if (row.role === "assistant") { rChars += text.length; rCount++; }
    }
    const pOff = s._archivedPrompts || 0, rOff = s._archivedResponses || 0;
    return {
      empty: false, keptTurns: tail.length,
      p: { from: pOff + 1, to: pOff + pCount, count: pCount, chars: pChars },
      r: { from: rOff + 1, to: rOff + rCount, count: rCount, chars: rChars },
    };
  }
   ...
   *
   *  EVERY terminal `recall` event carries a MACHINE-READABLE `reason` alongside the human `error`
   *  string: null on a hit, else one of "not-found" | "no-archive" | "refused" | "internal-error".
   *  Without it the only way to tell a miss apart from a broken archive was to string-match the four
   *  English messages — so rewording one silently degraded a client into "internal-error". The `error`
   *  string stays and is still what a UI should render verbatim (it is what distinguishes "not
   *  archived" from "still in the active window"); it is just no longer load-bearing for CLASSIFYING. */
  _recall({ sessionKey, kind, number, query, limit } = {}) {
    ...
    // Refused outright (no number, no query): no `lookingUp` was sent, so there is no cue to clear —
    // but this still carries `at` like EVERY other recall path, so a client can timestamp/order it
    // uniformly instead of special-casing the one event that lacked a clock.
    if (!byNumber && !q) return this.send("event", sessionKey, { kind: "recall", mode: "query", query: "", ok: false, hits: [], reason: "refused", error: "Nothing to recall — give a turn number or a search query.", at: Date.now() });
    ...
    try {
      ...
      if (!base) out = byNumber
        ? { kind: "recall", mode, kindOf: wantKind, number: num, ok: false, hit: null, reason: "no-archive", error: "No archive for this conversation yet." }
        : { kind: "recall", mode, query: q, ok: false, hits: [], reason: "no-archive", error: "No archive for this conversation yet." };
      else if (byNumber) {
        ...
        out = { kind: "recall", mode, query: q, ok: hits.length > 0, hits,
          reason: hits.length ? null : "not-found",
          error: hits.length ? null : "Nothing archived matches that." };
      }
    } catch (e) {
      out = byNumber
        ? { kind: "recall", mode, kindOf: wantKind, number: num, ok: false, hit: null, reason: "internal-error", error: String(e && e.message || e) }
        : { kind: "recall", mode, query: q, ok: false, hits: [], reason: "internal-error", error: String(e && e.message || e) };
    }
