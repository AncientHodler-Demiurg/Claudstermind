...
export class WorkspaceManager {
  ...
  /** Did the CURRENT turn produce anything the user can see? Set by the events that carry content,
   *  cleared when the turn ends. This is what separates "the turn failed" from "the SDK had a
   *  remark about a turn that worked". */
  _noteTurnOutput(s, ev) {
    if (!s || !ev) return;
    if (ev.kind === "assistant" || ev.kind === "assistant_delta" || ev.kind === "tool_use" || ev.kind === "tool_result") s._turnOutput = true;
  }
  ...
  _onEvent(sessionKey, ev) {
    ...
    if (s) {
      ...
      if (ev.kind === "result") {
        ...
        const madeOutput = !!s._turnOutput || (ev.usage && Number(ev.usage.output_tokens) > 0);
        if (ev.isError) {
        ...
        s._turnOutput = false;   // this turn is over either way
