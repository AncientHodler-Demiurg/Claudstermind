...
import { ClaudeSession, emptyUsage } from "./claudeSession.mjs";
...
/** Walk the workspace into a nested folder tree, flagging folders that ARE git repos.
 *  Bounded depth + a skip-list keep it cheap and cross-platform (fs/path only). */
export function walkTree(dir, name, depth = 0, maxDepth = 4) {
  const node = { name, isRepo: existsSync(join(dir, ".git")), children: [] };
...
export class WorkspaceManager {
  ...
  sessionSummary(s) {
    return { sessionKey: s.key, cwd: s.cwd, repo: s.repoLabel || null, status: s.status,
      sessionId: s.sessionId, trusted: s.trusted, usage: s.usage };
  }
  ...
  _prompt(sessionKey, { repo, text, trusted, model, resume }) {
    ...
    if (!s) {
      ...
      s = new ClaudeSession({
        ...
        trusted: typeof trusted === "boolean" ? trusted : this.trustedDefault,
      ...
      if (typeof trusted === "boolean") s.setTrusted(trusted);
      s.transcript.push({ role: "user", text, at: Date.now() });
      s.prompt(text);
      this.send("state", sessionKey, { session: this.sessionSummary(s) });
    }
  ...
  _control({ action, args = {} }) {
    switch (action) {
      case "setTrusted":
        this.trustedDefault = !!args.value;
        for (const s of this.sessions.values()) s.setTrusted(this.trustedDefault);
        return this._sendList();
      case "list": return this._sendList();
      case "delete": {
        const s = this.sessions.get(args.sessionKey);
        if (s) { this._resolvePendingFor(args.sessionKey, "deny"); try { s.stop(); } catch {} this.sessions.delete(args.sessionKey); }
        return this._sendList();
      }
