...
export class WorkspaceManager {
  ...
  _prompt(sessionKey, { repo, worktree, text, trusted, mode, model, resume, by: from, image }) {
    ...
    if (!s) {
      ...
      // its own echo via `by`. `image`/`workspaceId` ride along too — confirmed in production the
      ...
      this.send("event", sessionKey, { kind: "user", text, at: turn.at, by: from || null, image: savedImage, workspaceId });
      s.prompt(text, image);
      s.start();
      ...
      if (savedImage) turn.image = savedImage;
      s.transcript.push(turn);
      this.send("event", sessionKey, { kind: "user", text, at: turn.at, by: from || null, image: savedImage, workspaceId: s.workspaceId });
      s.prompt(text, image);
      this.send("state", sessionKey, { session: this.sessionSummary(s) });
    }
  }
