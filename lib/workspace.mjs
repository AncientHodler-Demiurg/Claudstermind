...
export class WorkspaceManager {
   ...
   *  for this instead: the CURRENT state of the session it was watching, straight from the
   *  in-memory source of truth (`s.transcript`/`s.status` are updated live, turn-by-turn, not
   *  just at persist time — see `_onEvent`). A session that already ended (or a resync racing a
   *  fresh session's own creation) falls back to whatever was last durably saved, same source
   *  `_openTranscript` reads, so the pane still shows the real outcome instead of nothing. */
  _resync(sessionKey) {
    if (!sessionKey) return;
    const s = this.sessions.get(sessionKey);
    if (s) {
      return this.send("event", sessionKey, { kind: "resync", transcript: s.transcript, status: s.status,
        usage: s.usage, sessionId: s.sessionId, mode: s.mode, live: true });
    }
    const direct = this._latestWorkspaceRow(sessionKey);
    const bySession = direct ? null : store.findSession(this.transcriptDir, sessionKey);
    const workspaceId = direct ? sessionKey : bySession?.workspaceId;
    if (!workspaceId) return;   // nothing known about this key on this process — leave the pane as-is
    const row = direct || this._latestWorkspaceRow(workspaceId);
    const transcript = (this.transcriptDir ? store.readWorkspace(this.transcriptDir, workspaceId) : [])
      .map(({ workspaceId: _w, ...turn }) => turn);
    this.send("event", sessionKey, { kind: "resync", transcript, status: "idle",
      usage: bySession?.usage ?? null, sessionId: row?.sessionId ?? bySession?.sessionId ?? null, live: false });
  }
