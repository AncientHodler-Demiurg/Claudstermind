...
test("restartCommand returns the exact sudo+systemctl invocation as data — never runs it", async () => {
  ...
  // Default (daemon:true) restarts BOTH the engine daemon AND the web — for a reload that picks up
  // engine (workspace/session) changes. The daemon is listed first so it's coming back up as the web
  // returns and reconnects to it.
  assert.deepEqual(cmd, { cmd: "sudo", args: ["-n", "systemctl", "restart", "claudstermind-sessiond", "claudstermind"] });
...
test("restartCommand({daemon:false}) restarts the WEB only — engine (and running agents) untouched", () => {
  // A web/client-only reload must NOT restart sessiond, or it would interrupt in-flight agent turns
  // and lose a pending prompt (the deploy-survivable-agents property).
  assert.deepEqual(restartCommand({ daemon: false }), { cmd: "sudo", args: ["-n", "systemctl", "restart", "claudstermind"] });
});
