...
test("restartCommand returns the exact sudo+systemctl invocation as data — never runs it", async () => {
  ...
  // Restarts BOTH the engine daemon AND the web — so a Reload picks up engine (workspace/session)
  // changes too, not just web code. The daemon is listed first so it's coming back up as the web
