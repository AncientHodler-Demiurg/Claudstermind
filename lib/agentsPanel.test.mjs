...
test("no startedAt renders 'unknown', never a fake 0s", () => {
  // exactly the "first seen via background_tasks_changed" case: no startedAt is known, so
  // shapeBackground emits elapsedMs null (it used to emit 0, which read as "just started").
  const s = reduceBackground({ tasks: {} }, { kind: "background", tasks: [{ id: "a", taskType: "agent", description: "d" }] });
  const panel = shapeBackground(s, NOW);
  assert.equal(panel.agents[0].elapsedMs, null, "precondition: the server says unknown, not 0");
  assert.equal(panel.agents[0].startedAt, null);
