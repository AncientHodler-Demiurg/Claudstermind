...
test("localhostProcesses: non-Claudstermind apps are NOT core (collapse behind 'others')", () => {
  const rows = localhostProcesses([{ key: "a", name: "StoaChain Website", port: 5001 }], [{ key: "a", up: true }]);
  assert.notEqual(rows[0].core, true);
});
test("localhostProcesses: Claudstermind-named apps ARE core (shown with the web + daemon)", () => {
  const rows = localhostProcesses([{ key: "cd", name: "Claudstermind Dashboard", port: 3001 }], [{ key: "cd", up: true }]);
  assert.equal(rows[0].core, true);
});
...
test("changedFilesFromGit: includes committed diff lines", () => {
...
test("changedFilesFromGit: empty committed diff STILL surfaces working-tree changes (the reload-of-uncommitted bug)", () => {
  // A reload loads the ON-DISK working tree, so an uncommitted engine edit must count even when the committed
  // diff (liveSha..HEAD) is empty. Regression guard: previously this returned [] and sessiond never restarted.
  const files = changedFilesFromGit({ diffOut: "", porcelainOut: " M lib/workspace.mjs" });
  assert.deepEqual(files, ["lib/workspace.mjs"]);
});

test("changedFilesFromGit: UNIONs committed diff + working-tree changes, de-duped", () => {
  const files = changedFilesFromGit({ diffOut: "lib/workspace.mjs\ndocs/x.md\n", porcelainOut: " M lib/workspace.mjs\n?? lib/new.mjs" });
  assert.deepEqual(files.sort(), ["docs/x.md", "lib/new.mjs", "lib/workspace.mjs"]);
});
