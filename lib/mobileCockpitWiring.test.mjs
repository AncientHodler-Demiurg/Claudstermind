...
test("a queued message survives a VIEW SWITCH, not only a reload", () => {
  ...
  const enqueue = app.slice(app.indexOf("p._queue.push({ text, images: attachedImages });"), app.indexOf("p._queue.push({ text, images: attachedImages });") + 200);
  assert.match(enqueue, /wsQueueSave\(\)/, "saving on change is what makes the stored copy current");
  const drain = app.slice(app.indexOf("function drainQueue(p)"), app.indexOf("function drainQueue(p)") + 500);
  assert.match(drain, /wsQueueSave\(\)/, "…and clearing it on drain is what stops a sent message coming back");
});
