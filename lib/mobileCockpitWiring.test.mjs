...
test("the Chats sheet marks the box you are IN, not the one the app last focused", () => {
  // "While I am in other repository it shows the name of other repository." A cockpit is mounted PER
  // PANE, so the sheet you pull up belongs to THIS pane — but it marked whichever pane was globally
  // active (`st.activeId`), so opening it from DemiourgosMotionPictures highlighted Claudstermind and
  // labelled it live. The sheet was describing a different box than the one wrapped around it.
  const at = app.indexOf("chats: st.panes.map(");
  assert.ok(at > 0, "Core's chat list moved — this assertion is aimed at nothing");
  const body = app.slice(at, app.indexOf("})),", at));
  assert.match(body, /active: x\.id === p\.id/,
    "the active entry is the pane that OWNS this cockpit");
  assert.ok(!/active: x\.id === st\.activeId/.test(body),
    "st.activeId is the app's focus, which is a different question and a different box");
  // Busy is still reported for every box: "another one is working" is worth knowing from in here.
  assert.match(body, /x\.status === "running" \|\| x\.status === "deepwork" \? "working" : ""/,
    "a box that is working should say so even when it is not the one you are in");
});
...
test("coming back reconnects a dead stream instead of waiting out the zombie window", () => {
  // "Sometimes when I come back it hangs and I need to refresh." A stream is only declared dead after
  // WS_STALE_MS (65s) of silence, checked every 10s — the right instrument for a CONNECTED client
  // noticing a socket that quietly died, and the wrong one for a phone returning from the background.
  // While hidden its timers were frozen and nothing arrived, so on return the client sat on a
  // possibly-dead stream for up to ~75s showing the state it had when you left. Refreshing worked
  // because a reload rebuilds the stream at once.
  const at = app.indexOf("function wsResumeOnVisible");
  assert.ok(at > 0, "nothing acts on the one event that means 'the user is back'");
  const body = app.slice(at, app.indexOf("\n}", at));
  assert.match(app, /addEventListener\("visibilitychange", wsResumeOnVisible\)/,
    "becoming visible is handed to us for free and is the moment the stale clock is meaningless");
  assert.match(body, /readyState === 2/, "a closed stream must be rebuilt, not waited on");
  assert.match(body, /WS_RESUME_QUIET_MS/,
    "…on a shorter fuse than the zombie window, because the page was frozen rather than idle");
  const ms = Number((app.match(/const WS_RESUME_QUIET_MS = ([0-9_]+)/) || [])[1].replace(/_/g, ""));
  const stale = Number((app.match(/const WS_STALE_MS = ([0-9_]+)/) || [])[1].replace(/_/g, ""));
  assert.ok(ms < stale, `the resume fuse (${ms}ms) must be shorter than the zombie window (${stale}ms)`);
  // …and a HEALTHY stream must survive a quick app switch: reconnecting on every visibility change
  // would churn a working connection and re-ship a catch-up for nothing.
  assert.match(body, /else pactChatResyncAll\(\)/,
    "a live stream gets a catch-up, not a teardown");
});

test("the wake lock is re-applied, not acquired once and trusted", () => {
  // The browser RELEASES a screen wake lock every time the page is hidden. A one-shot request works
  // once and then silently stops working the first time you switch apps. It must also be given back
  // when you leave the chat views — it is a real battery cost — and the sentinel can be dropped by
  // the system on its own (battery saver, a call), which is what the `release` listener is for.
  const at = app.indexOf("async function wsWakeApply");
  assert.ok(at > 0, "the wake-lock controller must exist");
  const body = app.slice(at, app.indexOf("\n}", at));
  assert.match(body, /visibilityState === "visible"/,
    "a lock requested while hidden is refused, and one held across a hide is already gone");
  // A PIN, NOT A POLICY. It used to hold only on the chat views to save battery, so "keep the screen
  // on" quietly stopped meaning that the moment you looked at anything else — one more way for the
  // feature to look broken without saying so. Asked for directly: pin it, everywhere.
  assert.ok(!/VIEW === "workspace" \|\| VIEW === "pact"/.test(body),
    "a view condition makes the pin conditional, which is exactly what it must not be");
  assert.match(body, /WS_WAKE_ON && document\.visibilityState === "visible"/,
    "on plus visible, and nothing else — a hidden page cannot hold a lock in any case");
  assert.match(body, /addEventListener\("release"/,
    "the system can drop the lock; without this we believe we still hold one and never ask again");
  // Comments quote the OLD code to explain the fix, so this must read code, not prose.
  const codeOnly = app.replace(/\/\*[\s\S]*?\*\//g, "").split("\n").filter((l) => !/^\s*\/\//.test(l)).join("\n");
  assert.ok(!/catch \{ WS_WAKE = null; \}/.test(codeOnly),
    "a swallowed refusal is why this was reported as simply not working — the reason must be kept");
  assert.match(body, /WS_WAKE_ERR = /, "…and recorded, so the switch can say why it is not holding");
  assert.match(body, /if \(WS_WAKE_BUSY\) return;/,
    "one request at a time: `await` leaves a window where the sentinel is still null, so two callers " +
    "would each request and the second would orphan the first");
  assert.match(app, /function wsWakeArmRetry\(\)/,
    "some browsers grant a screen lock only from a user activation — a refusal must be retried on the " +
    "next touch rather than sitting there permanently refused");
  assert.match(app, /addEventListener\("visibilitychange", wsWakeApply\)/,
    "…and it must be re-applied when the page comes back, or it works exactly once");
  assert.match(app, /wsWakeApply\(\);\s*\/\/ held only on the chat views/,
    "the view change must re-evaluate it, or leaving the chat keeps the screen burning");
});
...
test("the ghost is published only while the countdown is actually running", () => {
  ...
  assert.match((function(){const a=app.indexOf('function wsAutoEnsure');return app.slice(a, app.indexOf('\\n  }', a));})(), /autoNext:\s*\(d\.arm && !d\.fire\)/,
...
test("every bubble carries when it was spawned, in both workspaces, from data that already existed", () => {
  ...
  // DATE AND TIME ON EVERY BUBBLE, not time-only for today. A bare "4:37 PM" is unambiguous only
  // while you are still in the same day as the turn, and these transcripts get read across days —
  // "I need a medallion with the date and time it was spawned."
  assert.match(app, /const label = d\.toLocaleDateString\(\[\], \{ day: "numeric", month: "short" \}\) \+ " " \+ hm;/,
    "the stamp must carry its own date, or it only reads correctly on the day it was written");
  // …and every bubble the CLIENT invents must carry one too, or the stamp is missing on exactly the
  // messages you are looking at when you wonder: a queued prompt, and a prompt echoed from another
  // device. Both were pushed without `at` and rendered blank.
  assert.equal((app.match(/at: Date\.now\(\) \}/g) || []).filter(Boolean).length >= 4, true,
    "all four queue paths (send + recover, in both workspaces) must stamp when YOU wrote it");
  assert.match(app, /images: d\.images \|\| \[\], at: d\.at \|\| Date\.now\(\)/,
    "a prompt echoed from another terminal must carry its arrival time, not render blank");
  assert.equal((app.match(/wsWhenChip\(q\.at\)/g) || []).length, 2,
    "a queued bubble is a bubble — it shows when you wrote it, in both workspaces");
  assert.match(app, /title: d\.toLocaleString\(\)/, "the full date/time belongs in the title — the short form is for scanning");
});

test("a read-only pane's phone box says so instead of swallowing what you type", () => {
  // Found while fixing "I come back to main chat, the typing box, I can't type in it": on a phone the
  // visible box is the cockpit's own, and it was never told about `p.readonly`. So a read-only pane
  // (the history column's 👁) accepted every keystroke and then `send()` returned at its first line —
  // the worst of both, a control that looks alive and does nothing. Measured: `mcDisabled:false` on a
  // pane whose own textarea read `disabled:true`.
  assert.match(app, /readonly: !!p\.readonly,/, "the host must publish it — the cockpit shows what the HOST says");
  assert.match(mod, /if \("readonly" in s\) state\.readonly = !!s\.readonly;/, "…and the cockpit must absorb it");
  assert.match(mod, /box\.disabled = !!state\.readonly;/, "…and stop taking text it cannot send");
  assert.match(mod, /state\.readonly \? BOX_PLACEHOLDER_RO/,
    "a disabled box with an inviting placeholder is still a lie — it has to say why");
});
