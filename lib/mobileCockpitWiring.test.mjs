...
test("the cockpit mounts only on a phone, and only with the switch on", () => {
  ...
  assert.match(app, /localStorage\.getItem\("ws\.mobile\.v2"\) === "1"/, "the switch is remembered");
  assert.match(app, /\[\?&\]m2=1/, "…and can be turned on by URL");
  assert.match(app, /\[\?&\]m2=0/, "…and off again, which is the half people forget");
});
