...
test("BOTH workspaces go through it — that is the entire point", () => {
  ...
  // THREE surfaces now: Core's pane, Pact's desktop pane, and Pact's mobile cockpit. The count is the
  // point — every surface that paints Send/Stop must reach this one function, and the moment a fourth
  // appears with its own chain of ternaries the two drift again (which is what 1.13.0 fixed, and what
  // the phone re-broke in 1.15.5 by reading `busy` on its own).
  assert.equal((app.match(/window\.ChatShell\.sendPresentation\(/g) || []).length, 3,
    "every surface must compute the shared decision — no chain of ternaries anywhere");
