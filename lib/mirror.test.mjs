...
import {
  ...
  mirrorForwardedHeaders, rewriteRootAbsoluteAssets, rewriteJsImports,
} from "./mirror.mjs";
...
test("rewriteRootAbsoluteAssets prefixes root-absolute entry <script>/<link> URLs — the mobile white-page fix", () => {
  // The real Vite dev-server entry: two root-absolute module scripts that <base> can't bend and the
  // fetch/XHR patch can't reach (the HTML parser fetches them directly). Before this, they resolved to
  // the origin root and only loaded if Referer/cookie provenance survived — which it doesn't over the
  // HTTPS relay on a phone, so the entry module 404'd and the app never mounted (white page).
  const vite = `<script type="module" src="/@vite/client"></script><script type="module" src="/src/main.tsx"></script>`;
  const out = rewriteRootAbsoluteAssets(vite, 3007);
  assert.match(out, /src="\/mirror\/3007\/@vite\/client"/);
  assert.match(out, /src="\/mirror\/3007\/src\/main\.tsx"/);

  // A stylesheet <link href> gets the same treatment.
  assert.match(rewriteRootAbsoluteAssets(`<link rel="stylesheet" href="/assets/app.css">`, 3007),
    /href="\/mirror\/3007\/assets\/app\.css"/);

  // Leave everything that ISN'T a root-absolute same-origin asset exactly as it was.
  const untouched = [
    `<script src="https://cdn.example.com/x.js"></script>`,   // absolute URL
    `<script src="//cdn.example.com/x.js"></script>`,          // protocol-relative
    `<script src="./rel.js"></script>`,                        // relative (handled by <base>)
    `<link href="/mirror/3007/already.css">`,                  // already prefixed — must not double
    `<a href="/internal/page">go</a>`,                         // not a script/link tag
  ].join("");
  const outU = rewriteRootAbsoluteAssets(untouched, 3007);
  assert.match(outU, /src="https:\/\/cdn\.example\.com\/x\.js"/);
  assert.match(outU, /src="\/\/cdn\.example\.com\/x\.js"/);
  assert.match(outU, /src="\.\/rel\.js"/);
  assert.match(outU, /href="\/mirror\/3007\/already\.css"/);
  assert.ok(!/mirror\/3007\/mirror\/3007/.test(outU), "an already-prefixed URL must never be prefixed twice");
  assert.match(outU, /<a href="\/internal\/page">/, "non script/link tags (e.g. <a>) are left alone");
});

test("rewriteJsImports prefixes root-absolute ES-module specifiers — the nested-module white-page fix", () => {
  // Exactly the shapes the real codex-playground entry module emits (verified against the live :3007
  // dev server). These are issued by the browser's native module loader, so the fetch/XHR patch can't
  // touch them; before this they resolved to the origin root and 404'd on the phone (no provenance).
  const mod = [
    `import __v0 from "/node_modules/.vite/deps/react_jsx-dev-runtime.js?v=2f7f937c";`,
    `import "/src/polyfills.ts";`,
    `import { App } from "/src/App.tsx";`,
    `import "/@fs/home/x/codex-ui/dist/ui.css";`,
    `const lazy = () => import("/src/panel.tsx");`,
    `export { thing } from "/src/thing.ts";`,
  ].join("\n");
  const out = rewriteJsImports(mod, 3007);
  assert.match(out, /from "\/mirror\/3007\/node_modules\/\.vite\/deps\/react_jsx-dev-runtime\.js\?v=2f7f937c"/, "query strings ride along");
  assert.match(out, /import "\/mirror\/3007\/src\/polyfills\.ts"/, "bare side-effect import");
  assert.match(out, /from "\/mirror\/3007\/src\/App\.tsx"/, "named import");
  assert.match(out, /import "\/mirror\/3007\/@fs\/home\/x\/codex-ui\/dist\/ui\.css"/, "the /@fs/ css import");
  assert.match(out, /import\("\/mirror\/3007\/src\/panel\.tsx"\)/, "dynamic import()");
  assert.match(out, /export \{ thing \} from "\/mirror\/3007\/src\/thing\.ts"/, "re-export from");
});

test("rewriteJsImports leaves non-root-absolute specifiers, and already-prefixed ones, alone", () => {
  const keep = [
    `import x from "react";`,                          // bare package specifier
    `import y from "./relative.js";`,                  // relative
    `import z from "../up.js";`,                       // relative
    `import w from "https://cdn/x.js";`,               // absolute URL
    `import v from "//cdn/x.js";`,                     // protocol-relative
    `import u from "/mirror/3007/already.js";`,        // already prefixed — must not double
  ].join("\n");
  const out = rewriteJsImports(keep, 3007);
  assert.equal(out, keep, "nothing that isn't a fresh root-absolute specifier may change");
  assert.ok(!/mirror\/3007\/mirror\/3007/.test(out), "an already-prefixed specifier must never be doubled");
});

test("rewriteJsImports does not rewrite the word 'from' followed by a slash inside ordinary prose/strings", () => {
  // A specifier can't contain whitespace; the path charclass excludes it, so a sentence like this in a
  // string literal is left alone (the space after the slash-word breaks the match).
  const s = `const msg = "copied from /home/user to /tmp";`;
  assert.equal(rewriteJsImports(s, 3007), s);
});

test("injectBase also applies the root-absolute asset rewrite, so a mirrored Vite entry loads deterministically", () => {
  const html = `<html><head><script type="module" src="/src/main.tsx"></script></head><body></body></html>`;
  const out = injectBase(html, 3007);
  assert.match(out, /src="\/mirror\/3007\/src\/main\.tsx"/, "the entry script is prefixed end-to-end through injectBase");
  assert.match(out, /<base href="\/mirror\/3007\/">/, "and the <base> still goes in");
});
