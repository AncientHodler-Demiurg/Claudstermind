...
test("neither signal fires on a navigation, so a bad dashboard URL still 404s", () => {
  assert.equal(mirrorFromReferer({ referer: "http://localhost:3001/mirror/3002/", "sec-fetch-mode": "navigate" }), null);
  assert.equal(mirrorFromCookie({ cookie: `${MIRROR_COOKIE}=3002`, "sec-fetch-mode": "navigate" }), null);
});
