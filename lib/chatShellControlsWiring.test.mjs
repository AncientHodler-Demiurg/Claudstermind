...
test("Core's loop is off by default, capped, and resets on a human send", () => {
  assert.ok(app.includes("autoContinue: { shown: false, on: !!p._autoContinue }"),
    "a fresh pane must not start an autonomous loop");
  assert.ok(app.includes("function wsAutoResetOnHumanSend(p) { p._autoCount = 0; p._autoCap = PACT_AUTO_CAP; }"),
    "the ceiling exists to stop a runaway robot, not to ration the human");
  assert.ok(app.includes("if (!p._autoSending) wsAutoResetOnHumanSend(p);"),
    "…so only a HUMAN send resets it — an auto send must count toward the cap");
  assert.ok(app.includes("p._autoCount = (p._autoCount || 0) + 1;"), "…and an auto send does count");
  assert.ok(app.includes("if (on && (p._autoCount || 0) >= cap) p._autoCap = (p._autoCount || 0) + PACT_AUTO_CAP;"),
    "re-ticking at the ceiling must grant the next batch, or the toggle looks dead");
});
...
test("Pact's auto-continue moved onto the shell's control instead of being duplicated beside it", () => {
  ...
  assert.ok(app.includes("autoContinue: (v) => {"), "Pact must wire the shell control to its engine");
