...
test("Core's loop is off by default, and the browser neither counts nor sends", () => {
  ...
  assert.ok(!/p\._autoCount = \(p\._autoCount \|\| 0\) \+ 1;/.test(app),
    "the browser must not count rounds — the ceiling is the server's, and two counters disagree the " +
    "moment one of them is asleep");
  assert.ok(!app.includes("ui.promptEl.value = WS_AUTO_TEXT"),
    "the browser must not put the next prompt in the box and send it: that is the second loop");
