...
test("parseGitLog reads date, hour + churn from an ISO --numstat window", () => {
  const out = [
    "__C__2026-07-15T23:47:12+02:00|abc123|feat: thing",
    "10\t2\tsrc/a.js",
    "0\t5\tsrc/b.js",
    "",
    "__C__2026-07-14T03:10:00+02:00|def456|fix: bug",
    "-\t-\tbin.png",         // binary → 0
    "3\t3\tsrc/c.js",
  ].join("\n");
...
test("repoCommitActivity aggregates per repo with byDay + totals", () => {
  ...
  // per-day hourly histogram is present and sums to the day's commit count
  const someDay = Object.keys(act.dayHours)[0];
  assert.ok(Array.isArray(act.dayHours[someDay]) && act.dayHours[someDay].length === 24);
  const hourSum = Object.values(act.dayHours).reduce((s, arr) => s + arr.reduce((a, b) => a + b, 0), 0);
  assert.equal(hourSum, 2);
