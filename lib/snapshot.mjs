...
import { readDataSizes } from "./workspace.mjs";
...
/** The Brain view payload. When transcriptDir is given, each repo also carries `raw` — the
 *  collected raw-conversation volume from `.claude/workspace` (matched by repo name/path). */
export function readBrain(brainDir, transcriptDir) {
  const out = { repos: [], worklog: [], totals: {}, daily: {} };
  const sizes = transcriptDir ? readDataSizes(transcriptDir) : [];
  const byExact = new Map(), byLast = new Map();
  for (const s of sizes) { byExact.set(s.repo, s); const last = String(s.repo).split(/[\\/]/).pop(); if (!byLast.has(last)) byLast.set(last, s); }
  const rawFor = (repoPath, key) => byExact.get(repoPath) || byLast.get(key) || byLast.get(String(repoPath).split(/[\\/]/).pop()) || null;
  let worklogLines = [];
...
export async function buildSnapshot(paths) {
  ...
  // The build sitting on THIS machine. The live relay has no other way to learn it, so
  // without this its Deploy panel can only report the pending version as "unreachable".
  try { snap.version = readVersion(); } catch { snap.version = null; }
  ...
  try { snap.brain = readBrain(brainDir, join(root, ".claude", "workspace")); } catch (e) { snap.brain = { repos: [], worklog: [], totals: {}, error: String(e) }; }
