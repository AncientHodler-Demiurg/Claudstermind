...
export const WS_CONTROL_ACTIONS = Object.freeze([
  ...
  "sessions", "sessionOpen", "sessionDelete",    // per-session history (the Pact chat panel): list one repo's saved sessions, open/rehydrate one, delete one
  "resync",                                      // reconnect catch-up: re-send a session's current live state
  "workspacesOn",                                // what workspaces are live on a given repo (multi-terminal)
  "worktrees", "worktreeAdd", "worktreeRemove",  // git worktrees: a second checkout for a parallel workspace
  "setModel", "setEffort", "setFastMode",        // model/effort/fast-mode: workspace default or one pane
  "setAutoWrap",                                 // opt a session out of the automatic-roll threshold (manual wrap is unaffected)
  "autoContinue",                                // the SERVER-side auto-continue loop: on/off, and "a human is typing here"
  "models",                                      // the model list (display name, effort/fast-mode support) for the selector
  "contextUsage",                                // context-window breakdown for one live session
  "usageLimits",                                 // claude.ai plan rate-limit utilization (5h/7d) — EXPERIMENTAL, see claudeSession.mjs
  "keysUsage",                                    // the multi-key store: each OAuth key's name/usage/active/exhausted, for the Usage tab
  "recall",                                       // read a rolled-off turn back out of the archive, by absolute P#/R# or by substring query
  "wrapPreview", "wrapNow",                       // manual wrap: preview what committing would archive, then actually do it
]);
