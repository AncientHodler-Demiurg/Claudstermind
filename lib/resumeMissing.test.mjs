...
test("output measured EITHER way — a streamed turn, or one the usage says produced tokens", () => {
  const root = fixture();
  const { m, sends } = mgr(root, join(root, "none"));
  for (const [label, pre, usage] of [
    ["streamed text", { kind: "assistant", text: "x" }, null],
    ["ran a tool", { kind: "tool_use", tools: [{ name: "Bash" }] }, null],
    ["usage alone", null, { output_tokens: 21039 }],
  ]) {
    ...
    assert.equal(s.transcript.filter((r) => r && r.kind === "turnError").length, 0, `${label}: not a failure`);
  }
  rmSync(root, { recursive: true, force: true });
});
