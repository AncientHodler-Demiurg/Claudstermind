...
test("modelRowLabel renders the HUMAN name, never a wire id and never a bare alias", () => {
  // An alias row is labelled by the MODEL IT RESOLVES TO, with no alias in the text. The alias is how
  // the pick is spelled on the wire, not a fact about the model — and appending it put the word
  // "default" in front of you on every paint. The exact wire id is still one hover away (modelRowTitle).
  assert.equal(H.modelRowLabel(OPUS), "Opus 4.5");
  assert.equal(H.modelRowLabel(DEFAULT_ROW), "Sonnet 4.5");
  assert.ok(!/default/i.test(H.modelRowLabel(DEFAULT_ROW)), "the word `default` must never reach the picker");
