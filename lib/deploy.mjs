 ...
 *  localTarball is an OS-appropriate path (the dashboard may run on Windows); remoteTarball is
 *  always the Linux box's /tmp path. */
export function deploySteps({ repoRoot, host = "stoanodeprime", localTarball, remoteTarball = "/tmp/cm-deploy.tgz",
  remoteDir = "/opt/claudstermind", version = "0.0.0", gitSha = "unknown", builtAt = "" }) {
  localTarball = localTarball || join(tmpdir(), "cm-deploy.tgz");
  ...
  return [
    ...
    { label: "Ship", cmd: "scp", args: ["-o", "BatchMode=yes", localTarball, `${host}:${tarball}`] },
