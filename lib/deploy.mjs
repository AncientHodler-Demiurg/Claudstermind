...
export function deploySteps({ repoRoot, host = "stoanodeprime", localTarball = "cm-deploy.tgz",
  ...
  return [
    // WHAT SHIPS. This list IS the image's build context, so it must carry every path relay/Dockerfile
    // COPYs — including `packages/claude-chat-shell/src`, the chat box itself, which index.html loads
    // from /chat-shell/* and the relay serves from that directory. Missing it does not degrade the
    // site, it fails the remote `docker build` outright ("/packages/claude-chat-shell/src": not
    // found), which is how the 1.13.5 deploy died. lib/release.test.mjs derives the requirement from
    // the Dockerfile so the next COPY is covered the day it lands.
    { label: "Package", cmd: "tar", args: ["czf", localTarball, "--exclude=relay/.env", "--exclude=relay/docker-compose.override.yml", "--exclude=node_modules", "--exclude=*.test.mjs", "dashboard", "lib", "relay", "packages/claude-chat-shell/src", "package.json", ".dockerignore"], cwd: repoRoot },
