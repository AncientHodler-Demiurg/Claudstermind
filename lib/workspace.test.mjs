...
function mgr(fx, extra = {}) {
  ...
  const m = new WorkspaceManager({ root: fx.root, secretsDir: fx.secretsDir, sdkQuery: mockQuery(),
    ...
    // No SDK session store for the fixture: _prompt only drops a resume id when it can SEE the
    // store and the file is genuinely absent, so an invisible store means "pass the id through
    // unchanged" — which is what every resume test below is asserting. A test that wants the
    // drop behaviour builds a real store (see lib/resumeMissing.test.mjs).
    sdkProjectsDir: join(fx.root, "no-sdk-store"),
...
test("worktree control actions create, list, and remove a worktree", () => {
  ...
  /* A REFUSED CREATE MUST STILL PUBLISH WHAT EXISTS. Creating a worktree whose checkout is already
     on disk is refused with "already exists — pick it from the list instead of creating it", and
     that refusal used to `return` before the list was sent. Combined with a client that only ever
     asked for the list when a person CHANGED the repo, the result was the dead end reported from a
     real session: the worktree was on disk, missing from the dropdown, and the one route left
     refused with an instruction that could not be followed. */
  const before = sends.length;
  m.handleIn("control", null, { action: "worktreeAdd", args: { repo: repoRel, name: "wt-a" } });
  const after = sends.slice(before);
  const err = after.find((s) => s.kind === "event" && s.data.kind === "error");
  assert.match(err.data.message, /already exists/, "it still says why");
  const listed = after.find((s) => s.kind === "state" && s.data.worktrees);
  assert.ok(listed, "…and it publishes the list the message tells you to pick from");
  assert.ok(listed.data.worktrees.some((w) => w.name === "wt-a"),
    "…which must actually contain the thing that already exists");
...
test("_maybeRoll: rolls onto a fresh session once past the threshold, emits the cue, then guards re-rolling", () => {
  ...
  grow(Math.floor(CAP / 4)); m._maybeRoll(s);
  assert.equal(rolls, 0, "no roll below the maxTurns threshold");

  grow(CAP + TAIL + 20); m._maybeRoll(s);
  ...
  grow(2 * (CAP + TAIL) + 40); m._maybeRoll(s);
  assert.equal(rolls, 2, "rolls again after another window");

  rmSync(fx.root, { recursive: true, force: true });
});

test("_maybeRoll: autoWrap:false opts a session out of the automatic threshold — manualRoll on the SAME session is unaffected", () => {
  // The web's auto-wrap checkbox (buildWrapControls) posts a setAutoWrap control action, which sets
  // this flag on the live ClaudeSession. _maybeRoll must refuse outright once it's false — a manual
  // wrap on the same session must still work, since it's an explicit action, not the threshold.
  const fx = fixture();
  const { m } = mgr(fx);
  let rolls = 0;
  const s = { key: "conv-roll-autowrap", workspaceId: "Test/repo@main", transcript: [], autoWrap: false,
    roll: () => { rolls++; return true; } };
  const CAP = ROLL_DEFAULTS.maxTurns, TAIL = ROLL_DEFAULTS.tailTurns;
  for (let i = 0; i < CAP + TAIL + 20; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "turn " + i });

  m._maybeRoll(s);
  assert.equal(rolls, 0, "autoWrap:false must refuse the automatic roll even well past the threshold");

  const rolled = m._performRoll(s, s.transcript.slice(m._rollFrom(s)));
  assert.equal(rolled, true, "manualRoll's underlying _performRoll must still work on an autoWrap:false session");
  assert.equal(rolls, 1);

  // Flipping it back on re-arms the automatic path for the NEXT window (not retroactively for turns
  // already covered by the wrap mark _rollFrom reads back).
  s.autoWrap = true;
  for (let i = s.transcript.length; i < 2 * (CAP + TAIL) + 40; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "turn " + i });
  m._maybeRoll(s);
  assert.equal(rolls, 2, "re-enabling autoWrap must let the automatic path fire again once past threshold");

  rmSync(fx.root, { recursive: true, force: true });
});

test("control setAutoWrap flips ONE session's flag and gates its automatic roll; a session with no override still defaults ON", async () => {
  const fx = fixture();
  const { m } = mgr(fx);
  m.handleIn("prompt", "a", { repo: "repo", text: "hi" });
  await new Promise((r) => setTimeout(r, 40));
  const s = m.sessions.get("a");
  assert.equal(s.autoWrap, true, "a freshly started session must default to auto-wrap ON");

  m.handleIn("control", null, { action: "setAutoWrap", args: { sessionKey: "a", enabled: false } });
  assert.equal(s.autoWrap, false, "the control action must reach the live session");

  m.handleIn("control", null, { action: "setAutoWrap", args: { sessionKey: "a", enabled: true } });
  assert.equal(s.autoWrap, true, "and flipping it back on must reach the same session");
  rmSync(fx.root, { recursive: true, force: true });
});
...
test("_ensureSegmentsMigrated: a LEGACY root archive is relocated into its workspace on the next roll", async () => {
  ...
  for (let i = 0; i < ROLL_DEFAULTS.maxTurns + ROLL_DEFAULTS.tailTurns + 20; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "turn " + i });
  m._maybeRoll(s);
...
test("_maybeRoll: segment numbering + absolute offsets survive a process RESTART (a fresh session object)", async () => {
  ...
  const mk = () => { const s = { key: "conv-restart", workspaceId: wid, transcript: [], roll: () => true };
    for (let i = 0; i < ROLL_DEFAULTS.maxTurns + ROLL_DEFAULTS.tailTurns + 20; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "turn " + i }); return s; };
...
// ---------------------------------------------------------------------------
// MANUAL WRAP — a user-triggered roll bypassing the automatic threshold, plus a read-only preview.
// Chat Shell migration, topic 7: production had NO manual trigger at all before this — only the
// automatic threshold in _maybeRoll. manualRoll/wrapPreview reuse _performRoll (the SAME code path
// the automatic roll uses) so a manual wrap cannot drift from what the automatic one does.
// ---------------------------------------------------------------------------
test("manualRoll: rolls even though the automatic threshold was never reached", () => {
  const fx = fixture();
  const { m, sends } = mgr(fx);
  let rolls = 0;
  const s = { key: "conv-manual-1", workspaceId: "Test/repo@main", transcript: [], roll: () => { rolls++; return true; } };
  // Just past the tail budget, nowhere near ROLL_DEFAULTS.maxTurns — the automatic path would refuse.
  for (let i = 0; i < ROLL_DEFAULTS.tailTurns + 10; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "turn " + i });
  m.sessions.set(s.key, s);

  m._maybeRoll(s);
  assert.equal(rolls, 0, "precondition: the automatic threshold does NOT fire here");

  const result = m.manualRoll(s.key);
  assert.equal(result.ok, true);
  assert.equal(rolls, 1, "manualRoll must actually perform the roll the automatic path refused");
  assert.ok(sends.some((x) => x.data && x.data.kind === "rolling"), "emits the same ⟳ rolling cue as an automatic roll");
  rmSync(fx.root, { recursive: true, force: true });
});

test("manualRoll: reports ok:false, reason 'too-light' rather than silently doing nothing, when there is nothing to roll", () => {
  const fx = fixture();
  const { m } = mgr(fx);
  const s = { key: "conv-manual-2", workspaceId: "Test/repo@main", transcript: [{ role: "user", text: "hi" }], roll: () => true };
  m.sessions.set(s.key, s);
  const result = m.manualRoll(s.key);
  assert.equal(result.ok, false);
  assert.equal(result.reason, "too-light");
  rmSync(fx.root, { recursive: true, force: true });
});

test("manualRoll: an unknown session key reports ok:false, reason 'no-session', never throws", () => {
  const fx = fixture();
  const { m } = mgr(fx);
  assert.doesNotThrow(() => {
    const result = m.manualRoll("does-not-exist");
    assert.equal(result.ok, false);
...
test("wrapPreview: NEVER mutates state — calling it repeatedly reports the identical span, and a real roll afterward still works", () => {
  const fx = fixture();
  const { m, sends } = mgr(fx);
  const s = { key: "conv-preview-1", workspaceId: "Test/repo@main", transcript: [], roll: () => true };
  for (let i = 0; i < ROLL_DEFAULTS.tailTurns + 30; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "x".repeat(10) });
  m.sessions.set(s.key, s);

  const p1 = m.wrapPreview(s.key);
  const p2 = m.wrapPreview(s.key);
  assert.deepEqual(p1, p2, "calling the preview twice must not change its own answer");
  assert.equal(m._rollFrom(s), 0, "a preview must not move the window boundary");
  assert.ok(!s.transcript.some((r) => r && r.kind === "wrapped"), "and must not leave a wrap mark behind");
  assert.equal(p1.empty, false);
  assert.ok(p1.p.count > 0 && p1.r.count > 0, "some prompts and responses must be in the previewed head");
  assert.equal(p1.p.chars, p1.p.count * 10, "prompt char total must match the fixture's own text length");

  const result = m.manualRoll(s.key);
  assert.equal(result.ok, true, "a preceding preview must not have consumed or corrupted anything a real roll needs");
  rmSync(fx.root, { recursive: true, force: true });
});

test("wrapPreview: reports empty:true (not null, not a throw) when the conversation is too short to have a head", () => {
  const fx = fixture();
  const { m } = mgr(fx);
  const s = { key: "conv-preview-2", workspaceId: "Test/repo@main", transcript: [{ role: "user", text: "hi" }], roll: () => true };
  m.sessions.set(s.key, s);
  const preview = m.wrapPreview(s.key);
  assert.equal(preview.empty, true);
  rmSync(fx.root, { recursive: true, force: true });
});

test("wrapPreview's P#/R# ranges continue from the archive offsets — matches a real roll's own numbering exactly", () => {
  const fx = fixture();
  const { m } = mgr(fx);
  const wid = "Test/repo@main";
  const s = { key: wid, workspaceId: wid, transcript: [], roll: () => true };
  const need = ROLL_DEFAULTS.maxTurns + ROLL_DEFAULTS.tailTurns + 20;
  for (let i = 0; i < need; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "t" + i });
  m.sessions.set(s.key, s);
  m._maybeRoll(s);                              // first (automatic) roll — establishes _archivedPrompts/_archivedResponses
  const beforeP = s._archivedPrompts, beforeR = s._archivedResponses;

  for (let i = 0; i < ROLL_DEFAULTS.tailTurns + 10; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "u" + i });
  const preview = m.wrapPreview(s.key);
  assert.equal(preview.p.from, beforeP + 1, "the preview's prompt range must continue exactly where the archive left off");
  assert.equal(preview.r.from, beforeR + 1, "…and the response range too");
  rmSync(fx.root, { recursive: true, force: true });
});

test("control wrapPreview + wrapNow: the full wire path, via handleIn, not just the bare methods", () => {
  const fx = fixture();
  const { m, sends } = mgr(fx);
  const s = { key: "conv-control-wrap", workspaceId: "Test/repo@main", transcript: [], roll: () => true };
  for (let i = 0; i < ROLL_DEFAULTS.tailTurns + 15; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "c" + i });
  m.sessions.set(s.key, s);

  m.handleIn("control", null, { action: "wrapPreview", args: { sessionKey: s.key } });
  const previewSend = sends.find((x) => x.data && x.data.kind === "wrapPreview");
  assert.ok(previewSend, "wrapPreview must reach the client as an event");
  assert.equal(previewSend.data.preview.empty, false);

  m.handleIn("control", null, { action: "wrapNow", args: { sessionKey: s.key } });
  const resultSend = sends.find((x) => x.data && x.data.kind === "wrapResult");
  assert.ok(resultSend, "wrapNow's outcome must reach the client as an event");
  assert.equal(resultSend.data.ok, true);
  rmSync(fx.root, { recursive: true, force: true });
});
...
function archivedFixture() {
  ...
  // Enough filler to push the first two turns OUT of the carried tail and into the archive, derived
  // from the constants so raising tailTurns cannot silently stop this fixture from archiving anything.
  const need = ROLL_DEFAULTS.maxTurns + ROLL_DEFAULTS.tailTurns + 20;
  for (let i = 0; i < need; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: "filler " + i });
  m._maybeRoll(s);
  sends.length = 0;
...
test("GUARANTEE: a roll does NOT truncate s.transcript — jump-to-#N still reaches rolled-off turns", () => {
  ...
  const wid = store.workspaceId("repo", "main");
  const s = { key: wid, workspaceId: wid, transcript: [], roll: () => true };
  const N = ROLL_DEFAULTS.maxTurns + ROLL_DEFAULTS.tailTurns + 20;   // enough to actually trigger a roll
  for (let i = 0; i < N; i++) s.transcript.push({ role: i % 2 ? "assistant" : "user", text: (i % 2 ? "r" : "p") + (Math.floor(i / 2) + 1) });
  const before = s.transcript.length;

  m._maybeRoll(s);
  assert.equal(m._rollFrom(s), before + 1, "the roll marks how far it archived — the boundary is the wrap mark itself…");
  // …but the ARRAY is retained in full. jump-to-#N depends on this: if a roll ever started splicing,
  // `around`/`aroundTurn` would silently stop reaching old turns and the client would have no field
  // telling it to fall back to `recall`. A roll APPENDS exactly one row — the `wrapped` mark that
  // draws the boundary line in the transcript — and removes none.
  assert.equal(s.transcript.length, before + 1, "the array grows by the wrap mark only — nothing is spliced out");
  assert.deepEqual(s.transcript.slice(0, before).map((r) => r.text), Array.from({ length: before }, (_, i) => (i % 2 ? "r" : "p") + (Math.floor(i / 2) + 1)),
    "every pre-roll row is still there, in order");
  const mark = s.transcript[before];
  assert.equal(mark.kind, "wrapped", "the appended row is the wrap mark");
  assert.equal(typeof mark.at, "number", "it carries the same `at` the rolling event does, so a client's optimistic copy is replaced, not doubled");
