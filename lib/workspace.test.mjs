...
test("a prompt with an image includes the image in the LIVE broadcast event, not just the persisted record", async () => {
  ...
  assert.ok(userEvent.data.image, "the live event must carry the image, not just the persisted JSONL record");
  assert.equal(userEvent.data.image.mediaType, "image/png");
  assert.match(userEvent.data.image.path, /images[\\/].*\.png$/);
