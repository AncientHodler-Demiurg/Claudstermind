...
const { pactFindMatches, pactReplaceOne, pactReplaceAll, pactFindOverlaySegs } = new Function(
  block + "\nreturn { pactFindMatches, pactReplaceOne, pactReplaceAll, pactFindOverlaySegs };"
)();
...
// ---- overlay segmentation: covers the WHOLE text so columns align; marks the current match distinctly.
test("overlaySegs: gaps + hits cover the whole text with the current match flagged", () => {
  const t = "foo bar foo";
  const ms = pactFindMatches(t, "foo");   // [0..3], [8..11]
  assert.deepEqual(pactFindOverlaySegs(t, ms, 1), [
    { text: "foo", mark: "hit" },
    { text: " bar ", mark: null },
    { text: "foo", mark: "cur" },
  ]);
  // concatenating every segment reproduces the source text exactly (no dropped/duplicated chars)
  assert.equal(pactFindOverlaySegs(t, ms, 0).map((s) => s.text).join(""), t);
});

test("overlaySegs: leading match, trailing gap, and no-current (idx -1)", () => {
  const t = "foobar";
  const ms = pactFindMatches(t, "foo");   // [0..3]
  assert.deepEqual(pactFindOverlaySegs(t, ms, -1), [
    { text: "foo", mark: "hit" },
    { text: "bar", mark: null },
  ]);
...
test("overlaySegs: empty matches → single gap covering all text; empty text → []", () => {
  assert.deepEqual(pactFindOverlaySegs("abc", [], 0), [{ text: "abc", mark: null }]);
  assert.deepEqual(pactFindOverlaySegs("", [], 0), []);
});

test("overlaySegs: zero-width matches are skipped (nothing to paint)", () => {
  const segs = pactFindOverlaySegs("abc", [{ start: 1, end: 1 }], 0);
  assert.deepEqual(segs, [{ text: "abc", mark: null }]);
});
