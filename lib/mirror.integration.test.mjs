...
test("mirror proxies a real dev server end to end", async (t) => {
  ...
  await t.test("a typed dashboard URL still 404s on the dashboard, cookie notwithstanding", async () => {
    ...
    assert.equal(r.status, 404);
    assert.equal(r.body, "dashboard 404", "a navigation must land on the dashboard, not silently become the mirrored site");
  });
