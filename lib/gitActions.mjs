...
function git(cwd, args) {
  const r = spawnSync("git", ["-C", cwd, ...args], { encoding: "utf8", windowsHide: true, maxBuffer: 16 * 1024 * 1024 });
...
export function pushRepo(abs) {
  ...
  if (r.code !== 0) {
    return { ok: false, branch, message: `git push failed: ${(r.err || r.out || "").slice(-400)}` };
  }
