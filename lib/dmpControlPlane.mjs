...
// NAME COLLISION, deliberately renamed (was DMP_MAIN_URL): this is OUR loopback probe target — the address
// this box uses to ask the local dmp-main "are you alive?". It is NOT the `DMP_MAIN_URL` env var from the
// early DMP proxy design, which was the REJECTED http-proxy alternative to the reverse tunnel and no longer
// exists on the DMP side (confirmed by the DMP agent: zero hits across every .mjs). Two different things
// sharing one name is exactly how a settled decision gets re-litigated a month later.
export const DMP_MAIN_LOCAL_URL = `http://127.0.0.1:${DMP_PORT}`;
 ...
 */
export async function gatherDmpStatus({ mainUrl = DMP_MAIN_LOCAL_URL, remoteUrl = null } = {}) {
