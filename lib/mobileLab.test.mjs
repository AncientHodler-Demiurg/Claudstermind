...
test("the header carries ONE state medallion — the connection — and nothing it does not need", () => {
  const { page, win } = runPage();
  const mob = findIn(page, ".mob");
  const head = findIn(mob, ".m-head");
  assert.ok(head.textContent.includes("Live"), "connected/working must be readable without opening anything");
  assert.ok(head.textContent.includes("Claudstermind"), "…beside the name, which must not be squeezed for chips");
  // The other two moved, each to where it means something:
  //   the agents COUNT rides the rail that opens the pane listing them,
  //   the reconnect NOTICE is an event that says so in words and then leaves.
  assert.ok(!/◉\s*\d/.test(head.textContent), "the agents count is not a header chip");
  const railR = findIn(mob, ".m-rail.--r");
  assert.ok(findIn(railR, ".m-rail-n"), "it rides the rail that opens the pane listing them");
  assert.equal(findIn(railR, ".m-rail-n").textContent, "1", "…and it is the RUNNING count, not the total");
  assert.ok(railR.classList.contains("--busy"), "…which also colours the rail, so it reads at a glance");
});

test("a reconnection is an EVENT: it says so in words, then clears itself", () => {
  const { page, win } = runPage();
  // A permanent ↻ was read as a reload button, and a thing that has finished happening does not
  // deserve a chip forever.
  const chip = findAllIn(findIn(page, ".mob"), ".m-hmed").find((c) => c.textContent.includes("↻"));
  assert.ok(chip, "it is shown when it happens");
  assert.ok(chip.textContent.includes("caught up"), "…in words, not as a glyph that reads as an action");
  assert.ok(chip.classList.contains("--fade"), "…arriving, so it reads as having just happened");
  assert.match(html, /reconnectTimer = setTimeout\(function \(\) \{ S\.reconnected = false/,
    "…and it clears itself rather than becoming furniture");
  win.__S.reconnected = false; win.__render();
  assert.ok(!findIn(findIn(page, ".mob"), ".m-head").textContent.includes("↻"), "once cleared the row is three items");
});
...
test("the rails sit at the INPUT level, so the transcript keeps the whole width", () => {
  ...
  // AND THEY RESERVE NOTHING. Padding the zone to keep them clear took 48px from the field you type
  // into, to protect two corners that had nothing in them; they are drawn over instead.
  const css = html.slice(html.indexOf("<style>"), html.indexOf("</style>")).replace(/\s+/g, " ");
  assert.match(css, /\.m-low\{position:relative;flex:0 0 auto;display:flex;flex-direction:column\}/,
    "the low zone must reserve no side space for the rails");
  assert.match(css, /\.m-rail\{position:absolute;top:0;transform:translateY\(-50%\)/,
    "…and each rail must be centred on the seam between the transcript and the footer");
...
test("no ☰ in the button row: every door it opened is already open elsewhere", () => {
  const { page } = runPage();
  const row = findIn(findIn(page, ".mob"), ".m-btnrow");
  assert.ok(!row.textContent.includes("☰"), "the menu button is gone");
  // What it used to open is still reachable, twice: the rail, and the Chats riser.
  assert.ok(findIn(findIn(page, ".mob"), ".m-rail.--l"), "the left rail opens the pane");
  assert.ok(findAllIn(findIn(page, ".mob"), ".m-riser")[0], "…and so does the Chats riser");
  assert.ok(findIn(row, ".m-ico"), "attach stays — it has nowhere else to be");
});

test("History is its own page, and carries what the sidebar column carried: search and scope", () => {
  const { page, win } = runPage();
  findIn(page, ".m-rail.--l").fire("click");
  const panel = findIn(findIn(page, ".mob"), ".m-panel.--l");
  const row = findAllIn(panel, ".m-row").find((r) => r.textContent.includes("All conversations"));
  assert.ok(row, "the left pane is where you reach it — it is about this chat box");
  row.fire("click");
  assert.equal(win.__S.overlay, "history");
  const over = findIn(findIn(page, ".mob"), ".m-over");
  assert.ok(findIn(findIn(over, ".m-find"), "input"), "a search field, because history IS search");
  assert.ok(over.textContent.includes("This repository") && over.textContent.includes("All repositories"),
    "…and the scope toggle the sidebar column had, or this is a downgrade");
  const rows = findAllIn(over, ".m-hist");
  assert.ok(rows.length >= 1, "conversations of this repository by default");
  assert.ok(over.textContent.includes("Resume"), "…and Resume, which is not the same action as Open");
  assert.ok(over.textContent.includes("worktree removed"), "…including the one whose worktree is gone, which must say so BEFORE you pick it");
  // scope really widens
  const all = findAllIn(over, ".m-btn").find((b) => b.textContent.includes("All repositories"));
  all.fire("click");
  const after = findIn(findIn(page, ".mob"), ".m-over");
  assert.ok(findAllIn(after, ".m-hist").length > rows.length, "all repositories is a wider list, not a relabelled one");
});

test("a scrolling surface that is also a flex column must not let its children shrink", () => {
  // "there isn't a scroll bar, and i cant scroll down" on the repository chooser. The content was not
  // overflowing — it was being COMPRESSED: every scroll container here is also a flex column (that is
  // how sections stack with a gap), and a flex item shrinks by default, so the cards squashed and
  // scrollHeight stayed equal to clientHeight. Measured in Chromium at 412x915 before and after:
  // scrollHeight 810 → 910 against a clientHeight of 810.
  const css = html.slice(html.indexOf("<style>"), html.indexOf("</style>")).replace(/\s+/g, " ");
  assert.match(css, /\.m-over-body > \*, \.m-sheet-body > \*, \.m-pbody > \*, \.m-list > \*, \.m-sec > \* \{ flex: 0 0 auto; \}/,
    "every scroll container's direct children must be flex:0 0 auto, or the list gets quietly shorter than its content");
  // …and each of those five containers must actually be one of the page's scrolling surfaces.
  for (const sel of ["m-over-body", "m-sheet-body", "m-pbody", "m-list"])
    assert.ok(new RegExp("\\." + sel + "\\{[^}]*overflow-y:auto").test(css), `.${sel} must scroll`);
});
