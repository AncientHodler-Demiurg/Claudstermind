...
test("what the header used to say is in the left panel, including the REAL stats chips", () => {
  const { page, win } = runPage();
  findIn(page, ".m-rail.--l").fire("click");
  const panel = findIn(findIn(page, ".mob"), ".m-panel.--l");
  const text = panel.textContent;
  for (const must of ["Session", "Claudstermind", "agents", "Conversations", "Recall"])
    assert.ok(text.includes(must), `the left panel must carry "${must}" — it is what the header row said`);
  ...
  assert.ok(win.__S.stats.tokens > 0);
});
