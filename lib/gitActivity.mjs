...
/** Parse one repo's `git log --numstat` window into commits with date + churn. */
export function parseGitLog(out) {
  ...
  for (const line of (out || "").split(/\r?\n/)) {
    if (line.startsWith(COMMIT)) {
      ...
      cur = { date, hash, subject, churn: 0, insertions: 0, deletions: 0 };
      commits.push(cur);
    } else if (cur) {
...
function logForRepo(abs, sinceDays) {
  const r = spawnSync("git", ["-C", abs, "log", `--since=${sinceDays} days ago`, "--date=short",
    `--pretty=tformat:${COMMIT}%ad|%h|%s`, "--numstat", "--no-merges"],
...
export function repoCommitActivity(repos, root, opts = {}) {
  ...
  for (const r of repos || []) {
    ...
    outRepos.push({
      ...
      total, byDay,
      commits: stripMessages ? undefined : commits.map((c) => ({ date: c.date, hash: c.hash, subject: c.subject, churn: c.churn })),
    });
  ...
  return { sinceDays, days, repos: outRepos, totals };
}
