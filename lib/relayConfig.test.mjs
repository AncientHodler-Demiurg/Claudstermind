...
test("writeRelayConfig REFUSES to overwrite a corrupt file — no field loss, backup kept", () => {
  // Regression: a corrupt relay.json used to read as DEFAULTS, so patching only `enabled` wiped the saved url.
  const dir = mkdtempSync(join(tmpdir(), "rl-corrupt-"));
  writeFileSync(join(dir, "relay.json"), "{ broken", "utf8");
  assert.throws(() => writeRelayConfig(dir, { enabled: true }), /corrupt/i);
  assert.equal(readFileSync(join(dir, "relay.json"), "utf8"), "{ broken");   // untouched
  assert.ok(existsSync(join(dir, "relay.json.corrupt.bak")));
})

test("writeRelayConfig works on a fresh dir (missing file = defaults base)", () => {
  const dir = mkdtempSync(join(tmpdir(), "rl-fresh-"));
  const next = writeRelayConfig(dir, { enabled: true, url: "brain.example.eu" });
  assert.equal(next.enabled, true); assert.equal(next.url, "wss://brain.example.eu/agent");
})
