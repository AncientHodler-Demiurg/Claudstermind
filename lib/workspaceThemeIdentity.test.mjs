...
// styles.css @imports chat-shell-theme.css (the file the chat-shell lab ALSO links directly) — the
// accent/palette rules now live there, not in styles.css itself, so both must be read to see the
// cascade production actually gets.
const css = readFileSync(join(__dir, "..", "dashboard", "public", "styles.css"), "utf8")
  + "\n" + readFileSync(join(__dir, "..", "packages", "claude-chat-shell", "src", "theme.css"), "utf8");
