...
test("prompt-state colours are COPIED from production, not re-invented", () => {
  // The lab had invented washed-out versions of these, so what was being judged was not what ships.
  // Each colour is asserted to appear in BOTH files — if production changes one, this goes red.
  const pairs = [
    ["#f59e0b", "queued (orange)"],
    ["#f472b6", "queued behind deep work (pink)"],
    ["#172554", "interrupted (blue)"],
    ["#3b5bbf", "interrupted border"],
    ["#3a0d0d", "discarded (red)"],
    ["#7f1d1d", "discarded border"],
  ];
  for (const [hex, what] of pairs) {
    assert.ok(prodCss.includes(hex), `production no longer uses ${hex} for ${what} — the lab is now stale`);
    assert.ok(html.includes(hex), `the lab is missing the production colour ${hex} for ${what}`);
  }
});
