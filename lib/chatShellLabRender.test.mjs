...
test("Compact and Wrap are ONE split control — the two answers to the same question", () => {
  ...
  const { doc } = runLab();
  const row = doc.querySelector("#f-ctxrow");
  assert.ok(row, "the context-actions row must exist in the footer");
  assert.equal(row.parentNode && row.parentNode.classList.contains("rg-footer"), true,
    "…and must hang off the footer, not the header");
  assert.ok(doc.querySelector("#f-autowrap"), "the auto-wrap tick moved down with it");
});
...
test("the header is ONE split row: actions left, numbers right", () => {
  // The search line and the stats line were separate full-width rows, each half empty.
  assert.ok(html.includes('class: "hrow --split"'), "the header must use a single split row");
  assert.ok(html.includes('class: "hleft"') && html.includes('class: "hright"'), "with an explicit left and right half");
  const { doc } = runLab();
  assert.ok(doc.querySelector(".hleft"), "left half must render");
  assert.ok(doc.querySelector(".hright"), "right half must render");
  assert.ok(doc.querySelector("#h-jump"), "search stays on the left");
});
...
test("the footer still carries the context ACTIONS after the header rework", () => {
  ...
  const { doc } = runLab();
  const row = doc.querySelector("#f-ctxrow");
  assert.ok(row, "the context-actions row must exist");
