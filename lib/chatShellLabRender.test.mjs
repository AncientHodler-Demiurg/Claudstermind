...
function runLab() {
  ...
  new Function("window", "document", "ChatShell", script
    ...
    + "\nwindow.__advanceStream = advanceStream; window.__mdRender = mdRender;")(win, doc, win.ChatShell);
  return {
...
test("compaction count and the wrap RANGE are both on screen", () => {
  ...
  assert.ok(html.includes('" compacted \\u00b7 this window"'), "the compaction count must always render, scoped to the window");
...
test("the footer Wrap button explains its refusal instead of just being dead", () => {
  ...
  assert.ok(html.includes("wbtn2.disabled = !wr2.canWrapManually"), "it stays gated below the threshold");
  assert.ok(html.includes('"\\u27f3 Wrap at " + Math.round(wr2.manualAt * 100)'),
...
test("the two counters have DIFFERENT scopes, and say which", () => {
  ...
  assert.ok(html.includes("compacted \\u00b7 this window"), "compactions must be labelled per-window");
  assert.ok(html.includes("wrapped \\u00b7 this conversation"), "wraps must be labelled per-conversation");
  assert.ok(html.includes("mk.afterRound >= lastWrapRound"), "compactions must be counted only since the last wrap");
  assert.ok(html.includes("resets at every wrap"), "…and the reset must be stated, not left to be discovered");
  assert.ok(html.includes("Cumulative: this only ever goes up"), "…and the wrap count's opposite behaviour too");
});
...
test("the toggle explains that nothing is created or split", () => {
  // The store already writes one file per conversation (appendTurn(dir, id, sessionId, …)) and offers
  // both readSession() and readWorkspace(); Core simply picks the merged view. So this is a read-mode
  // flip, and the UI should say so rather than implying a migration.
  assert.ok(html.includes("already stored separately") || html.includes("already keeps them apart"),
    "the toggle must say the conversations were always stored apart");
  assert.ok(html.includes("stops merging them for display"), "…and that the change is one of display");
});

test("the toggle is Core-only — Pact cannot be made single-chat", () => {
  assert.ok(html.includes('CS.slotsFor(S.kind, { multiChat: S.multiChat })'), "the flag must reach slotsFor");
  const { doc, S, render } = runLab();
  S.kind = "pact"; render();
  assert.ok(!doc.querySelector("#h-multichat"), "Pact must not offer the toggle at all");
});
...
test("calm mode cannot reflow: the streaming block is unformatted and pre-wrapped", () => {
  ...
  assert.ok(/\.msg\.--calm \.rawstream\{white-space:pre-wrap/.test(html),
    "the live block must be pre-wrap so appended characters cannot re-lay-out what is above them");
  assert.ok(html.includes("no markdown, so adding characters can never reflow"), "…and the reason recorded");
});
...
test("counter mode has a FIXED height — that is the whole point", () => {
  ...
  assert.ok(/\.msg\.--counter\{min-height:46px/.test(html), "the counter bubble must not grow while text arrives");
  assert.ok(html.includes("a fixed height cannot push the scroll"), "…stated as the reason");
});
...
test("the reveal is ONE transition at the end, not a per-character animation", () => {
  ...
  assert.ok(html.includes(".msg.--live{transition:none}"), "no transition while characters arrive");
  assert.ok(html.includes("@keyframes reveal"), "one settle at the end");
...
test("the transcript stays pinned to the newest turn across renders — never snaps back to the FIRST turn", () => {
  // Reported directly: "whenever I do something, it's always pushed to the beginning." Root cause:
  // build() throws away .rg-core and builds a fresh one on EVERY render() — a brand-new element starts
  // at scrollTop 0 — and nothing restored it, even though S.live (follow-newest) defaults to true.
  const { doc, S, render } = runLab();
  assert.equal(S.live, true, "Live (follow-newest) is the default");

  const core1 = doc.querySelector(".rg-core");
  assert.equal(core1.scrollTop, core1.scrollHeight,
    "the very first paint must already show the newest turn, not the oldest");

  // Any state change triggers a full rebuild. That must not relocate the reader.
  render();
  const core2 = doc.querySelector(".rg-core");
  assert.notEqual(core2, core1, "build() genuinely replaces the node — this is what made the bug possible");
  assert.equal(core2.scrollTop, core2.scrollHeight, "still pinned to the newest turn after a second render");

  // The exact scenario reported: running the stream simulator, which re-renders repeatedly.
  doc.querySelector("#simStream").onclick();
  clearInterval(S.stream._timer); S.stream._timer = null;
  for (let i = 0; i < 5; i++) render();
  const core3 = doc.querySelector(".rg-core");
  assert.equal(core3.scrollTop, core3.scrollHeight,
    "repeated renders while an answer streams must stay pinned to the bottom, not jump to the top");
});
