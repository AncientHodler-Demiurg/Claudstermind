...
test("runningVersion freezes at this process's first readVersion() call; version keeps reading live", () => {
  // By this point in the file, readVersion() has already run at least once (the tests above), so
  // ENV_CACHE.runningVersion is already frozen at whatever the package version was THEN.
  const before = readVersion();
  const frozen = before.runningVersion;
  const prev = process.env.CM_VERSION;
  process.env.CM_VERSION = "8.8.8";   // simulate the on-disk/env version changing under a live process
  const after = readVersion();
  assert.equal(after.version, "8.8.8", "`version` reflects the change immediately — it's read live every call");
  assert.equal(after.runningVersion, frozen, "`runningVersion` does NOT — it's what THIS process actually loaded at start, unaffected by a later on-disk/env change");
  assert.notEqual(after.version, after.runningVersion, "the whole point: they now visibly diverge, exactly the case a long-running unrestarted process hits");
  if (prev === undefined) delete process.env.CM_VERSION; else process.env.CM_VERSION = prev;
});
