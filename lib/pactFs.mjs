...
/** Continuous write-back: append a dated note to the pact brain's LEARNINGS.md (created if absent).
 *  `stamp` is injected so the caller controls the timestamp (and tests are deterministic). */
export function appendBrainNote(brainDir, text, stamp) {
  const t = String(text == null ? "" : text).trim();
  if (!t) return { ok: false, error: "empty note" };
  const file = join(brainDir, "LEARNINGS.md");
  try {
    if (!existsSync(brainDir)) mkdirSync(brainDir, { recursive: true });
    const entry = `\n\n## ${stamp} — from Pact chat\n${t}\n`;
    appendFileSync(file, entry);
    return { ok: true, file, bytes: Buffer.byteLength(entry) };
  } catch (e) { return { ok: false, error: e.message }; }
}
