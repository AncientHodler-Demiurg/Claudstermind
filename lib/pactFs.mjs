...
import { resolveWorktreeDir, listWorktrees, createWorktree, removeWorktree, mergeWorktree } from "./worktrees.mjs";
...
// The Pact repo's path relative to the master root — the key `worktrees.mjs` slugs its worktrees under
// (`$ROOT/.worktrees/<slug>/<name>`). Must match what the Core workspace passes when it CREATES an
// Ouronet worktree, so the Pact IDE resolves the same folders.
export const PACT_REPO_REL = join("OuroborosNetwork", "_onchain", "Ouronet");
// Resolve the Pact repo root for a given worktree: main → the primary checkout; a named worktree → its
// isolated checkout under .worktrees (or null if that worktree doesn't exist — callers error, never
// silently fall back to main, which would defeat the isolation). This is the one place the Pact IDE
// maps "which worktree" → "which folder on disk".
export function pactRootFor(masterRoot, worktree) {
  if (!worktree || worktree === "main") return pactRoot(masterRoot);
  return resolveWorktreeDir(masterRoot, PACT_REPO_REL, worktree);
}
// Every checkout of the Pact repo (main + worktrees) for the box worktree selector: [{ name, branch, isMain }].
export function pactWorktrees(masterRoot) {
  return listWorktrees(masterRoot, PACT_REPO_REL);
}
// Worktree lifecycle for the Pact IDE (Stage-3): create / remove / merge-to-main. One place so the local
// endpoint and the tunnel bridge share it. `merge` is conflict-safe (aborts, leaving main untouched).
export function pactWorktreeAction(masterRoot, action, name) {
  if (action === "create") return createWorktree(masterRoot, PACT_REPO_REL, name);
  if (action === "remove") return removeWorktree(masterRoot, PACT_REPO_REL, name);
  if (action === "merge") return mergeWorktree(masterRoot, PACT_REPO_REL, name);
  return { ok: false, error: "unknown worktree action" };
}
...
export function writeTextFile(root, rel, content, opts = {}) {
  const abs = safeResolve(root, rel);
  if (!abs) return { ok: false, error: "path escapes the Pact root" };
  ...
  try {
    ...
    // CONFLICT GUARD (data-loss fix): if the caller passed `expected` — the content the editor believes is
    // currently on disk (its last-loaded/saved baseline) — and the real disk content has DIVERGED since
    // (the agent, or another concurrent session sharing this checkout, wrote it), REFUSE to blindly
    // overwrite. Return the current bytes so the caller reconciles instead of silently destroying the
    // other writer's work. This is exactly the "uncommitted edits reverted to a stale snapshot" failure a
    // silent editor autosave used to cause in a shared worktree. `force` bypasses it after an explicit
    // user decision. `expected == null` (callers that opt out) keeps the old blind-write behavior.
    if (opts && opts.expected != null && !opts.force && existsSync(abs)) {
      const current = readFileSync(abs, "utf8");
      if (current !== String(opts.expected)) {
        return { ok: false, conflict: true, current, size: Buffer.byteLength(current, "utf8"),
          message: "File changed on disk since it was opened — NOT overwritten. Reload to take the on-disk version, or force-save to overwrite it." };
      }
    }
