...
test("the static <pre> highlighter (deleted diff lines) now colors by the SAME families via the wrapper", () => {
  // pact-highlight.js's pactHighlight() routes each word through root.pactClassifyWord (the wrapped
  // global) — so removed diff lines match the editable view exactly, not the old bands.
  assert.match(win.pactHighlight("URH_ScanAll"), /class="pk-heavy"/);
  assert.match(win.pactHighlight("URD_Old"), /class="pk-heavy"/);        // migration spelling, same family
  assert.match(win.pactHighlight("URCx_Aux"), /class="pk-readx"/);       // a new aux prefix the base alone didn't know
  assert.match(win.pactHighlight("A_Admin"), /class="pk-recipe"/);
  assert.match(win.pactHighlight("CT_Max"), /class="pk-const"/);
  assert.match(win.pactHighlight("UEV_IMC"), /class="pk-struct"/);
});
