  ...
  for (const ent of safeReaddir(dir)) {
    if (ent.isDirectory()) {
      ...
      for (const f of safeReaddir(wsDir)) {
        ...
        const realSessionId = transcript.find((r) => r && r.realSessionId)?.realSessionId || null;
 ...
 *  `_openTranscript`'s explicit-resume response seed `options.resume` with. It falls back to the
 *  file-derived lookup key for a workspace whose files predate that stamp (no crash, no migration
 *  needed) — resuming there just can't be better than the file name already allowed. */
export function listWorkspaces(dir) {
  ...
  for (const s of eachSession(dir)) {
    ...
    if ((s.updatedAt || 0) >= (entry.updatedAt || 0)) {
      ...
      entry.sessionId = s.realSessionId || s.sessionId || s.legacyKey || null;
    }
