...
import { existsSync, mkdirSync, readdirSync, readFileSync, appendFileSync, writeFileSync, statSync, rmSync } from "node:fs";
...
export function parseWorkspaceId(id) {
  const s = String(id || "");
  /* THE CONVERSATION SLOT IS NOT PART OF THE WORKTREE NAME. Multi-chat appends `#<n>` to the id
     (`Claudstermind@main#1`) to give a side conversation its own identity — and this split on the
     last `@` only, so the suffix landed in `worktree` and a **worktree called "main#1" appeared in
     the picker**, selectable, for a checkout that does not exist. Every consumer here was affected:
     history rows and summaries for a slot conversation reported that phantom worktree too.
     A real worktree name can never contain `#` (createWorktree's SAFE_NAME is letters, digits and
     `._-`), so a trailing `#<digits>` is unambiguously the slot. Returned as its own field rather
     than discarded, because "which conversation of this worktree" is a real question. */
  const hash = s.lastIndexOf("#");
  const slot = hash > -1 && /^\d+$/.test(s.slice(hash + 1)) ? Number(s.slice(hash + 1)) : 0;
  const base = slot ? s.slice(0, hash) : s;
  const at = base.lastIndexOf("@");
  if (at === -1) return { repo: base, worktree: "main", slot };
  return { repo: base.slice(0, at), worktree: base.slice(at + 1) || "main", slot };
}
...
// Directory names inside the transcript root that are NOT workspaces. `_segments` is the rolled-off
// conversation archive (lib/conversationArchive.mjs) and `_images` a blob store — both hold `.jsonl`
// / binary files that `eachSession` would otherwise enumerate as a bogus conversation. Worse: an
// archived segment's rows carry their ORIGINAL `workspaceId` stamp, and `eachSession` derives a
// directory's workspace id from `transcript[0].workspaceId` — so a root-level archive whose first
// row happens to carry that stamp gets merged straight back into the real workspace by
// `readWorkspace`, duplicating every rolled-off turn in the pane. Exact names only: a real
// workspace slug can legitimately start with `_` (e.g. the repo `_Archive/foo`).
const RESERVED_DIRS = new Set(["_segments", "_images"]);
...
// Exported so callers that need to validate a BATCH of images atomically (all-or-nothing, before
// any of them touch disk — see workspace.mjs's `_saveImages`) can check mediaType membership
// without duplicating this map or re-deriving it from saveImage's throw behavior.
export const IMAGE_EXT = { "image/png": "png", "image/jpeg": "jpg", "image/webp": "webp" };

// DOCUMENTS. The Agent SDK's user message is a Messages API MessageParam, whose content may contain
// `document` blocks as well as `image` ones — so the platform has always taken these and we simply
// never sent one. Stored exactly like images (content-addressed, deduplicated) but under `documents/`,
// because they are charged differently: an image is a fixed cost, a PDF is charged by its extracted
// pages, so a long document can fill the window on its own.
//
// PDF goes up as base64 for the API's native PDF handling. The text formats go up as `type: "text"`
// document sources, which is what the API expects for plain text rather than base64.
...
export const DOC_SOURCE_KIND = { "application/pdf": "base64" };   // everything else: "text"

/** Every media type the engine will accept as an attachment, images and documents together. */
export function attachmentKind(mediaType) {
  if (IMAGE_EXT[mediaType]) return "image";
  if (DOC_EXT[mediaType]) return "document";
  return null;
}
...
/** Save a DOCUMENT the same way — content-addressed under `<workspace>/documents/`. Returns
 *  `{ path, hash, mediaType }`. Kept separate from saveImage so the image path stays exactly as it
 *  was (its `IMAGE_PATH_RE` guard is a security boundary, not a formatting choice). */
export function saveDocument(dir, id, bytes, mediaType) {
  const ext = DOC_EXT[mediaType];
  if (!ext) throw new Error(`saveDocument: unrecognized mediaType "${mediaType}"`);
  const hash = createHash("sha256").update(bytes).digest("hex").slice(0, 24);
  const wsDir = join(dir, slugFor(id));
  const relPath = join("documents", `${hash}.${ext}`);
  const abs = join(wsDir, relPath);
  if (!existsSync(abs)) {
    mkdirSync(join(wsDir, "documents"), { recursive: true });
    writeFileSync(abs, bytes);
  }
  return { path: relPath, hash, mediaType };
}
...
// Exactly the shape saveImage() ever produces — a hash slugFor() never needs to touch (hex
// digits are already in slugFor's kept set) followed by one of the three known extensions.
const IMAGE_PATH_RE = /^images[\\/][0-9a-f]{24}\.(png|jpg|webp)$/;
const IMAGE_MIME = { png: "image/png", jpg: "image/jpeg", webp: "image/webp" };

/**
 * Resolve a saved image's absolute path + MIME type for serving, validating `relPath` strictly
 * against IMAGE_PATH_RE first — the one thing standing between "show an attached chat image" and
 * an arbitrary-file-read primitive, since `relPath` arrives from an untrusted client requesting
 * it. Nothing outside that exact shape (no `..`, no absolute path, no other extension) can
 * possibly match, so there is no separate path-traversal check to get right or forget — the
 * validation IS the containment. Returns `null` for anything that doesn't match, or a path
 * `saveImage()` never actually wrote.
 */
export function resolveImagePath(dir, id, relPath) {
  if (typeof relPath !== "string" || !IMAGE_PATH_RE.test(relPath)) return null;
  const abs = join(dir, slugFor(id), relPath);
  if (!existsSync(abs)) return null;
  const ext = relPath.slice(relPath.lastIndexOf(".") + 1);
  return { abs, mimeType: IMAGE_MIME[ext] };
}
...
/** The user's OWN first message, capped for display — with the Pact IDE's fixed auto-skill preamble
 *  ("[Pact IDE — auto-skill] …\n\n<user text>") stripped FIRST, so the saved `firstPrompt` (and any chat
 *  name a client derives from it) reflects what the user wrote, not the boilerplate. Stripping BEFORE the
 *  cap is the whole point: the preamble is far longer than the cap, so capping first would only ever keep
 *  boilerplate — the exact bug where a Pact chat showed as "[Pact IDE — auto-skill] You are working…" on a
 *  client that lacked the friendly-name overlay, while another client showed the real name. */
export function firstPromptText(text, cap = 120) {
  let s = String(text || "");
  if (/^\s*\[Pact IDE/.test(s)) { const i = s.indexOf("\n\n"); if (i >= 0) s = s.slice(i + 2).replace(/^\s+/, ""); }
  return s.slice(0, cap);
}
function summarise({ id, sessionId, realSessionId, transcript, retired, retiredAt, updatedAt, usage, legacyKey }) {
  ...
  return {
    ...
    // The real Claude SDK session id (per-session), which `_prompt`'s `resume` needs to continue a
    // specific saved conversation with full context — distinct from `sessionId` (the file/lookup key).
    realSessionId: realSessionId || null,
    repo, worktree, updatedAt: updatedAt || null, turns, usage: usage || null,
    retired, retiredAt, firstPrompt: first ? firstPromptText(first.text) : "",
  };
}
...
function* eachSession(dir, { withText = false } = {}) {
  for (const ent of safeReaddir(dir)) {
    if (ent.isDirectory()) {
      if (RESERVED_DIRS.has(ent.name)) continue;   // archive / blob store, not a conversation
...
/** Permanently remove one saved session's JSONL file (the history panel's per-row Delete). New
 *  layout only — a legacy flat file is a whole workspace, not one session, so it's never targeted
 *  here. Returns `{ ok }` / `{ ok: false, error }`; never throws. */
export function deleteSession(dir, id, sessionId) {
  if (!dir || !sessionId) return { ok: false, error: "missing session" };
  const abs = join(dir, slugFor(id), `${slugFor(sessionId)}.jsonl`);
  if (!existsSync(abs)) return { ok: false, error: "not found" };
  try { rmSync(abs); return { ok: true }; } catch (e) { return { ok: false, error: e.message }; }
}
...
export function findSession(dir, sessionKey) {
  ...
  for (const s of eachSession(dir)) {
    ...
    if (s.sessionId === sessionKey || s.legacyKey === sessionKey) {
      ...
      const { repo, worktree } = parseWorkspaceId(id);
      // `realSessionId` — Claude's OWN session id (never the file/lookup key), for resume purposes; null
      // when a session was interrupted before any real id was ever stamped. Distinct from `sessionId`
      // (the file key). This is what a scoped (Pact) auto-resume must use to continue a tab's own chat.
      return { workspaceId: id, sessionId: s.sessionId || sessionKey, realSessionId: s.realSessionId || null,
        repo, worktree, transcript: s.transcript,
