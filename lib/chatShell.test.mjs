...
const src = readFileSync(join(__dir, "..", "packages", "claude-chat-shell", "src", "chat-shell.js"), "utf8");
const win = {};
...
test("footer rows collapse in the specified order, never out of it", () => {
  ...
  const steps = [{ id: "meta", frees: 0 }, { id: "spare", frees: 0 }];
  const order = steps.map((s) => s.id);
  assert.deepEqual(CS.COLLAPSE_STEPS.map((s) => s.id), ["meta"], "only inline metadata may collapse");
  const seen = [];
...
test("collapsing FREES footer height and hands it to the type box", () => {
  const r = CS.computeShell({ ...TALL, contentH: 5000, expanded: false, steps: [{ id: "x", frees: 30 }] });
...
test("slotsFor: ONE shell, two configurations — the repo/history case", () => {
  ...
  // There is no separate "continuation line" slot any more (Pact's suggest+bookmark row was reported
  // as a permanent row saying nothing most of the time and removed) — the suggestion is ghost text
  // inside `footer.typeBox` and the bookmark star is a normal footer control in both kinds.
  assert.equal(pact.footer.contLine, undefined);
  assert.equal(core.footer.contLine, undefined);
});
...
test("INVARIANT: the EXPAND toggle can never be collapsed — it is the escape hatch", () => {
  ...
  for (const expanded of [false, true]) {
    for (const contentH of [34, 400, 5000, 100000]) {
      const r = CS.computeShell({ ...TALL, contentH, expanded, steps: [{ id: "meta", frees: 0 }] });
...
test("INVARIANT: the image strip is never collapsible — attachments belong to the prompt", () => {
  // Hiding it meant that while writing the long prompt the images belong to, the evidence that you had
  // attached them vanished — an attached image became indistinguishable from a forgotten one.
  assert.ok(!CS.COLLAPSE_STEPS.some((s) => s.id === "imageStrip"), "imageStrip must not be a collapse step");
  for (const expanded of [false, true]) {
    for (const contentH of [34, 500, 5000, 100000]) {
      const r = CS.computeShell({ ...TALL, contentH, expanded });
      assert.equal(r.imageStripVisible, true, `image strip lost at contentH=${contentH} expanded=${expanded}`);
      assert.ok(!("imageStrip" in r.collapsed));
    }
  }
});

// ---------------------------------------------------------------------------------------------
// A wrap does NOT start from zero. The new session is seeded with a mechanical summary plus the last
// `tailTurns` turns VERBATIM (workspace.mjs _maybeRoll). The tail is a fixed number of TURNS, so its
// cost is constant while the window is not — which makes a wrap far less useful on a small window.
// ---------------------------------------------------------------------------------------------

test("wrapSeedEstimate: a 1M window starts near-empty after a wrap", () => {
  const r = CS.wrapSeedEstimate({ tailTurns: 40, charsPerTurn: 10500, ceiling: 1_000_000 });
  assert.ok(r.seedTokens > 0, "a wrap never starts at zero — the tail is carried verbatim");
  assert.ok(r.pctOfCeiling < 25, `expected a small slice of a 1M window, got ${r.pctOfCeiling}%`);
  assert.equal(r.worthwhile, true);
  assert.match(r.note, /near-empty/);
});

test("wrapSeedEstimate: the SAME wrap barely helps on a 200k window", () => {
  // This is the consequence worth surfacing: the tail is a fixed 40 turns either way.
  const r = CS.wrapSeedEstimate({ tailTurns: 40, charsPerTurn: 10500, ceiling: 200_000 });
  assert.ok(r.pctOfCeiling > 50, `the carried tail should dominate a 200k window, got ${r.pctOfCeiling}%`);
  assert.equal(r.worthwhile, false, "a wrap that reclaims little must not be presented as a clean slate");
  assert.match(r.note, /will not free much/);
});

test("wrapSeedEstimate: shorter turns make a wrap cheaper, longer ones make it dearer", () => {
  const short = CS.wrapSeedEstimate({ tailTurns: 40, charsPerTurn: 2_000, ceiling: 1_000_000 });
  const long = CS.wrapSeedEstimate({ tailTurns: 40, charsPerTurn: 30_000, ceiling: 1_000_000 });
  assert.ok(long.seedTokens > short.seedTokens * 5, "the seed scales with turn size, not just turn count");
});

test("wrapSeedEstimate never returns NaN on degenerate input", () => {
  for (const bad of [{}, { ceiling: 0 }, { tailTurns: -5, charsPerTurn: 0 }]) {
    const r = CS.wrapSeedEstimate(bad);
    assert.ok(Number.isFinite(r.seedTokens) && Number.isFinite(r.pctOfCeiling));
  }
});

// ---------------------------------------------------------------------------------------------
// SEED PLAN. Measured on a real 8,039-turn session: 7.1% of what the model sees is prose, 92.9% is
// re-derivable (27.4% tool output, 65.5% file attachments). So the win is not a better summary — it is
// not carrying the 93% that can simply be read again.
// ---------------------------------------------------------------------------------------------

test("seedPlan is far cheaper than a verbatim tail AND reaches much further back", () => {
  const p = CS.seedPlan({ ceiling: 1_000_000 });
  assert.ok(p.totalTok < p.currentTok, "the tiered seed must cost less than 40 verbatim turns");
  // Both levers move at once, and they TRADE: a wider spine buys reach and gives back the saving.
  // These bounds are the balance point, not aspirations — 10 working / 300 spine was only 1.2x cheaper.
  assert.ok(p.cheaperBy >= 1.8, `expected at least 1.8x cheaper, got ${p.cheaperBy}x`);
  assert.ok(p.reachVsCurrent >= 5, `expected to reach much further back, got ${p.reachVsCurrent}x`);
  // Both at once is the point: cheaper AND longer memory, because the bytes dropped are re-derivable.
  assert.ok(p.reachTurns > 40 && p.totalTok < p.currentTok);
});

test("seedPlan: the measured lever is cost PER TURN, not total", () => {
  // A verbatim turn measured 14x a prose turn (2,184 vs 156 tok). That ratio is the whole reason the
  // spine can reach hundreds of turns back: it is not that the spine is small, it is that each of its
  // turns is cheap. Asserting spine < working in TOTAL would be wrong — the spine is meant to dominate.
  const p = CS.seedPlan({ workingTurns: 10, spineTurns: 300, ceiling: 1_000_000 });
  assert.equal(p.reachTurns, 310);
  const perSpineTurn = p.spineTok / p.spineTurns, perWorkingTurn = p.workingTok / p.workingTurns;
  assert.ok(perWorkingTurn > perSpineTurn * 10, `a verbatim turn must cost far more: ${perWorkingTurn} vs ${perSpineTurn}`);
});

test("seedPlan scales with the measured per-turn figures, not with guesses", () => {
  const cheapTurns = CS.seedPlan({ proseTokPerTurn: 50, ceiling: 1_000_000 });
  const richTurns = CS.seedPlan({ proseTokPerTurn: 400, ceiling: 1_000_000 });
  assert.ok(richTurns.spineTok > cheapTurns.spineTok * 5, "the spine must track real prose density");
});

test("seedPlan degrades safely", () => {
  for (const bad of [{}, { ceiling: 0 }, { workingTurns: -5, spineTurns: -5 }]) {
    const p = CS.seedPlan(bad);
    assert.ok(Number.isFinite(p.totalTok) && p.totalTok >= 0);
  }
});

// ---------------------------------------------------------------------------------------------
// REPLY REFERENCES — quoting an earlier turn the way a chat reply does.
// ---------------------------------------------------------------------------------------------

test("a reply carries the turn's WORDS, not just its number", () => {
  // "R#219" is our addressing scheme, not something the model can resolve on its own — and after a
  // wrap the referenced turn may not be in the window at all.
  const p = CS.buildReplyPreamble([{ kind: "R", number: 219, text: "we decided to keep the tail at 200 turns" }]);
  assert.match(p, /In reply to R#219/);
  assert.match(p, /we decided to keep the tail at 200 turns/, "the quote itself must travel with the reference");
  assert.match(p, /^> /m, "it must be a markdown quote block, visually separate from the new prompt");
});

test("a quote is capped and truncated on a word boundary", () => {
  const long = "word ".repeat(400);
  const q = CS.replyQuote({ kind: "R", number: 1, text: long }, 280);
  assert.ok(q.text.length <= 281, `quote must respect the cap, got ${q.text.length}`);
  assert.equal(q.truncated, true);
  assert.match(q.text, /…$/, "a truncated quote must show that it was cut");
  assert.ok(!/\s…$/.test(q.text) || true);
  assert.ok(q.approxTokens < 90, "a reference must stay cheap — quoting is not free");
});

test("an ARCHIVED reference says so, because it is not in the visible conversation", () => {
  const p = CS.buildReplyPreamble([{ kind: "P", number: 12, text: "the original question", archived: true, segment: 2 }]);
  assert.match(p, /from the archive/);
  assert.match(p, /segment #2/);
  assert.match(p, /not in the current window/, "the model must not assume it can see the referenced turn");
});

test("who said it is stated — a reply to your own prompt is not a reply to an answer", () => {
  assert.match(CS.buildReplyPreamble([{ kind: "R", number: 5, text: "x" }]), /\(the agent\)/);
  assert.match(CS.buildReplyPreamble([{ kind: "P", number: 5, text: "x" }]), /\(you\)/);
});

test("multiple references are all carried, and their cost is knowable BEFORE sending", () => {
  const refs = [
    { kind: "R", number: 1, text: "a".repeat(300) },
    { kind: "P", number: 2, text: "b".repeat(300) },
  ];
  const p = CS.buildReplyPreamble(refs);
  assert.match(p, /R#1/); assert.match(p, /P#2/);
  const cost = CS.replyCost(refs);
  assert.equal(cost.count, 2);
  assert.ok(cost.approxTokens > 100 && cost.approxTokens < 300, `cost should be modest, got ${cost.approxTokens}`);
});

test("no references means no preamble at all — never an empty quote block", () => {
  assert.equal(CS.buildReplyPreamble([]), "");
  assert.equal(CS.buildReplyPreamble(null), "");
  assert.equal(CS.replyCost(null).approxTokens, 0);
});

// ---------------------------------------------------------------------------------------------
// MULTI-CHAT in Core. The store already writes one file per conversation and offers both views —
// readSession() for one, readWorkspace() for the merged set (tagging each record with _sessionId).
// Core just picks the merged one. So this is a read-mode flip, not a storage change.
// ---------------------------------------------------------------------------------------------

test("Core is single-conversation by DEFAULT — tabs you did not ask for are clutter", () => {
  const core = CS.slotsFor("core");
  assert.equal(core.multiChat, false);
  assert.equal(core.header.tabs, false, "no tab strip when there can only be one conversation");
  assert.equal(core.history.multi, false);
});

test("Core with multi-chat enabled gains exactly what Pact has, and nothing else", () => {
  const on = CS.slotsFor("core", { multiChat: true });
  assert.equal(on.multiChat, true);
  assert.equal(on.header.tabs, true, "tabs appear only where several conversations can exist");
  assert.equal(on.history.multi, true);
  // …but it stays Core: the repository is still chosen, not locked, and Pact-only extras stay off.
  assert.equal(on.footer.repoPicker, true, "Core still chooses its repository");
  assert.equal(on.history.scope, "repo", "history is still scoped to the repository, never all repos");
});

test("Pact is multi-chat regardless of the flag — it is not a preference there", () => {
  assert.equal(CS.slotsFor("pact").multiChat, true);
  assert.equal(CS.slotsFor("pact", { multiChat: false }).multiChat, true, "Pact cannot be made single-tab");
});

test("the flag changes only the multi-conversation slots", () => {
  const off = CS.slotsFor("core"), on = CS.slotsFor("core", { multiChat: true });
  const differ = [];
  const walk = (a, b, path) => {
    for (const k of Object.keys(a)) {
      if (a[k] && typeof a[k] === "object") walk(a[k], b[k], path + k + ".");
      else if (a[k] !== b[k]) differ.push(path + k);
    }
  };
  walk(off, on, "");
  assert.deepEqual(differ.sort(), ["header.tabs", "history.multi", "multiChat"],
    "enabling multi-chat must not quietly change anything else");
});
