...
test("the percentage is of CORE, not the viewport — the old Core/Pact divergence", () => {
  ...
  // 40% of Core (650) = 260 — independent of any window/viewport figure
  assert.equal(CS.swallowCap({ ...TALL, expanded: false }), 260);
  // toggled: the box may take ALL of Core
  assert.equal(CS.swallowCap({ ...TALL, expanded: true }), 650);
});
...
test("a short window does NOT read as broken: the row floor beats the fraction", () => {
  ...
  assert.ok(cap > 0.40 * 90, "the fraction must not be allowed to produce a stunted box");
  assert.equal(cap, 90, "…but it can never exceed Core, or Core would go negative");
...
test("footer rows collapse in the specified order, never out of it", () => {
  const order = CS.COLLAPSE_STEPS.map((s) => s.id);
  assert.deepEqual(order, ["modelBar", "contLine", "meta", "attach"]);
  const seen = [];
  for (let h = 34; h <= 900; h += 6) {
    const r = CS.computeShell({ ...TALL, contentH: h, expanded: false });
