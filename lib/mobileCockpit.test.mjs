...
const DEMO = {
  ...
  conversations: [
    ...
    { id: "c3", name: "OuronetUI · dev", repo: "OuroborosNetwork/daimons/OuronetUI", worktree: "dev", sub: "2h ago", star: true },
  ],
  repos: [
  ...
  marks: [{ at: "R6992", note: "the wheel decision" }, { at: "P729", note: "left/right reveal brief" }],
...
test("the two side risers are the two selectors: which chat, and which mark inside it", () => {
  const { host, fired } = mounted();
  ...
  assert.ok(has(sheet, "Claudstermind · Main") && has(sheet, "OuronetUI · dev"),
    "the Chats riser selects between whole chats, across repositories — that is what makes it different from the left pane's list");
  findAllIn(sheet, ".mc-btn").find((b) => has(b, "New chat")).fire("click");
  assert.equal(fired("newConversation").length, 1, "offering a new chat that starts nothing is worse than not offering one");
  tabs[1].fire("click");
  sheet = findIn(host, ".mc-sheet");
  assert.ok(has(sheet, "the wheel decision"), "a mark carries the note you wrote, or it is an address you cannot recognise");
  assert.ok(/R6992|P729/.test(sheet.textContent), "…against the turn it was made ON, which is what makes it a mark and not a bookmark");
});
