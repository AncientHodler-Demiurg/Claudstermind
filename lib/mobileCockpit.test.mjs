...
const CALLBACKS = ["send", "stop", "autoContinue", "attach", "input", "model", "effort", "permission",
  ...
  "pickChat", "newChat", "pickMark", "wakeLock", "autoGrant"];
...
test("the keep-awake switch shows whether the screen is ACTUALLY being held", () => {
  // "Keeping screen on doesn't work on mobile, still lights off." The request can be refused — battery
  // saver, a policy, a missing user gesture — and the original swallowed that: `catch { … }` left the
  // switch reading ON with nothing held and nothing said, which is indistinguishable from a feature
  // that does not work. The state carries the WISH, whether it is HELD, and why not.
  const { host, view, fired } = mounted();
  const awake = () => findAllIn(host, ".mc-tog").find((t) => /awake/i.test(t.textContent));
  const open = () => findIn(host, ".mc-running").fire("click");

  view.setState({ wakeLock: null });
  open();
  assert.ok(!awake(), "a browser without the API must be offered no switch at all");

  view.setState({ wakeLock: { on: false, held: false, err: "" } });
  open();
  assert.ok(awake(), "where it is supported the switch must be there");
  awake().fire("click");
  assert.deepEqual(fired("wakeLock").map((c) => c[1]), [true], "tapping asks the host to turn it on");

  view.setState({ wakeLock: { on: true, held: true, err: "" } });
  open();
  assert.match(awake().className, /\bon\b/, "held: the switch reads on and there is nothing to explain");
  assert.ok(!findAllIn(host, ".mc-row").some((r) => /not holding/i.test(r.textContent)),
    "no warning while it is actually working");

  view.setState({ wakeLock: { on: true, held: false, err: "battery saver" } });
  open();
  assert.match(awake().className, /\bon\b/,
    "the switch still reflects what you asked for — flipping it back would look like the tap failed");
  const warn = findAllIn(host, ".mc-row").find((r) => /not holding/i.test(r.textContent));
  assert.ok(warn, "asked for but not held is the one state that must say so out loud");
  assert.match(warn.textContent, /battery saver/, "…and give the reason the browser gave");
});

test("while the loop is armed the field ghosts the text it is about to send", () => {
  // "When auto is on the field should be populated with the recommended message in ghost form, and
  // after the countdown expires it should be released." A send that happens on its own should never
  // be a surprise — you can read it before it goes.
  const { host, view } = mounted();
  const box = findIn(host, ".mc-box");
  const resting = box.attrs.placeholder || box.placeholder;
  view.setState({ autoNext: "Continue with DPMF." });
  assert.equal(box.placeholder, "Continue with DPMF.", "the ghost must show what is actually queued to send");
  assert.match(box.className, /--ghost/, "…and read as a pending send, not as an invitation to type");
  view.setState({ autoNext: "" });
  assert.equal(box.placeholder, resting, "with nothing armed the box goes back to inviting you to write");
  assert.ok(!/--ghost/.test(box.className), "a stale ghost claims a send that is not coming");
});

test("the ghost is never the box's VALUE, because a value would pause the loop it advertises", () => {
  // `composing` — any text in the compose box — is precisely the state that suspends auto-continue
  // (your half-typed message outranks the robot). Writing the suggestion into the field would stop
  // the countdown it is advertising, and the loop would sit there forever showing what it means to
  // send. The placeholder is the only way to show it without becoming it.
  const { host, view } = mounted();
  const box = findIn(host, ".mc-box");
  view.setState({ autoNext: "Continue where you left off." });
  assert.equal(box.value, "", "the field must stay empty, or the loop pauses on its own suggestion");
});

test("the bulb follows whichever element is really scrolling, not the node it was handed", () => {
  // "Live and held medallion don't work properly when scrolling, morphing between states."
  // `opts.transcript` is a WRAPPER in both workspaces — Pact's `.pc-scroll` and Core's stick wrapper
  // each keep the real scroller inside — and scroll events DO NOT BUBBLE. Measured with the debugger:
  // the adopted node carried the cockpit's scroll listeners and `overflow: hidden`, while the element
  // that actually scrolled (30,448px of content in 490px) carried only the host's own. So the bulb
  // changed only when the HOST repainted: constant during a streaming turn, which is why it seemed to
  // work, and dead the moment the conversation went idle.
  const adopted = makeNode("div");
  const inner = makeNode("div");                 // the host's real scroller, inside its wrapper
  adopted.appendChild(inner);
  adopted.scrollHeight = 100; adopted.clientHeight = 100;      // the wrapper never scrolls
  const { host, view } = mounted(DEMO, { transcript: adopted });
  view.setState({ stick: true });
  const bulb = () => findIn(host, ".mc-med").textContent;
  assert.match(bulb(), /Live/, "pinned to the bottom reads Live");

  inner.scrollHeight = 4000; inner.clientHeight = 500; inner.scrollTop = 0;   // scrolled far up
  adopted.fire("scroll", { target: inner });
  assert.match(bulb(), /Held/,
    "a scroll of the host's own scroller must reach the bulb — reading the wrapper's metrics instead " +
    "reports a node that never moves, and the bulb never morphs");

  inner.scrollTop = 3500;                                                     // back to the bottom
  adopted.fire("scroll", { target: inner });
  assert.match(bulb(), /Live/, "and back again, or Held is a state you can enter but not leave");
});

test("the scroll watcher is registered in the capture phase", () => {
  // Non-bubbling events still travel DOWN through the capture phase, so one listener on the adopted
  // node sees whichever descendant scrolls. Without capture this listener is attached to a node that
  // cannot scroll and is simply never called.
  assert.match(src, /addEventListener\("scroll", onCoreScroll, true\)/,
    "a bubble-phase listener on the wrapper never fires, because scroll does not bubble");
});

test("leaving hands the transcript back without the listener we put on it", () => {
  // The transcript is the HOST's element. renderStage() remounts on every chat switch, so a watcher
  // left behind outlives its cockpit: measured before this, two capture-phase scroll listeners on one
  // adopted node with a single cockpit on screen, growing with every remount, all of them running on
  // every scroll and painting bulbs nobody can see.
  const adopted = makeNode("div");
  const { view } = mounted(DEMO, { transcript: adopted });
  assert.ok((adopted._listeners.scroll || []).length >= 1, "the cockpit must be watching while mounted");
  view.destroy();
  assert.equal((adopted._listeners.scroll || []).length, 0,
    "a listener on someone else's node must come off with us, or it accumulates for the life of the page");
});

test("the context page's headline is the authoritative number, not the sum of its rows", () => {
  // "Context is reported incorrectly by the lateral pane on both core and pact space." The page
  // derived its total from `parts`, and NEITHER host ever published parts — both sent only
  // { tokens, ceiling }. So it read "0 of 1,000,000 tokens · 0%" with a single "Free · 100%" row over
  // a window that was two thirds full, in both workspaces. Two sources for one number, one of them
  // never fed.
  const { host, view } = mounted();
  view.setState({ context: { tokens: 640835, ceiling: 1000000, parts: [] } });
  findIn(host, ".mc-ctx").fire("click");
  const over = findIn(host, ".mc-over");
  assert.ok(over, "the medallion must open the context page");
  assert.ok(has(over, "640,835 of 1,000,000"),
    "the headline must report what the host says is used, even with no breakdown to show");
  assert.ok(!has(over, "0 of 1,000,000"), "a full window must never be reported as empty");
});

test("the rows and the free space can never add up to more than the window", () => {
  // The engine's categories include things its own total leaves out — deferred tools, the autocompact
  // buffer — so the parts legitimately sum to MORE than `tokens` (measured: 689,091 of parts against
  // a reported 640,835). Free is measured against whichever is larger; taking it from `tokens` alone
  // would hand out space the categories have already claimed.
  const { host, view } = mounted();
  view.setState({ context: { tokens: 100, ceiling: 1000, parts: [
    { name: "Messages", tokens: 400 }, { name: "Autocompact buffer", tokens: 200 }] } });
  findIn(host, ".mc-ctx").fire("click");
  const rows = findAllIn(findIn(host, ".mc-over"), ".mc-ctxrow").map((r) => r.textContent);
  const free = rows.find((r) => /Free/.test(r));
  assert.ok(free, "the free row must exist — a breakdown that omits what is left does not add up");
  assert.ok(/400/.test(free), "free must be the ceiling minus the LARGER of the parts and the total (1000-600)");
});

test("no reading is not zero", () => {
  // A pane with no session has ceiling 0, which clamped to 1 and rendered "0 of 1 tokens · 0%" plus a
  // "Free · 100%" row: a confident statement about a window nobody has measured. The rest of this
  // product keeps unavailable and 0% apart deliberately.
  const { host, view } = mounted();
  view.setState({ context: { tokens: 0, ceiling: 0, parts: [] } });
  findIn(host, ".mc-ctx").fire("click");
  const over = findIn(host, ".mc-over");
  assert.ok(has(over, "No reading yet"), "an unmeasured window must say so");
  assert.ok(!has(over, "of 1 tokens"), "a ceiling of 1 token is an artefact of clamping, not a window");
  assert.equal(findAllIn(over, ".mc-ctxrow").length, 0, "there is nothing truthful to list");
});
...
test("a riser width that could not be measured is never written, and is retried", () => {
  // "Changing chat box via the stripe bar is broken, first of all, it gets shrunken." An element that
  // is not laid out reports offsetWidth 0 — and the cockpit paints while DETACHED now, with this
  // running a frame later. Both tabs were pinned to `width: 0px` and their labels spilled out of a
  // tab with no tab left around them. Worse, it STUCK: the caller had already recorded those labels
  // as fitted, so nothing re-measured until a label's text changed. Same rule as the type box —
  // never write a measurement you could not take — plus: forget the record, so the next paint retries.
  const { host, view } = mounted();
  const tabs = findAllIn(host, ".mc-riser");
  const pin = (w) => tabs.forEach((t) => Object.defineProperty(t, "offsetWidth", { configurable: true, get: () => w }));

  pin(0);
  view.setState({ chats: [{ id: 1, name: "A" }, { id: 2, name: "B" }] });
  assert.notEqual(tabs[0].style.width, "0px", "a collapsed tab is a control you cannot press");
  assert.notEqual(tabs[1].style.width, "0px");

  // …and the failure must not be remembered as success. Asserted on the source because the DOM shim
  // derives offsetWidth from its own text and will not pretend to be unlaid-out for a second pass.
  const at = src.indexOf("function fitRisers");
  const body = src.slice(at, src.indexOf("\n    }", at));
  assert.match(body, /if \(!w\) \{ lastFit = null; return; \}/,
    "forgetting the record is what makes the next paint retry — without it the collapse outlives " +
    "whatever caused it, until a label's text happens to change");
});

test("the risers cannot crowd the readout off the screen", () => {
  // Two fixed tabs plus a middle that will not shrink below its text is a row wider than the phone —
  // which is how both ends end up off-screen, one clipped at each edge.
  const { host, view } = mounted();
  const tabs = findAllIn(host, ".mc-riser");
  tabs.forEach((t) => Object.defineProperty(t, "offsetWidth", { configurable: true, get: () => 9999 }));
  view.setState({ chats: [{ id: 1, name: "an absurdly long conversation name" }] });
  const w = parseInt(tabs[0].style.width, 10);
  assert.ok(w > 0 && w < 412 * 0.5,
    `one tab may not take half the row (got ${w}px) — the readout has to keep somewhere to live`);
});

test("at the ceiling a Continue appears beside the switch, and only there", () => {
  // "Once 10 steps are reached I need a sort of continue button that is exposed attached to the
  // toggle." The ceiling deliberately stops an unattended loop and waits for a person — but the only
  // way to grant the next batch was to tick the switch OFF and back ON, which reads as "turn off the
  // thing I want to continue" and at a glance looks like a switch that has stopped working.
  const { host, view, fired } = mounted();
  const more = () => findIn(host, ".mc-auto-more");
  assert.ok(more(), "the control must exist so it can be revealed without a rebuild");

  view.setState({ running: { auto: { on: true, n: 3, max: 10, in: 5 } } });
  assert.equal(more().hidden, true, "mid-batch there is nothing to grant — the loop is already running");

  view.setState({ running: { auto: { on: true, n: 10, max: 10, in: null } } });
  assert.equal(more().hidden, false, "at the ceiling it is the only thing the loop is waiting for");
  more().fire("click");
  assert.equal(fired("autoGrant").length, 1, "…and it asks the host to grant the next batch");

  view.setState({ running: { auto: { on: false, n: 10, max: 10, in: null } } });
  assert.equal(more().hidden, true, "switched off, there is no loop to continue");
});

test("the grant arrow does not push Send off the phone", () => {
  // Measured at 412px: with the arrow shown the button row came to 415px and overflowed — and what
  // falls off the end is Send. "auto" is the most expendable thing in the pill at that moment: the
  // counter reads 10/10 and there is an arrow beside it, which says what this is better than the word.
  const { host, view } = mounted();
  const lbl = () => findIn(host, ".mc-auto-lbl");
  view.setState({ running: { auto: { on: true, n: 3, max: 10, in: 5 } } });
  assert.notEqual(lbl().style.display, "none", "mid-batch there is room for the word");
  view.setState({ running: { auto: { on: true, n: 10, max: 10, in: null } } });
  assert.equal(lbl().style.display, "none", "at the ceiling the arrow takes its place");
});

test("tapping the grant answers immediately, before the engine has", () => {
  // The grant is a round trip, and what follows it is an EIGHT SECOND countdown before the next round
  // goes out — so pressing it looked like nothing happened at all, for about ten seconds, and then
  // work simply began. The host still owns the truth; this only says "heard you" until it arrives.
  const { host, view } = mounted();
  view.setState({ running: { auto: { on: true, n: 10, max: 10, in: null } } });
  const more = findIn(host, ".mc-auto-more");
  more.fire("click");
  assert.match(more.className, /--sent/, "the press must leave a visible mark at once");
  assert.match(findIn(host, ".mc-auto-n").textContent, /granting/i,
    "…and say what it is waiting for, rather than showing a stale 10/10");
  view.setState({ running: { auto: { on: true, n: 0, max: 10, in: 8 } } });
  assert.ok(!/--sent/.test(findIn(host, ".mc-auto-more").className),
    "and stop saying it the moment the real numbers arrive");
});
