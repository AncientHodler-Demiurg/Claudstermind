...
test("the keep-awake switch is offered only where the browser can honour it", () => {
  // "Can we prevent screen going off when the chat page is on? Like how the gps program stays always
  // on." Yes — the Screen Wake Lock API. But not every browser has it, and a switch that silently
  // does nothing is worse than no switch: `null` means no such capability, which is NOT "off".
  ...
  view.setState({ wakeLock: false });
  findIn(host, ".mc-running").fire("click");
  const off = awake();
  assert.ok(off, "where it is supported the switch must be there");
  assert.ok(!/\bon\b/.test(off.className), "…reading off until it is turned on");
  off.fire("click");
  assert.deepEqual(fired("wakeLock").map((c) => c[1]), [true], "tapping asks the host to turn it on");

  view.setState({ wakeLock: true });
  findIn(host, ".mc-running").fire("click");
  assert.match(awake().className, /\bon\b/, "and it shows what the host says is true, not what was tapped");
});
