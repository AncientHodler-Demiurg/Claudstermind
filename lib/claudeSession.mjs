...
// A message's `content` is EITHER a plain string OR an array of blocks — the Messages API allows both,
// and which one you get varies by model and by turn. A bare `content || []` does NOT save you: a string is
// truthy, so it sails through and then `.filter` throws "parts.filter is not a function", killing the whole
// message-translation path for that turn. That is a real bug we shipped (fixed 1.5.97). Normalise ONCE here
// so every consumer sees an array; a string becomes a single synthetic text block, which is exactly what the
// API means by it. Anything else (null/number/object) degrades to [] rather than throwing.
...
export function toEvent(msg) {
  ...
  if (msg.type === "assistant") {
    const parts = contentBlocks(msg.message?.content);
  ...
  if (msg.type === "result") {
    // `errors` is the SDK's own reason a turn failed, and it was being dropped here — so a turn that
    // died carried `isError: true` and nothing that could explain WHY, to anyone. That is how a
    // conversation whose SDK session file had been removed ended up looking merely "stuck": the SDK
    // said `No conversation found with session ID: …`, the flag survived, the sentence did not.
    const errs = Array.isArray(msg.errors)
      ? msg.errors.map((e) => (typeof e === "string" ? e : (e && (e.message || e.error)) || "")).filter(Boolean)
      : [];
    return { kind: "result", subtype: msg.subtype, isError: !!msg.is_error, errors: errs,
...
export class ClaudeSession {
  constructor(opts) {
    ...
    // Auto-wrap: whether WorkspaceManager._maybeRoll() is allowed to fire an automatic roll on this
    // session. Defaults ON (the automatic threshold has always existed and applied to everyone) —
    // this only lets the web opt a session OUT, via the auto-wrap checkbox added alongside the manual
    // Wrap control. See setAutoWrap() below and workspace.mjs's _maybeRoll().
    this.autoWrap = opts.autoWrap === false ? false : true;
  ...
  /** Toggle whether an automatic roll (workspace.mjs's _maybeRoll) may fire on this session at all.
   *  Unlike mode/model/effort/fastMode this has no SDK-side counterpart to apply — it's purely a gate
   *  _maybeRoll checks before calling shouldRoll(), so flipping it takes effect on the NEXT turn's
   *  automatic-roll check with no query()/applyFlagSettings round trip needed. A MANUAL wrap (see
   *  manualRoll()) is untouched by this flag either way — it's an explicit user action, not the
   *  threshold this flag gates. */
  setAutoWrap(enabled) {
    const next = enabled !== false;
    if (next === this.autoWrap) return this.autoWrap;
    this.autoWrap = next;
    this._emit({ kind: "autoWrap", autoWrap: this.autoWrap });
  ...
  async *_input(gen = this._gen) {
    ...
    while (!this._ended && this._gen === gen) {
      while (this._inbox.length && this._gen === gen) {
        ...
        // Attachments ride as Messages API content blocks. Images become `image` blocks; documents
        // become `document` blocks, which the API has always accepted (the SDK types a user message as
        // a MessageParam whose content may be "text, image, document, tool_result, …") and which we
        // simply never emitted. A PDF goes up base64 for native PDF handling; text-ish formats go up as
        // a `text` source, which is what the API expects for them.
        const blocks = [];
        for (const a of (images || [])) {
          if (!a) continue;
          if (a.kind === "document") {
            const src = a.sourceKind === "base64"
              ? { type: "base64", media_type: a.mediaType, data: a.base64Data }
              : { type: "text", media_type: a.mediaType || "text/plain", data: a.textData };
            const blk = { type: "document", source: src };
            if (a.title) blk.title = a.title;
            blocks.push(blk);
          } else {
            blocks.push({ type: "image", source: { type: "base64", media_type: a.mediaType, data: a.base64Data } });
          }
        }
        const content = blocks.length ? [{ type: "text", text }, ...blocks] : text;
  ...
  _requestRespawn() {
    ...
    // BOTH of these are ASYNC — they return promises. A bare try/catch only catches a SYNCHRONOUS throw, so
    // when the subprocess transport is already down (the common case during a roll: the query stream was just
    // ended on the line above, or the turn already finished) the promise REJECTS with "ProcessTransport is not
    // ready for writing" and the rejection escapes as an unhandledRejection. sessiond's keep-alive net caught
    // it, so nothing crashed — but it fired on every roll and on every lane-switch respawn, buried the log, and
    // would kill any process without that net. Swallow the rejection too: a dead transport is exactly the state
    // we are trying to reach, so failing to interrupt it is a success, not an error.
    try { Promise.resolve(this._q?.return?.()).catch(() => {}); } catch {}      // end the current SDK query stream…
    try { Promise.resolve(this._q?.interrupt?.()).catch(() => {}); } catch {}   // …and abort any in-flight turn on it
  ...
  _buildOptions() {
    ...
    const options = {
      ...
      /* THE PLUGINS CLAUDSTERMIND SHIPS WITH — bee, wasp, nectar, vendored under plugins/ and passed
         to every session. They used to be a per-MACHINE setting: on the work machine only nectar was
         enabled, so the same prompt behaved differently depending on which box answered it, and a
         fresh clone had none of them. Shipping them means a conversation gets the same methodology
         everywhere, with nothing to install and no marketplace to reach. See lib/vendoredPlugins.mjs. */
      plugins: vendoredPluginOptions(),
      /* GATE AGAINST A PROCESS THAT CAN NEVER TELL YOU IT'S STUCK — see lib/bashPollGuard.mjs. Eleven
         background bash jobs in one Pact session ended up polling files that had stopped changing
         hours earlier, each one an `until <condition>; do sleep N; done` loop with no ceiling of its
         own — the `timeout` on the thing it was waiting FOR never bounded the WAITING itself. Once a
         loop like that is running, nothing in this system can un-stick it; the only place to fix it is
         before it starts. Denied here, with a reason the model can act on in its very next call. */
      hooks: { PreToolUse: [{ matcher: "Bash", hooks: [async (input) => bashPollGuardHook(input && input.tool_name, input && input.tool_input) || {}] }] },
    };
  ...
  async _runQueryOnce(gen) {
    ...
    try {
      ...
      for await (const msg of this._q) {
        // toEvent() is a PURE translator, but it runs on whatever shape the SDK/model hands us, and
        // shapes vary by model and by turn. Before 1.5.97 a throw in here escaped straight to the catch
        // below, which set status "error" and ENDED THE WHOLE TURN — one unexpected message shape killed
        // a live conversation (that was the real "parts.filter is not a function" incident). A malformed
        // or novel message is not a reason to destroy a turn that is otherwise working. Skip the message,
        // surface it LOUDLY as its own event so it is never silently swallowed, and keep streaming.
        let ev;
        try {
          ev = toEvent(msg);
        } catch (e) {
          this._emit({ kind: "warning", scope: "translate",
            message: `Skipped one malformed engine message (${String(e && e.message || e)})` });
          continue;
        }
        if (!ev) continue;
  ...
  async stop() {
    ...
    // NOT awaited on purpose (stop() must not block on a subprocess that may never answer), which is exactly
    // why the rejection has to be caught explicitly — same async-vs-sync try/catch trap as _requestRespawn.
    try { Promise.resolve(this._q?.interrupt?.()).catch(() => {}); } catch {}
    this.status = "ended";
  }
