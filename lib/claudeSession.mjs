...
export class ClaudeSession {
  ...
  async *_input() {
    ...
    while (!this._ended) {
      while (this._inbox.length) {
        const { text, image } = this._inbox.shift();
        ...
        // before image support existed); a two-part array only when an image rides along.
        const content = image
          ? [{ type: "text", text }, { type: "image", source: { type: "base64", media_type: image.mediaType, data: image.base64Data } }]
          : text;
  ...
  /** Queue a follow-up prompt. `image`, if given, is `{ mediaType, base64Data }` and rides
   *  alongside the text as a second SDK content block (Anthropic vision input) — omit it
   *  (or pass undefined) for the plain-string behavior every existing caller relies on. */
  prompt(text, image) {
    this._inbox.push({ text, image });
  ...
  async start() {
    ...
    try {
      ...
      for await (const msg of this._q) {
        ...
        // only outgoing prompts.
        if (this.status === "idle" && ev.kind !== "result") { this.status = "thinking"; this._emit({ kind: "status", status: this.status }); }
