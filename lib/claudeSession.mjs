...
export function toEvent(msg) {
  ...
  if (msg.type === "system" && msg.subtype === "task_notification") {
    return { kind: "taskDone", id: msg.task_id, status: msg.status, summary: msg.summary || "", skipTranscript: !!msg.skip_transcript };
  }
  return null;
}
...
export class ClaudeSession {
  ...
  setModel(model) {
    ...
    this.model = next;
    if (this._started && !this._ended) Promise.resolve(this._q?.setModel?.(next)).catch(() => {});
  ...
  async start() {
    ...
    try {
      ...
      for await (const msg of this._q) {
        ...
        if (this.status === "idle" && !BG_KINDS.has(ev.kind) && ev.kind !== "result" && ev.kind !== "init") { this.status = "deepwork"; this._emit({ kind: "status", status: this.status }); }
