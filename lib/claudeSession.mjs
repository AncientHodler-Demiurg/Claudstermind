...
/** Strip the vars a nested Claude Code context injects, so the spawn authenticates
 *  with the headless subscription token instead of expecting host auth refresh. */
export function cleanClaudeEnv(base) {
  const env = { ...base };
  for (const k of Object.keys(env)) {
    if (k === "CLAUDECODE" || k === "AI_AGENT" || k === "BAGGAGE") { delete env[k]; continue; }
    if (/^CLAUDE_CODE_/i.test(k) && k !== "CLAUDE_CODE_OAUTH_TOKEN") delete env[k];
  }
  return env;
}
...
export class ClaudeSession {
  ...
  async start() {
    ...
    this.status = "thinking"; this._emit({ kind: "status", status: this.status });
    try {
      this._q = this.sdkQuery({ prompt: this._input(), options });
