...
export function writeRelayConfig(dataDir, patch = {}) {
  ...
  // Read-merge, but don't merge a partial patch onto DEFAULTS when the existing file is CORRUPT — that would drop
  // the configured url when only `enabled` is patched (and vice-versa). Missing = fresh (DEFAULTS ok); corrupt →
  // back up + throw so the caller reports an error instead of losing the other field.
  let cur = { ...DEFAULTS }, raw = null;
  try { raw = readFileSync(path, "utf8"); } catch (e) { if (e.code !== "ENOENT") throw e; }
  if (raw != null) {
    try { const o = JSON.parse(raw); cur = { enabled: Boolean(o.enabled), url: typeof o.url === "string" ? o.url : "" }; }
    catch { try { writeFileSync(path + ".corrupt.bak", raw); } catch {} throw new Error("relay config is corrupt — refusing to overwrite (backed up to .corrupt.bak)"); }
  }
  const next = { ...cur };
