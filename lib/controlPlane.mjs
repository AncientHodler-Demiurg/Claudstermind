...
export const UNITS = Object.freeze([
  ...
  Object.freeze({
    id: "omniroute", unit: "omniroute.service", label: "OmniRoute gateway",
    blurb: "OPTIONAL model gateway on :20128 (OpenRouter/Groq/your Claude abo via one key). Not needed for Claude — its dot is the /api/health probe (green = online AND serving).",
    critical: false,
  }),
]);
...
/** Turn the dashboard's /api/relay JSON into a tunnel probe result (pure — unit-testable, no network).
 *  `disabled:true` means the bridge is off in config (grey "off", NOT a failure); otherwise ok===connected. */
export function interpretRelay(r) {
  if (!r || typeof r !== "object") return { ok: false, error: "no-data" };
  if (!r.enabled) return { ok: false, disabled: true, state: "off" };
  return {
    ok: Boolean(r.connected),
    state: r.state || (r.connected ? "connected" : "down"),
    error: r.connected ? undefined : (r.error || r.state || "down"),
    url: r.url,
  };
}

/** Tunnel health from the LOCAL side — asks the dashboard's /api/relay whether the bridge is actually
 *  connected. No public URL needed; reflects the real tunnel, not just "is the VPS up". Never throws. */
export async function probeTunnel(dashboardUrl, timeoutMs = 4000) {
  try {
    const ctl = new AbortController();
    const timer = setTimeout(() => ctl.abort(), timeoutMs);
    const res = await fetch(dashboardUrl + "/api/relay", { signal: ctl.signal });
    clearTimeout(timer);
    if (!res.ok) return { ok: false, status: res.status, error: "http " + res.status };
    return interpretRelay(await res.json());
  } catch (e) { return { ok: false, error: (e && e.name) || "error" }; }
}

/**
 * The whole-system snapshot the CLI/GUI renders: each unit's systemd state + a few health probes (the local
 * dashboard serving, the internet reachable, and the bridge/tunnel actually connected — read from the local
 * dashboard's /api/relay, so it works with no config; an explicit relayUrl overrides it with a raw HTTP ping
 * to the public gateway instead). Read-only; safe to poll.
 */
export async function gatherStatus({ dashboardUrl = "http://localhost:3001", relayUrl = null, internetUrl = "https://cloudflare.com/cdn-cgi/trace", omniUrl = "http://127.0.0.1:20128" } = {}) {
  const units = readAllUnits();
  const [dashboard, internet, tunnel, omniroute] = await Promise.all([
    probeHttp(dashboardUrl + "/api/version"),
    probeHttp(internetUrl),
    relayUrl ? probeHttp(relayUrl) : probeTunnel(dashboardUrl),
    probeHttp(omniUrl + "/api/health"),   // OmniRoute "online AND serving" — green only when it actually answers
  ]);
  // Only CRITICAL units drive "overall" — OmniRoute is optional, so its being down/absent is NOT a failure.
  const criticalUp = units.filter((u) => u.critical).every((u) => u.running);
  // A bridge that's ENABLED but not connected is a real degradation; a disabled bridge (or n/a) is not.
  const tunnelBad = tunnel && tunnel.disabled !== true && tunnel.ok === false;
  return {
    units,
    probes: { dashboard, internet, tunnel, omniroute },
    overall: criticalUp && dashboard.ok
      ? (tunnelBad ? "degraded" : "up")
      : units.some((u) => u.critical && u.failed) ? "failed" : "degraded",
  };
}
