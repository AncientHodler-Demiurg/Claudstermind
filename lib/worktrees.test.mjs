...
import { worktreesRoot, listWorktrees, createWorktree, removeWorktree, mergeWorktree, needsInstall } from "./worktrees.mjs";
...
test("mergeWorktree: clean branch merges into main; uncommitted is refused; a conflict aborts and leaves main untouched", { skip: !hasGit }, () => {
  const { root, repoRel, repoAbs } = fixture();
  try {
    // 1) A worktree with a COMMITTED new file merges cleanly into main.
    const a = createWorktree(root, repoRel, "wt-a"); assert.equal(a.ok, true, a.error || "");
    writeFileSync(join(a.dir, "added.txt"), "from wt-a\n");
    git(a.dir, "add", "-A"); git(a.dir, "commit", "-qm", "add file in wt-a");
    const m = mergeWorktree(root, repoRel, "wt-a");
    assert.equal(m.ok, true, m.error || "");
    assert.ok(existsSync(join(repoAbs, "added.txt")), "the worktree's committed file is now in main");

    // 2) UNCOMMITTED work in a worktree is refused (a merge only takes commits).
    const b = createWorktree(root, repoRel, "wt-b"); assert.equal(b.ok, true, b.error || "");
    writeFileSync(join(b.dir, "wip.txt"), "not committed\n");
    const mb = mergeWorktree(root, repoRel, "wt-b");
    assert.equal(mb.ok, false); assert.match(mb.error, /uncommitted/i);

    // 3) A real conflict aborts and leaves main EXACTLY as it was (no half-merged tree).
    const c = createWorktree(root, repoRel, "wt-c"); assert.equal(c.ok, true, c.error || "");
    writeFileSync(join(c.dir, "package.json"), JSON.stringify({ name: "@demo/demo", version: "2.0.0-wt" }));
    git(c.dir, "add", "-A"); git(c.dir, "commit", "-qm", "bump in wt-c");
    writeFileSync(join(repoAbs, "package.json"), JSON.stringify({ name: "@demo/demo", version: "9.9.9-main" }));
    git(repoAbs, "add", "-A"); git(repoAbs, "commit", "-qm", "bump in main");
    const mc = mergeWorktree(root, repoRel, "wt-c");
    assert.equal(mc.ok, false);
    assert.equal(mc.conflict, true);
    assert.ok(mc.files.some((f) => f.includes("package.json")), "the conflicting file is reported");
    assert.match(readFileSync(join(repoAbs, "package.json"), "utf8"), /9\.9\.9-main/, "main's version is untouched");
    assert.equal(git(repoAbs, "status", "--porcelain").stdout.trim(), "", "no half-merged/uncommitted state left in main");
  } finally { rmSync(root, { recursive: true, force: true }); }
});

test("mergeWorktree: untracked/scratch files in MAIN don't block a merge; a tracked mod still does", { skip: !hasGit }, () => {
  const { root, repoRel, repoAbs } = fixture();
  try {
    const w = createWorktree(root, repoRel, "wt-s"); assert.equal(w.ok, true, w.error || "");
    writeFileSync(join(w.dir, "feat.txt"), "feature\n"); git(w.dir, "add", "-A"); git(w.dir, "commit", "-qm", "feat in wt-s");
    // main has ONLY untracked scratch (no tracked mods) — must NOT block the merge (the reported confusion:
    // a plain `git commit` leaves new files untracked, and they have nothing to do with either branch).
    writeFileSync(join(repoAbs, "_probe.tmp"), "scratch\n");
    writeFileSync(join(repoAbs, "_draft.repl"), "(expect 1 1 1)\n");
    const m = mergeWorktree(root, repoRel, "wt-s");
    assert.equal(m.ok, true, "untracked scratch in main must be ignored by the merge; got: " + m.error);
    assert.ok(existsSync(join(repoAbs, "feat.txt")), "the merge actually landed");
    // …but a TRACKED, uncommitted modification in main DOES still block it (safety preserved).
    const w2 = createWorktree(root, repoRel, "wt-t"); assert.equal(w2.ok, true, w2.error || "");
    writeFileSync(join(w2.dir, "g.txt"), "g\n"); git(w2.dir, "add", "-A"); git(w2.dir, "commit", "-qm", "g in wt-t");
    writeFileSync(join(repoAbs, "package.json"), JSON.stringify({ name: "@demo/demo", version: "5.0.0" }));   // tracked mod, not committed
    const m2 = mergeWorktree(root, repoRel, "wt-t");
    assert.equal(m2.ok, false);
    assert.match(m2.error, /TRACKED/i);
  } finally { rmSync(root, { recursive: true, force: true }); }
});
