...
test("deploySteps yields package → ship → rebuild(relay-only) → cleanup, no .env in the tar", () => {
  ...
  assert.match(rebuild, /docker compose up -d --build relay/, "relay service ONLY (no caddy)");
  assert.match(rebuild, /CM_VERSION=0\.3\.0/, "stamps the version into the build");
  assert.ok(!rebuild.includes(";rm"), "no destructive box ops beyond the documented ones");
});
