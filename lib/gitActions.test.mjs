...
test("resolveRepo rejects a SIBLING that merely string-prefixes the root", () => {
  // root = <tmp>/ga-esc  ;  a sibling <tmp>/ga-escEVIL string-prefixes it but is OUTSIDE.
  const base = mkdtempSync(join(tmpdir(), "ga-esc-base-"));
  const root = join(base, "root");
  const evil = join(base, "rootEVIL");
  mkdirSync(root); mkdirSync(evil); mkdirSync(join(evil, ".git"));   // a real .git in the sibling
  assert.equal(resolveRepo("../rootEVIL", root), null, "a sibling sharing the root name prefix must not resolve");
