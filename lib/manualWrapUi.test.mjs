...
test("wiring: both workspaces gate opening the dialog on ChatShell.wrapReadiness, not a re-derived check", () => {
  ...
  // 3, not 2: the two dialogs, plus the phone's wrap BUTTON, which now reads the same decision to show
  // its threshold and dim itself when it would be refused. That third call site is the point of this
  // test rather than an exception to it — the button and the action it launches must agree about
  // whether a wrap can happen, and they can only agree by asking the same function. Before it existed
  // the fallback button said a flat "↺ Wrap" in every state, including the states where it refused.
  assert.equal(hits, 3, "Core's wsOpenWrapDialog, Pact's pactOpenWrapDialog and the cockpit's wrap line must each call the shared readiness check");
...
test("wiring: an empty side of the span is printed by the shared spanLabel, in words, in BOTH dialogs", () => {
  // "why would wrap show 127 answers and no prompts? that can't be right, right?" — it was right, and
  // the header chip already said so in words while the dialog still printed a backwards range
  // ("P#24–P#23 [0]") from the same numbers. Two hand-copies of one sentence; now one function.
  const hits = (src.match(/window\.ChatShell\.spanLabel\(/g) || []).length;
  assert.equal(hits, 4, "both dialogs must label both sides through the shared helper");
  assert.ok(!/"Responses  R#" \+ span\.r\.from/.test(src), "no hand-built range may survive — it cannot render an empty side");
  assert.ok(!/"Prompts    P#" \+ span\.p\.from/.test(src), "same for the prompt side, which is the one that goes empty");
  assert.ok(src.includes("WS_EMPTY_SIDE_WHY"), "and the dialog must explain WHY a side can be empty, where the number is");
});
