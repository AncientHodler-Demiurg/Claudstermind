...
export function createWorktree(root, repoPath, name) {
  ...
  const dir = join(worktreesRoot(root), repoSlug(repoPath), name);
  /* SAY WHAT TO DO, not just what went wrong. "already exists" is true and useless: the worktree you
     asked for is RIGHT THERE in the list you just came from, and the next move is to pick it, not to
     invent a second name. Reported as "I couldn't create a workspace" by someone who had one. */
  if (existsSync(dir)) return { ok: false, error: `A worktree named "${name}" already exists — pick it from the list instead of creating it.` };
...
/**
 * Merge a worktree's branch into the repo's current (main) branch — CONFLICT-SAFE: if the merge would
 * conflict, it is aborted and the main checkout is left EXACTLY as it was, with the conflicting files
 * reported for a manual resolution. Both sides must be committed-clean first (a merge combines COMMITS,
 * not working-tree edits — uncommitted work would be silently left behind or block the merge). This is the
 * deliberate, visible "combine the parallel work-streams" step. Returns { ok, merged, mainBranch } or
 * { ok:false, error, conflict?, files? }.
 */
export function mergeWorktree(root, repoPath, name) {
  if (!SAFE_NAME.test(name || "")) return { ok: false, error: "Invalid name." };
  const repoAbs = resolveRepo(root, repoPath);
  if (!isGitRepo(repoAbs)) return { ok: false, error: "Not a git repository." };
  const dir = join(worktreesRoot(root), repoSlug(repoPath), name);
  if (!existsSync(dir)) return { ok: false, error: `No worktree named "${name}".` };
  const mainBranch = git(repoAbs, ["rev-parse", "--abbrev-ref", "HEAD"]).stdout.trim();
  if (!mainBranch || mainBranch === "HEAD") return { ok: false, error: "The main checkout is in a detached HEAD — checkout a branch first." };
  if (mainBranch === name) return { ok: false, error: `The main checkout is on branch "${name}" itself — nothing to merge.` };
  // A merge only combines commits, so both sides must be committed: uncommitted worktree edits would be
  // left behind; uncommitted main edits would make the merge unsafe/blocked.
  // Worktree side: its own uncommitted work (incl. new/untracked files — the agent's edits) must be
  // committed to be part of the merge, so check everything.
  if ((git(dir, ["status", "--porcelain"]).stdout || "").trim())
    return { ok: false, error: `Worktree "${name}" has uncommitted changes — commit them there first (a merge only takes committed work).` };
  // Main side: only TRACKED modifications block a merge (git would refuse to overwrite them). NEW/untracked
  // files in main don't affect the merge (e.g. scratch/probe files) — ignore them (`-uno`) so a merge isn't
  // wedged by leftover scratch that has nothing to do with either branch.
  if ((git(repoAbs, ["status", "--porcelain", "-uno"]).stdout || "").trim())
    return { ok: false, error: "The main checkout has uncommitted TRACKED changes — commit or stash them before merging (new/untracked scratch files are fine)." };
  const ahead = Number((git(repoAbs, ["rev-list", "--count", `${mainBranch}..${name}`]).stdout || "0").trim());
  if (!ahead) return { ok: false, error: `Nothing to merge — "${name}" has no commits beyond ${mainBranch}.` };
  const m = git(repoAbs, ["merge", "--no-ff", "-m", `merge worktree ${name} into ${mainBranch}`, name]);
  if (m.status === 0) return { ok: true, merged: ahead, mainBranch };
  // Conflict (or any other failure): ABORT so main is left exactly as it was — never a half-merged tree.
  const files = (git(repoAbs, ["diff", "--name-only", "--diff-filter=U"]).stdout || "").trim().split("\n").filter(Boolean);
  git(repoAbs, ["merge", "--abort"]);
  return { ok: false, conflict: files.length > 0, files,
    error: files.length
      ? `Merge has conflicts in ${files.length} file(s) — aborted, main untouched. Resolve manually (git merge ${name} in the main checkout).`
      : `Merge failed: ${String(m.stderr || m.stdout).trim().slice(-200)} — aborted, main untouched.` };
}
