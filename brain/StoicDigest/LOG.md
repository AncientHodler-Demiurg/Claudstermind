...
## 2026-09-12 (research + reframe + companion 006)

**"First for Arweave" RETRACTED after research (owner asked to verify before publishing).** Web-checked: deterministic Arweave-from-mnemonic prior art EXISTS — `arweave-mnemonic-keys` (old, buggy/non-deterministic), **ZELF** (from-scratch deterministic RSA-2048 from BIP-39 via node-forge + SHA-256 PRNG, explicitly reproducible), **Wander/ArConnect** wallets offer 12-word recovery phrases, and **Arweave 2.9 added ECDSA/secp256k1 keys** (trivially seed-deterministic). So the claim is indefensible — and the ORIGINAL handoff §12 warned against exactly this. Reframed the flagship's whole section 3 (was "A first for Arweave" → **"The hard part, and what's ours"**): openly acknowledge prior art, drop the superlative, and locate real novelty in (a) **not BIP-39** (custom dictionary-free multilingual 256-glyph seed) and (b) **one seed unifying EC + Apollo + RSA/Arweave identity**, not an Arweave-only tool + cross-language byte-identity + multi-address. Also fixed the overclaim in the summary, the premise line. Sources: zelf.world/blog/building-deterministic-arweave-wallets, npmjs arweave-mnemonic-keys, arweave docs/cookbook, wander.app, arconnect.io.

**Codex tooltip fixed** (owner: runs on OuronetUI AND freely accessible on Mnemosyne, already working) — def updated.

**Worker decision (owner leaning yes):** the live lab allows up to **1000** RSA generations (~1 hour of BigInt crunching) with a progress bar → confirmed we'll use a **Web Worker** for RSA gen. Clarified for owner: a Worker is still 100% client-side JS on a background browser thread — **the site stays fully static, no server**; it just keeps the main UI thread free (smooth progress bar, page stays responsive, no "page unresponsive" prompt) during long runs. EC accounts stay on main thread (instant). Everything still local per client.

**Companion 006 "The Custom Curve" WRITTEN** (`issue-006.mdx`, draft, localhost). level:6 · explainer · Long (17m) · pillars [logos,aletheia] · series the-key-to-arweave (tab flipped `ready:true`). 9 chapters: Portico · Why a key that large (1600/1024 vs 256; permanence-over-efficiency, ~2^800, honest size/speed tradeoff; comparison table DALOS/Apollo/Ed25519/secp256k1) · The DALOS ellipse (`x²+y²=1−26x²y²`, **P=2¹⁶⁰⁵+2315**, Q 1604-bit prime, cofactor 4, gen x=2, d=−26 QNR → complete addition law; verified 2 tools/7 checks) · Apollo (`x²+y²=1−729x²y²`, **P=2¹⁰²⁹+639**, gen x=18, 1024-bit, Codex-identity two-halves → points to 007) · From a phrase to a point (7-fold Blake3→bitstring→clamp→[k]·G→16×16 matrix address Ѻ./Σ.; 6 input modes, 105-vector corpus) · A seed with no dictionary (256-glyph base-256, homoglyph anti-spoofing) · Proving who you are (Schnorr Alice/Bob, cofactor check, replaces login — Ancient Hub auth + Pythia Apollo-ownership) · Why build your own (honest "don't roll your own crypto" — mitigations: independent verification, complete addition law, frozen corpus, Blake3 is standard) · premise. Curve params sourced from codex.ancientholdings.eu/docs/{dalos,apollo}-curve.html (Apollo was [unverified] in handoff — now sourced). Term tooltips: Portico/Ed25519/Twisted-Edwards/cofactor/Codex/Pythia/Blake3.

**Next:** 007 "The Alphabet Is the Address" (Apollo double-half Codex identity + Pythia API-key lifecycle — from `2026-09-12-apollo-identity-and-pythia-api-keys.md`), then the live `/lab` PoC (with Web Worker).

## 2026-09-12 (flagship written) — Issue #005 "The Key to Arweave" rewritten as the flagship

**Owner: "yes go ahead and write the flagship"** (confirmed the 3 framing calls: accurate DPTF-AR naming w/ gloss, Caduceus-as-roadmap, diplomatic BTC/ETH). Rewrote issue-005 wholesale, **tied to the `@ouronet/dalos-crypto` package** as the throughline. 9 chapters (chapter-dots verified): Portico (Term tooltip) · The seed becomes the key (fork diagram, multi-address 4.4.0) · A first for Arweave (determinism problem, compressed engine — deep dive deferred to companion 006) · **A seed in your own language** (NEW — 256-glyph base-256 charset, homoglyph exclusion, no dictionary, one phrase → EC+Apollo+Arweave identity, links char-set doc) · Native Arweave in Codex (hold/upload/manage permaweb — roadmap) · **Caduceus** (bridge, DESIGN-STAGE, DPTF-AR direct mint w/ gloss "no separate wrapped coin", links caduceus.ancientholdings.eu) · **Monetising STOA the Stoic way** (Epictetus dichotomy-of-control; no exchange listings; sSTOA/DPTF-AR pool; buy/sell path AR→bridge→DPTF-AR→sSTOA→WSTOA→STOA; add liquidity over time; honest "we still depend on Arweave, but chose it") · **Why Arweave** (kinship: both think in centuries/permanence; diplomatic contrast — security-budget Q links issue-004, no name-calling, "our view as a view") · The premise (shipped-vs-roadmap causation chain). 7 `<Term>` tooltips (Portico/BIP-39/Codex/Caduceus/DPTF/SilverStoa). level:5 · announcement · Long (24m) · pillars [agora,logos,aletheia] · featured. Strengthened not-investment-advice disclaimer (buy/sell/liquidity) + roadmap labels + companion/live-demo teaser. draft:true, localhost-only, **NOT deployed**.

**Series model — HYBRID confirmed + built (owner: "yes, we do hybrid"):** each part is its own page/URL/grade/nav, bound by a persistent **series tab bar**. New: `src/lib/series.ts` (central config — tabs can be issues OR non-issue routes like the lab; `ready:false` → dimmed "soon" chip, no dead link) + `src/components/SeriesTabs.astro` (kicker "Series · N of 4", title, tagline, active/link/soon tab states) + CSS `.series-tabs*`. Rendered in `[id].astro` when `entry.data.series` set. issue-005 tagged `series: the-key-to-arweave`. Verified: flagship shows Overview active, other 3 as "soon". Flip each tab's `ready:true` as it ships. Series title (not "Issue #005") is the bar's identity, so companion pages #006/#007 don't clash with the "Issue 5" package feel.

**Next in the series (per confirmed plan):** 006 "The Custom Curve" (DALOS+Apollo EC, equations/primes, Schnorr sign/verify, seed words, charset — gitbook ouronet-cryptography refs); 007 "The Alphabet Is the Address" (Apollo→Pythia API keys + Codex identity — Pythia handoff); then the live `/lab` PoC page (RSA handoff §10 spec: both curves shown, curve-source toggle, 4 gen modes, 1000-cap, combined-progress popup, blur/reveal; needs client-side npm pkg + likely web worker).
...
**Verification (§9):** independently web-verified the §1 comparisons — Bitcoin (→0 ~2140, 21M, 210k halving), **Monero (0.6 XMR/block tail forever — the key precedent)**, Ethereum (PoS, EIP-1559 burn), Kadena (~1B/120yr→0). UrStoa distribution (250k founders/250k ICO/500k foundation) matches ECOSYSTEM-FACTS. §7 respected: **zero security-audit content**; no out-of-scope entities. Used handoff's **corrected** figures (1,000,000 ANU / 339 yr / 2,880 per-chain), NOT the stale gitbook 400k/133yr. Omitted uncertain gitbook URLs; linked the explorer + named on-chain functions as the checkable source instead.

**Also:** fixed **issue-001**'s stale gas-floor figures (400,000→1,000,000 ANU, 133→339 yrs) — deployed live (they contradicted #004). Made **drafts fully private in prod** (`[id].astro` getStaticPaths filters drafts when PROD → 404, verified). Updated `ECOSYSTEM-FACTS.md` gas-floor section (supersedes 400k/133). issue-004 stays **local/private** pending owner review + publish.

**Open for owner:** recompute floor at exact publish date (used ~11,570 ANU @ 2026-09-08); confirm the 600,000≈2026-09-25 hedge; optionally add the two gitbook links (yin-yang-emission-engine / emission-projection) if URLs confirmed.

**Flagship expansion (owner, 2026-09-08):** reworked issue-004 into the investment-thesis piece (~3,900 words / 20 min, draft). Added: **two-camps frame** (subsidy *dies* = BTC/Kaspa/Kadena vs *never dies* = Monero/Solana/StoaChain — web-verified Solana 8%→1.5% terminal, Kaspa 92%+ mined/no tail); the **miner's-question hook** ("how much is left?" / *la spargerea târgului*) + **ceiling & %-minted** analysis (the ceiling runs away, always coin to mine); a **"how the chain grows Yin"** scaling chapter (more chains + bigger blocks + rising floor + usage → Yin can surpass Yang; late arrival is *better*); **store-of-value crescendo** (sound money + Ethereum-grade contracts + Pact safety + Chainweb scale → generational value, restrained/no price claims/kept disclaimer). Pact-safety framed as language-design class-elimination (reentrancy etc.), NOT contradicting issue-002 and NOT touching the audit (§7). **3 SVG data-charts** via new `src/lib/econ.ts` (formulas) + generic `Chart.astro` (dataviz skill: static, gold-on-dark, recessive axes, legend for 2-series). **2 explorer screenshots** via `Shot.astro` placeholders — owner must drop `public/images/issues/004/{gas-schedule,tx-pricing}.png` then remove `pending`. Drafts now render private in prod (verified 404).

**Owner refinements (same session):** dropped the Romanian idiom → professional English; corrected current supply **~18.9M** (not 20M, ~3.8% of ceiling); reframed Solana honestly (web-verified: **1.5% is a permanent *rate* → absolute issuance compounds without bound**; StoaChain's declining tail → inflation→0, tail not approached until 3026 = the differentiator; Monero flat-amount = hard step); added **currency marks ❖ STOA gold (U+2756) / ✦ UrStoa grey (U+2726)** + `.mark-stoa/.mark-urstoa` CSS; linked the **UrStoaVault explorer account** (fueled on chain 0 each block). Screenshots now live (owner supplied `gas-schedule.png`, `tx-pricing.png`; `Shot pending` removed). NB: site header/favicon still use ✦ (now UrStoa's mark) — offered to switch the StoicDigest brand mark to ❖ for consistency (not yet done).

**Mechanism correction (owner, same session):** the vault 10% is **minted extra, not diverted from miners**. Every chain's miner gets 90% of its block; on **chain 0** each block mints an *additional* `10% × 10 chains` (the whole network's staker share) into the UrStoaVault; vault → stakers via **RPS**. UrStoa: 250k founders / 250k ICO (1 per $5, **ICO open to end-2026**) / 500k foundation **+ any unsold ICO** (grows foundation coffers, funds development); only staked earns. Fixed in issue-004 + folded into ECOSYSTEM-FACTS.
