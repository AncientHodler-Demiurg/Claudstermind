...
- **⚠️ BIG SHIFT (2026-08, the "Seer Migration" / "Pantheonica"): the Explorer is now a Pantheon *seer*.** Much of this ONBOARDING predates it — read `STATE.md` for the current architecture. The essentials: (a) there are **two frontends** — `frontend-stoa/` (mature public explorer, port stays public-only, NO login) and `frontend-ouronet/` (greenfield, carries ALL admin behind an `AdminGate`, hostname `ouroscan.ancientholdings.eu`). (b) **8 new backend modules**: `auth` (AncientHub OIDC login + `requireAncient`), `chain-source` (admin-owned runtime ingest node w/ fallback), `pythia` (Ouronet reads routed through Pythia's metered gateway), `codex`/`vault` (server-custody Codex + sealed store), `database` (index lifecycle), `deploy` (on-box blue-green), `update` (organ versions; Khronoton shown-but-not-pulled = the seer marker). (c) A shared **`packages/pantheonic/`** workspace package holds the 3-tier header/theme — themed twice, don't fork it. (d) The seer **signs nothing** and **never adopts Khronoton**. All of this lives on branch `feature/ouronet-explorer` (not merged to master).
